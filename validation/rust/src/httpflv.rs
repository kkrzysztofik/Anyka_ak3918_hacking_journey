...
mod tests {
    use super::{
        ...
        parse_ffmpeg_summary_bitrate_kbps, run_httpflv_validation, url_with_credentials,
        validate_audio_tag_payload_header, validate_video_tag_payload_header,
    };
    use crate::config::{EffectiveConfig, InitialTimestampPolicyArg};
    use crate::report::TestResult;
    ...
    async fn test_run_httpflv_validation_short_flv_header() {
        let (port, handle) =
            spawn_single_response_server("200 OK", vec![0x46, 0x4C, 0x56, 0x01]).await;
