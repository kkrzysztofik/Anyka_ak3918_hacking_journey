...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --output-terminfo-dir) output_terminfo_dir="$2"; shift 2 ;;
    -h|--help)             usage; exit 0 ;;
...
rm -rf "${build_dir}"
tp_fetch_archive "htop-${version}" "tar.xz" "${url}" "${sha256}" "${archive_override}" "${build_dir}"
...
tp_show_binary_info "${output_bin_dir}/htop.bin"
# link_mode is not set for htop (static/dynamic is attempted automatically above);
# tp_bundle_libs defaults to dynamic mode so it bundles without error.
tp_bundle_libs "${output_bin_dir}/htop.bin" "${output_lib_dir}"
