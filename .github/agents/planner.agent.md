...
## Example: Adding a New ONVIF PTZ Operation

```
# Implementation Plan: PTZ ContinuousMove Operation

**Affected layers**: Rust, C
**Key risk**: Anyka SDK motor API blocking behavior unknown — may need async wrapper

## Task 1: Define ContinuousMove types
**Agent**: coder-rust
**File(s)**: cross-compile/onvif-rust/src/onvif/ptz/types.rs
**Depends on**: none

### What to do:
Add `ContinuousMoveRequest` and `ContinuousMoveResponse` structs matching
ONVIF 24.12 tptz:ContinuousMove schema. Derive `Debug`, implement
`quick_xml::de::DeserializeOwned` and `quick_xml::se::Serialize`.

### Acceptance criteria:
- Structs compile on x86_64 target
- XML round-trip test passes (serialize → deserialize → same struct)

## Task 2: Add IPC command ID for continuous move
**Agent**: coder-c + coder-rust (coordinated)
**File(s)**:
  - cross-compile/vendor-daemon/src/main.c (add CMD_PTZ_CONTINUOUS = 0x30)
  - cross-compile/onvif-rust/src/platform/ipc.rs (add matching constant)
**Depends on**: none (parallel with Task 1)

### What to do:
Reserve command ID 0x30 for PTZ continuous move. Document in the wire
protocol comment block in main.c. Add Rust constant in ipc.rs.

### Acceptance criteria:
- Both files use identical numeric value (0x30)
- No conflict with existing command IDs (audit the table)

## Task 3: Implement platform trait method
**Agent**: coder-rust
**File(s)**: cross-compile/onvif-rust/src/platform/mod.rs
**Depends on**: Task 2

### What to do:
Add `async fn ptz_continuous_move(&self, pan_speed: f32, tilt_speed: f32)
-> Result<(), PlatformError>` to the `Platform` trait and
`AnykaPlatform` implementation. The implementation sends CMD_PTZ_CONTINUOUS
over /tmp/vd-ctrl.sock.

### Acceptance criteria:
- Trait method signature matches
- MockPlatform can be generated by mockall for tests
