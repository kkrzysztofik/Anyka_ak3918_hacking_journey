...
LIB_DIR_DEFAULT="/mnt/anyka_hack/onvif/lib"
if [ -d "$LIB_DIR_DEFAULT" ]; then
    LIB_DIR="$LIB_DIR_DEFAULT"
else
    LIB_DIR="$SCRIPT_DIR/lib"
fi
BIN_PATH="$SCRIPT_DIR/onvif-rust.bin"
...
if contains_text "$BIN_PATH" "venc_encoder_init failed: register map unavailable"; then
    ok "Binary contains fail-fast VENC map guard"
else
    warn "Binary missing VENC map fail-fast marker (may be older build)"
fi

if contains_text "$BIN_PATH" "Skipping no-data IDR recovery before first frame"; then
    ok "Binary contains IDR-before-first-frame guard"
else
    warn "Binary missing IDR-before-first-frame guard marker"
fi

if contains_text "$BIN_PATH" "kernel_map_size=0x%x" \
    || contains_text "$BIN_PATH" "/sys/class/uio/uio0/maps/map%u/size" \
    || contains_text "$BIN_PATH" "capping mmap size"; then
    ok "Binary contains libakuio short-UIO-map compatibility marker"
else
    warn "Binary missing libakuio short-UIO-map marker (may fail on short UIO map windows)"
fi
