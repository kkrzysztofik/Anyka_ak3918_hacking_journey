...
2) open second telnet and home the camera axes `echo "init" > /mnt/tmp/ptz.daemon`
3) move to whatever position you want `echo "t2p 190 95" > /mnt/tmp/ptz.daemon` 190 degree horizontal, 40 vertical (0 is top)
4) quit the daemon if you want with `echo "q" > /mnt/tmp/ptz.daemon`, but it can just always run in the background
...
```
echo "t2p 300 50">/mnt/tmp/ptz.daemon
echo "up">/mnt/tmp/ptz.daemon
echo "left">/mnt/tmp/ptz.daemon
```
