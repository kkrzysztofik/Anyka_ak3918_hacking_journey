 ...
 */
export async function getImagingSettings(videoSourceToken: string = 'VideoSource_1'): Promise<ImagingSettings> {
...
export async function setImagingSettings(
  ...
  const settingsXml = [
    settings.brightness !== undefined ? `<tt:Brightness>${settings.brightness}</tt:Brightness>` : '',
    settings.contrast !== undefined ? `<tt:Contrast>${settings.contrast}</tt:Contrast>` : '',
    settings.saturation !== undefined ? `<tt:ColorSaturation>${settings.saturation}</tt:ColorSaturation>` : '',
