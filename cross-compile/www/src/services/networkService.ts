...
export async function getDNS(): Promise<DNSConfig> {
  ...
  return {
    ...
    searchDomain: Array.isArray(searchDomain) ? searchDomain.map(String) : searchDomain ? [String(searchDomain)] : [],
    dnsServers: Array.isArray(dnsServers)
      ? dnsServers.map((d: Record<string, unknown>) => String(d.IPv4Address || ''))
      : dnsServers ? [String((dnsServers as Record<string, unknown>).IPv4Address || '')] : [],
  }
}
...
export async function getNetworkConfig(): Promise<NetworkConfig> {
  const [interfaces, dns] = await Promise.all([
...
export async function setNetworkInterface(
  ...
  const manualConfig = !dhcp && address
...
export async function setDNS(fromDHCP: boolean, dnsServers?: string[]): Promise<void> {
  const manualDNS = !fromDHCP && dnsServers?.length
    ? dnsServers.map(ip => `<tds:DNSManual><tt:Type>IPv4</tt:Type><tt:IPv4Address>${ip}</tt:IPv4Address></tds:DNSManual>`).join('')
    : ''
