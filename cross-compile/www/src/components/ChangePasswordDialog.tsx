...
import { setUser, getUsers } from '@/services/userService'
  ...
  const onSubmit = async (data: ChangePasswordFormData) => {
    ...
    try {
      ...
      if (!verifyResult.success) {
        form.setError('currentPassword', { message: 'Invalid current password' })
        setIsLoading(false)
        return
      }
        ...
        <DialogHeader>
          <div className="flex items-center gap-3 mb-2">
            <div className="size-10 bg-accent-red/10 rounded-lg flex items-center justify-center">
              <KeyRound className="size-6 text-accent-red" />
            </div>
            <div>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 py-4">
            ...
            <DialogFooter className="pt-4">
              <Button
                ...
                disabled={isLoading}
                className="text-dark-secondary-text hover:text-white hover:bg-dark-hover"
              >
                Cancel
              </Button>
              <Button
                type="submit"
                disabled={isLoading}
                className="bg-accent-red hover:bg-accent-red/90 text-white min-w-[140px]"
              >
                {isLoading ? (
                  <>
