...
const SystemInfo: React.FC<SystemInfoProps> = ({ cameraIP, onvifStatus, onStatusCheck }) => {
  ...
  const getStatusIcon = () => {
    switch (onvifStatus) {
      ...
      default:
        return (
          <div className="border-accent-red h-4 w-4 animate-spin rounded-full border-2 border-t-transparent" />
        );
    }
  };
  ...
  return (
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div className="space-y-3 text-sm">
          {endpoints.map((endpoint, index) => (
            <div key={index} className="flex justify-between">
              ...
              <code className="break-all text-blue-400">{endpoint.url}</code>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* ONVIF Status */}
      <div className="card">
        <h2 className="mb-4 text-xl font-semibold">ONVIF Status</h2>
