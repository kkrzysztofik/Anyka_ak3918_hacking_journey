...
#define PMEM_ALIGN_BYTES	(4096UL)
#define PMEM_MIN_ALLOC_BYTES	(4UL * 1024UL * 1024UL)
#define PMEM_RETRY_STEP_BYTES	(1UL * 1024UL * 1024UL)
...
static unsigned long get_pmem_cap_from_env(void)
{
	const char *cap_mb_env = getenv("AKUIO_PMEM_CAP_MB");
	char *endptr = NULL;
	unsigned long cap_mb = 0;

	if (!cap_mb_env || !cap_mb_env[0]) {
		return 0;
	}

	cap_mb = strtoul(cap_mb_env, &endptr, 10);
	if (!endptr || *endptr != '\0' || cap_mb == 0) {
		LOGW("invalid AKUIO_PMEM_CAP_MB='%s', ignoring\n", cap_mb_env);
		return 0;
	}

	return cap_mb * 1024UL * 1024UL;
}

static unsigned long choose_initial_pmem_len(unsigned long kernel_reserved_len)
{
	unsigned long requested = kernel_reserved_len;
	unsigned long env_cap = get_pmem_cap_from_env();

	if (env_cap > 0 && env_cap < requested) {
		LOGW("AKUIO_PMEM_CAP_MB active: capping PMEM request 0x%08lx -> 0x%08lx\n",
			requested, env_cap);
		requested = env_cap;
	}

	requested = LEFT_ALIGN(requested, PMEM_ALIGN_BYTES);

	if (requested < PMEM_MIN_ALLOC_BYTES && kernel_reserved_len >= PMEM_MIN_ALLOC_BYTES) {
		requested = PMEM_MIN_ALLOC_BYTES;
	}
	if (requested > kernel_reserved_len) {
		requested = LEFT_ALIGN(kernel_reserved_len, PMEM_ALIGN_BYTES);
	}
...
{
	unsigned int munmap_size = size;

	if (uio->mapped_size > 0 && munmap_size > uio->mapped_size) {
		LOGW("clamping munmap size %u -> %u for paddr=0x%08x\n",
			munmap_size, uio->mapped_size, uio->paddr);
		munmap_size = uio->mapped_size;
	}

#if TRACE_UIO
	LOGD ("%s: unmap %p(total size: %i), size = %i\n", __func__, 
		uio->vaddr, uio->size, munmap_size);
#endif

	if(-1 == munmap(uio->vaddr, munmap_size)) {
		LOGE("##CALL munmap FAILED3,uio->vaddr[%p]size[%d]\n", uio->vaddr, munmap_size);
	}
	...
	if ((uio->size == size) || (uio->mapped_size == munmap_size)) {
		...
		uio->vaddr += munmap_size;
		uio->size -= munmap_size;
		if (uio->mapped_size >= munmap_size) {
			uio->mapped_size -= munmap_size;
		} else {
			uio->mapped_size = 0;
		}
	}
...
int akuio_pmem_init(void)
{
	...
	pmem.length = choose_initial_pmem_len(pmem_size);
	iad.flags = ION_HEAP_TYPE_CARVEOUT | ION_HEAP_TYPE_SYSTEM_CONTIG;
	iad.align = PMEM_ALIGN_BYTES;
	iad.heap_id_mask = 0xFF;
	{
		unsigned long try_len = pmem.length;
		unsigned long min_try = PMEM_MIN_ALLOC_BYTES;
		int alloc_ok = 0;

		if (try_len == 0) {
			LOGE("invalid PMEM request length: 0\n");
			goto _pmem_err_1;
		}
		if (try_len < min_try) {
			min_try = PMEM_ALIGN_BYTES;
		}

		while (try_len >= min_try) {
			iad.len = try_len;
			...
			LOGW("ION alloc failed for %lu bytes (errno=%d: %s), retrying smaller block\n",
				try_len, errno, strerror(errno));

			if (try_len <= min_try) {
				break;
			}
			if (try_len > (min_try + PMEM_RETRY_STEP_BYTES)) {
				try_len -= PMEM_RETRY_STEP_BYTES;
			} else {
				try_len = min_try;
			}
		}
		...
		if (!alloc_ok) {
			fprintf(stderr, "ION: alloc failed for process %d (tried down to %lu bytes)\n",
				getpid(), try_len);
			goto _pmem_err_1;
		}
	...
	fprintf(stdout, "[%s]ION: alloc %lu bytes for process %d\n",
		MODULE_TAG, (unsigned long)iad.len, getpid());
	...
	fprintf(stdout, "[%s]ION: yield %d for iad 0x%x, len: %lu\n",
		MODULE_TAG, ifd.fd, (unsigned int)&iad, (unsigned long)iad.len);
...
void* akuio_map_regs(unsigned int paddr, unsigned int size)
{
    ...
    /* Read kernel-reported mapping size and cap mmap request.
     * The AK3918 kernel reports only 0x430 bytes for the video encoder
     * register region at 0x20020000, but callers (e.g. venc_cb_init_reg)
     * request 0x10000. The kernel UIO mmap handler rejects requests that
     * exceed the mapping size with EINVAL. */
    unsigned int mmap_size = size;
    unsigned int kernel_map_size = 0;
    sprintf(path, "/sys/class/uio/uio0/maps/map%u/size", j);
    fd = fopen(path, "r");
    if (fd) {
        fscanf(fd, "0x%x", &kernel_map_size);
        fclose(fd);
    }
    if (kernel_map_size > 0) {
        unsigned int page_aligned = (kernel_map_size + getpagesize() - 1)
                                    & ~(getpagesize() - 1);
        if (page_aligned < mmap_size) {
            LOGD("[akuio_map_regs] capping mmap size 0x%x -> 0x%x "
                 "for paddr 0x%08x (kernel reports 0x%x)\n",
                 mmap_size, page_aligned, paddr, kernel_map_size);
            mmap_size = page_aligned;
        }
    }
	...
	uio->uio_fd    =  uio0_fd ;
	uio->paddr     = paddr;
	uio->vaddr     = mmap (NULL, mmap_size, PROT_READ|PROT_WRITE, MAP_SHARED,
	                uio->uio_fd, uio->map_index*getpagesize());
	uio->offset    = uio_offset;
	uio->size      = size;  /* Keep ORIGINAL size for akuio_unmap_regs compat */
	uio->mapped_size = mmap_size;
    ...
    if (uio->vaddr == MAP_FAILED) {
        LOGE ("mmap() failed: %s (paddr=0x%08x, req_size=0x%x, mmap_size=0x%x, "
              "kernel_map_size=0x%x, map_index=%d)\n",
              strerror(errno), paddr, size, mmap_size, kernel_map_size, j);
        free (uio);
        return NULL;
    }
...
unsigned int akuio_get_mapped_regs_size(void *vaddr)
{
	...
	list_for_each_entry(uio, &uio0_list_head, list) {
		if ((uio->vaddr + uio->offset) == vaddr) {
			return uio->mapped_size;
		}
	}
