...
#define NUM_GETWIFIAP_RESPONSE 5                                                                    // Number of retries when wifi list upload fails
#define DANA_VIDEO_BUFFER_SIZE 3*1024*1024                                                          // default size is 2mb (Set Danale maximum video buffer size)
	   	...
	   	uint32_t trans_id, void *cmd_arg, uint32_t cmd_arg_len)
{
	...
	switch (cmd) {
		...
		case DANAVIDEOCMD_GETWIFIAP: {
				...
				if(dana_wifi_driver_chk() < 0) {                                                    // Check if wifi driver is loaded
				...
				if(wifi_list == NULL) {
					...
					for(j = 0; j < ap_list->ap_num; j++) {
						wifi_list[j].enc_type = ap_list->ap_info[j].enc_type;  // SSID encryption type
						wifi_list[j].quality  = ap_list->ap_info[j].quality;   // Signal quality
						strcpy(wifi_list[j].essid, ap_list->ap_info[j].essid); // SSID name
				...
				for( i = 0 ; i < NUM_GETWIFIAP_RESPONSE ; i ++ ) {                  // If wifi list upload fails, retry upload
		...
		case DANAVIDEOCMD_SETWIFI: {
			...
			*/
			if((setwifi_arg->essid != NULL) && (0 != strcmp(psys_wifi->ssid,setwifi_arg->essid))){
				...
				*/
				if( AK_TRUE == dana_wifi_conn_new_ssid(wifi_ifo->ssid,wifi_ifo->passwd)){ // Attempt to connect to the currently set wifi
					ak_config_set_sys_wifi(wifi_ifo,1);                         // Write current wifi configuration to the global configuration file
				}
				else{
					ak_print_normal("\nset wifi ap fail\n");
				}
		...
		case DANAVIDEOCMD_RESOLVECMDFAILED: {
				...
				// Call lib_danavideo_cmd_response according to different methods
				error_code = 20145;
...
#if 1
	// Called before lib_danavideo_start, default is fixed 34102
...
/*
#if 0
	// Called after lib_danavideo_start, default is fixed 34102
	//bool listen_port_fixed = true; // false
	listen_port_fixed = true;
...
#endif
*/
	//lib_danavideo_cloud_set_cloud_mode_changed(anyka_cloud_changed_callback); // Register cloud storage plan change callback function
