...
#define NUM_TRY_WIFI_CONNECT    5                                                                   //wifi connection attempt count
#define LEN_BUFF 65536
#define NUM_WIFI_CHK            5                                                                   //number of checks for wifi connection
                                                                                                    //wpa_cli -iwlan0 status can be used here to get ssid and passwd, then try to retrieve the ssid
                                                                                                    //suitable for judging here whether the wifi connection can succeed
#define CMD_WIFI_STAT_FILE "cat /sys/class/net/wlan0/operstate"                                     //currently using operstate to check and judge
#define WIFI_STAT_SUCCESS  "up"                                                                     //flag indicating wifi is connected and running
#define FILE_WIFI_TMP      "/tmp/wifi_info"                                                         //temporary file storing wifi info
/*
 * wifi ap data struct.
...
int dana_wifi_driver_chk(void)
{
	...
	for( i = 0 ; i < NUM_WIFI_CHK ; i ++ ) {
		...
		if(0 == strlen(result)) {                                                           //wlan0 device not found, wifi not loaded
			ak_print_normal_ex("the wifi device is not working, start it.\n");
			memset( result , 0 , LEN_WIFI_CHK ) ;
			ak_cmd_exec("wifi_driver.sh station", result , LEN_WIFI_CHK );              //load driver
			memset( result , 0 , LEN_WIFI_CHK ) ;
			ak_cmd_exec("ifconfig wlan0 up", result , LEN_WIFI_CHK );                   //bring up interface
		} else {                                                                            //wlan0 device found, exit
...
static int check_wifi_connection(const char *ssid)
{
	...
	while(result[i]){                                                                           //replace the first carriage return with a string terminator
		if (result[i]=='\n' || result[i]=='\r'){
			result[i] = '\0';
			break;
		}
...
int dana_wifi_conn_new_ssid(const char *ssid, const char* pswd)
{
	...
	i_wifi_curret = check_wifi_connection( NULL ) ;                                             //get current wifi connection status
	 ...
	 */
	save_wifi_info2tmp(ssid, pswd);                                                             //save current wifi info to temporary file

	ak_print_normal_ex(" want to set this info: ESSID: %s, PSK: %s\n",
				 ssid, pswd);
	/* waiting for driver install and wpa_supplicant run */
	ak_cmd_exec( "wifi_manage.sh start" , ac_buff_shellres , LEN_BUFF ) ;                       //execute wifi connection script

	for( i = 0 ; i < NUM_TRY_WIFI_CONNECT ; i ++ ) {                                            //loop to check if wifi connection succeeded
	...
	if ( ret == AK_FALSE ) {
		...
		ak_cmd_exec( ac_cmd , ac_buff_shellres , LEN_BUFF ) ;                               //delete temporary wifi info file
		if( i_wifi_curret == AK_TRUE ) {                                                    //currently using wifi connection
			ak_cmd_exec( "wifi_manage.sh start" , ac_buff_shellres , LEN_BUFF ) ;       //restart using config file wifi settings to reconnect
		}
		else {
			ak_cmd_exec( "wifi_manage.sh stop" , ac_buff_shellres , LEN_BUFF ) ;        //unload wifi driver and corresponding services
			ak_cmd_exec( "eth_manage.sh start" , ac_buff_shellres , LEN_BUFF ) ;        //start ethernet
		}
	}
	/*
	if (0 == ret) {
