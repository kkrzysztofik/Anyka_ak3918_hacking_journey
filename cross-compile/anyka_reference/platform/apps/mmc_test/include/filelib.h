...
{
    T_U32  driver;               //
    T_U32  first_parentID;       // Root directory of the first level
    T_U16 *fileName;             //
    T_S32  find_cnt;             // Forward or backward?
    T_U32  folder_lastfdt;       // Forward or backward?
    T_U32  folder_firstfdt;      // First one
    T_U16 *pattern;              // Compare with Filter_Match, the buffer was provided by caller.
    T_U16 *fileter_pattern;      // Compare with Filter_Match to fileter, the buffer was provided by caller.
    T_U8   updatedir_flag;       // Flag for searching up one level
    T_U8   Dirorfolder_flag;     // Flag for root directory or folder
    T_BOOL deep_flag;   // If searching forward for the last one
    T_BOOL forward_find_flag;   // If searching forward for the last one
    T_BOOL backward_find_flag;   // If searching forward for the last one
