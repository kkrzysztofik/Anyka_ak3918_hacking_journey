...
typedef 
#ifndef __GNUC__ // Used for ADS compilation
#ifndef WIN32
        __packed
#endif
#endif
...
#ifdef  __GNUC__ // Used for GCC compilation
...
struct tag_FolderInfo_chkdsk
{
    ...
    unsigned long start_sector;  // First sector of the FDT
}
#ifdef  __GNUC__ // Used for GCC compilation
__attribute__((packed))
#endif
...
#define FILE_FIND_DEEP_CNT       7    // Max search depth cannot exceed 6 = 7 - 1
#define FIND_FILENAME_PATHLEN    262  // Max path length
...
struct tag_FileInfo_BeforePower
{
    ...
    unsigned long deep_cnt;          // Current depth
}
#ifdef  __GNUC__ // Used for GCC compilation
__attribute__((packed))
#endif
...
{
    T_PFILEINFO_BEFOREPOWER find_fileinfo; // Records first cluster number and FDT for each file level
 ...
 * RETURN:     return  pFilePosInfo  for success, otherwise return ak_null,   free pFilePosInfo  by File_GetfileclusterInfo_ToFree()
**************************************************************************/
// This interface obtains the logical addresses of files and folders
 ...
 * RETURN:     return (T_U32)file ,fasle return 0,
 Notes:
 This interface is only for special use cases and only supports opening in read-only and overlay modes.
 This file must exist; if not, an error will be raised. Also, the file size cannot be 0, otherwise an error is indicated.
 It only supports writing no more than the original file size; if exceeded, a write error is indicated.
 T_U32 FDTOffset should be the first position of the FDT, which can be obtained through File_GetFileinfo_fastopen
 , or obtained through File_get_findfile_info_WithID(T_U32 obj, T_PFILEINFO_BEFOREPOWER fileinfo);.
 However, it cannot be obtained through the File_FindInfo_WithID interface, as this interface calls an old interface that records short filename FDTs.
**************************************************************************/
