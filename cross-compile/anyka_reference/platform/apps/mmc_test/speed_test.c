...
int start_speed_test( )
{
	...
	struct timeval timeval_start, timeval_last;                                                     // Test time for TF card defect detection

	timeval_mark( &timeval_start );

	gpc_buff_speed = ( unsigned char * )calloc( 1, gi_speed_sector * SEC_SIZE + LEN_OFFSET );
	gpc_addr_speed = gpc_buff_speed + SIZE_SECTOR - ((unsigned long)gpc_buff_speed & SIZE_SECTOR_ALIGN); // Get an aligned address pointer

	i_sector_now = gi_speed_offset;                                                                 // Get the starting sector offset
	for( i = 0 ; i < g_card_result.ai_status_point[ TEST_STATUS_SPEED ] ; i ++ ) {
		...
		if ( i_sector_now + gi_speed_sector < g_card_result.i_total_sector ) {                      // Set current number of sectors to read
		...
		i_times_test ++;
		if ( speed_sector( i_sector_now, i_sector_num) == AK_FAILED ) {                             // ***** Entry function for TF card speed detection *****
			i_times_error ++;
		}
