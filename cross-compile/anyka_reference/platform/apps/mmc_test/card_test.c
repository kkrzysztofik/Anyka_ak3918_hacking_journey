...
int start_check_disk( )
{
	...
	struct timeval timeval_start, timeval_last;                                                     //test elapsed time for TF card defect detection
	...
	pc_erase_buff = ( unsigned char * )calloc( 1, SEC_SIZE + LEN_OFFSET );                          //allocate one sector length; after erase, only analyze the first sector
	...
	i_sector_now = get_sector_offset( );                                                            //get the starting sector offset
	...
	while ( ( gc_prog_run == AK_TRUE ) && ( i_sector_now < g_card_result.i_total_sector ) ) {
		if ( ( gc_sector_align == AK_TRUE ) && ( gc_full_test != AK_TRUE ) ) {                      //perform 1MB sector alignment
			if ( ( i_sector_modify = get_sector_align( i_sector_now ) ) >= g_card_result.i_total_sector ) {//get the aligned sector number
				break;
			}
		...
		else {                                                                                      //no sector alignment
			i_sector_modify = i_sector_now;
		}

		if ( i_sector_modify + gi_test_sector < g_card_result.i_total_sector ) {                    //get the number of sectors available for testing
		...
		if ( test_sector( i_sector_modify, i_sector_num) == AK_FAILED ) {                           //***** entry function for TF card testing *****
		...
		i_us_total = timeval_count( &timeval_start, timeval_mark( &timeval_last ) ) ;               //total elapsed time so far

		if ( ( gi_test_timeout > 0 ) && ( i_us_total >= gi_test_timeout * SEC2USEC ) ) {            //exit if detection time limit exceeded
		...
		if ( ( i_us = timeval_count( &timeval_begin, timeval_mark( &timeval_end ) ) ) <= 0 ) {       //elapsed time for current test
			timeval_begin = timeval_end;
			continue ;
		}

		if( gc_full_test == AK_TRUE ) {                                                             //full card scan
			if ( ( i_sector_now + i_sector_num ) % COUNT_SECTOR == 0 ) {                            //print info only after scanning COUNT_SECTOR sectors
				if ( gc_view_detail == AK_TRUE ) {
		...
		else if ( gc_per_mb == AK_TRUE ) {                                                          //scan by sampling points based on capacity
		...
		else {                                                                                      //scan using configured number of sample points (default mode)
			if ( i_us >= gi_test_point_timeout * MSEC2USEC ) {
				if ( gc_view_detail == AK_TRUE ) {
					DEBUG_VAL( COLOR_MODE_NORMAL, COLOR_BACK_BLACK, COLOR_FRONT_RED,
					           "[%llu] %s (%llums)\n",
...
unsigned long long get_sector_offset( )                                                             //set the starting sector number at the beginning of the test
{
	double f_rand;
	int i_sector_range ;
	...
	if( gc_full_test == AK_TRUE ) {                                                                 //set starting sector based on offset value
		return 0 ;                                                                                  //full card scan starts from block 0
	}
	else if ( gc_per_mb == AK_TRUE ) {                                                              //scan based on capacity
		if ( gi_test_offset < 0 ) {                                                                 //randomly set a sector offset value
...
int test_sector( off64_t i_sector, off64_t i_num )                                                  //***** entry function for TF card testing *****
{
	int i;
	...
	if ( ( gc_write == AK_TRUE ) &&
		 ( gc_write_check == AK_TRUE ) ) {                                                          //write random data to verify that card is truly writable
	...
	if ( gc_erase == AK_TRUE ) {
		...
		if ( erase_disk( gi_fd_dev, i_sector * SEC_SIZE, i_num * SEC_SIZE) == AK_FAILED ) {         //erase
			g_card_result.ai_card_error[ CARD_TEST_TYPE_ERASE ] ++;
			c_flag = AK_FAILED;
		}
		else if ( gc_erase_check == AK_TRUE ) {                                                     //erase verification: check whether to perform post-erase validation
			if( read_disk( gi_fd_dev, i_sector * SEC_SIZE, pc_erase_align, SEC_SIZE ) <= 0 ) {      //read only one sector
				g_card_result.ai_card_error[ CARD_TEST_TYPE_ERASE ] ++;
				c_flag = AK_FAILED;
			}
			else {
	...
	if ( gc_write == AK_TRUE ) {
		...
		if ( write_disk(gi_fd_dev, i_sector * SEC_SIZE, pc_read_align, i_num * SEC_SIZE) <= 0 ) {   //write back data
			g_card_result.ai_card_error[ CARD_TEST_TYPE_REWRITE ] ++;
			return AK_FAILED;
		}
		else if ( gc_write_check == AK_TRUE ){                                                      //write-back verification
...
int ak_format_tf( )                                                                                 //format the TF card
{
	...
	*/
	...
	if ( ( read_disk( gi_fd_dev, 0, pc_bak_align , i_size ) != i_size ) ||
	     ...
	     ( memcmp( pc_write_align , pc_read_align , i_size ) != 0 ) ) {                             //write random data to verify write succeeded
		DEBUG_VAL( COLOR_MODE_NORMAL, COLOR_BACK_BLACK, COLOR_FRONT_RED,
...
#if 0
	if (is_dev_file(DEV_MMCBLK0P1) != AK_TRUE) {                                                    //check if repartitioning is needed
		...
		if ( ( write_disk( gi_fd_dev, 0, pc_write_align, i_size ) != i_size ) ||
		     ( read_disk( gi_fd_dev, 0, pc_read_align, i_size ) != i_size ) ||
		     ( memcmp( pc_write_align , pc_read_align , i_size ) != 0 ) ) {                         //write all-zero data to verify write succeeded (clear partition table)
			DEBUG_VAL( COLOR_MODE_NORMAL, COLOR_BACK_BLACK, COLOR_FRONT_RED,
		...
		DEBUG_HINT( COLOR_MODE_NORMAL, COLOR_BACK_BLACK, COLOR_FRONT_BLUE )
		get_shell_res( CMD_FDISK , ac_fdisk , LEN_SHELL_RES );                                      //repartition: create a single partition only
		...
		*/
		if ( ( read_disk( gi_fd_dev, 0, pc_read_align, i_size ) != i_size ) ||
		     ( memcmp( pc_write_align , pc_read_align, i_size ) == 0 ) ) {                          //compare against all-zero buffer after writing partition table; !=0 means write succeeded
			c_mkfs = AK_FALSE;
			DEBUG_VAL( COLOR_MODE_NORMAL, COLOR_BACK_BLACK, COLOR_FRONT_RED,
	...
	if ( ( write_disk( gi_fd_dev, 0, pc_bak_align, i_size ) == i_size ) &&
		     ( read_disk( gi_fd_dev, 0, pc_read_align, i_size ) == i_size ) &&
		     ( memcmp( pc_bak_align , pc_read_align , i_size ) == 0 ) ) {                           //write back backup partition table and verify write succeeded
		c_mkfs = AK_TRUE;
	}

	if ( c_mkfs == AK_TRUE ) {                                                                      //close mmcblk0 file descriptor and create filesystem
		close( gi_fd_dev );
		gi_fd_dev = 0;
