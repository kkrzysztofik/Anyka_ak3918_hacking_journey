...
void daemon_set_timer(int sec)
{
	...
	s_time.it_interval.tv_sec  = 0; // Whether multiple uses are needed?
...
/**
 * daemon_umount_sd - Unmount SD card
 */
...
/**
 * daemon_init_hotplug_socka - Hot plug detection
 * return int: -1, failure; else success
 */
...
#define PRESS_STANDBY_TIME	2
    ...
    switch (event->value) {
    case KEY_PRESS_UP:      // Release to stop timing, get button press duration
		period = daemon_diff_timeval(&event->time, &start_time);
		ak_print_normal("period=%ld(ms)\n", period);
		if (period >= PRESS_STANDBY_TIME) {
#if 0
			/* Set RTC alarm wakeup time, unit: seconds */
        ...
        break;
    case KEY_PRESS_DOWN:    // Press to start timing
        start_time.tv_sec = event->time.tv_sec;
		start_time.tv_usec = event->time.tv_usec;
        break;
    default:
...
/*
 * get_machine_net - Obtain the IP, subnet mask and gateway of the network card currently used by the machine
...
/**
 * daemon_get_local_info - After receiving specific broadcast packet information, obtain some information on the system through this function.
 *@param[in]   char * send_buf, pointer to the buffer storing system information that needs to be sent by the system
 *@return        void
 */
...
int main (int argc, char **argv)
{
	...
	/* BROADCAST thread */
	/* Receive PC side broadcast, return information of the current machine */
	ak_thread_create(&broadcast_sk_id, broadcast_pth,
		NULL, ANYKA_THREAD_MIN_STACK_SIZE, -1);

	/*** Thread for detecting hot plug status and taking corresponding actions ***/
	ak_thread_create(&pth, daemon_pth_func, NULL, ANYKA_THREAD_MIN_STACK_SIZE, -1);

    daemon_handle_key();

	/** Stop hot plug detection thread **/
