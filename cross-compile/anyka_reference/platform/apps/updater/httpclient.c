...
/********************************************
Function: Search for the first matching character from the right side of the string
********************************************/
...
/********************************************
Function: Convert string to all lowercase
********************************************/
...
/**************************************************************
Function: Analyze website address and port from string src, and get the file to be downloaded
***************************************************************/
...
int HttpDownload(char* url, char* filename)
{
	...
	ToLowerCase(url);/* Convert parameters to all lowercase */
	...
	GetHost(url, host_addr, HOST_ADDR_LEN, host_file, HOST_FILE_LEN, 
			&portnumber);/* Parse URL, port, filename, etc. */
	printf("webhost:%s\n", host_addr);
	printf("hostfile:%s\n", host_file);
	printf("portnumber:%d\n\n", portnumber);
	...
	printf("%s", request);/* Prepare request to be sent to host */
	...
	i=0;
	/* Connection successful, receive HTTP response */
	while((nbytes=read(sockfd,buffer,1))==1)
	{
		if(i < 4)
		{
			if(buffer[0] == '\r' || buffer[0] == '\n')
				...
				i = 0;
			printf("%c", buffer[0]);/* Print HTTP header info on screen */
		}
		else
		{
			fwrite(buffer, 1, 1, fp);/* Write HTTP body info to file */
			i++;
			if(i%1024 == 0)
			{
				printf("download %d bytes\n", i);
				fflush(fp);/* Save to disk once every 1K */
			}
		}
	}
	fclose(fp);
