...
static int fha_fd = 0;/* fha_char interface file descriptor */
static char *file;
static int erased;
...
struct erase_info_thread {/* erase thread input parameters */
	int fd;
...
int fha_interface_UpdateBin(const char* filename, const char* BinFileName)
{
	...
	while((nLen = read(fd, bufInfo.buff, MAX_BUF_LEN)) > 0)
	{
		...
		bufInfo.buflen = nLen;
		//printf("bufInfo.buflen=%ld,, nLen=%ld\n", bufInfo.buflen, nLen);
		// Call upgrade bin file interface
...
int fha_interface_UpdateMTD(const char* filename, int partition)
{
	...
	// Check if it is nandflash, this method does not support nandflash yet
#if 0
	if (info.type == MTD_NANDFLASH) {
	...
	// Loop to read from file, then write to mtd
	for (i = 0; i < cycle; i++) {
		putchar('.');
...
int fha_interface_UpdateMac(const char* mac)
{
	...
	// Call upgrade mac address interface
...
int fha_interface_UpdateSn(const char* sn)
{
	...
	// Call upgrade serial number interface
...
int fha_interface_UpdateBsn(const char* bsn)
{
	...
	// Call upgrade barcode interface
