...
enum test_info
{
	CASE_SPEAKER = 0,   // Speaker
	CASE_CLOUD_DECK,   // Pan-tilt
	CASE_WIFI_CONNECT,   // WiFi connection
	CASE_SD_CARD,    // SD card
	CASE_MONITOR,   // Monitor/Listening
	//CASE_INTERPHONE,  // Intercom (not required)
	CASE_FOCUS_EFFECT,   // Focus and effects
	CASE_IRCUT,     // IRCUT and IR light
	CASE_RESET,    // Reset
	CASE_MAC_BURN,  // MAC burning
	CASE_UID_BURN,  // UID burning
	CASE_ATTR_TYPE_NUM
};
...
struct test_data
{
	char *head_name ;  // Packet header identifier "VCMD"
	enum test_info  test_type;
	enum cmd_type  cmd_type;
	uint   datalen;// Data length
	char*   data;    // Data
	uint  check_sum;
	char   *end_name;  // Packet tail identifier "VEND"
};
 ...
 * *  @brief		get_machine_ip
 				Get the IP, subnet mask and gateway of the network card currently used by the machine
 * *  @author
 * *  @date       	 2014-10-28
 * *  @param[in]   char *send_buf, pointer storing information such as ip
 * *  @return        void
 * */
...
/**
 * get_net_dns - Get system DNS information
 * @param[in]   char *dns_buf, pointer storing dns information
 * return        NULL
 */
...
/**
 * test_get_local_info - After receiving specific broadcast packet information, obtain some information on the system through this function.
 *@param[in]   char * send_buf, buf pointer storing system information that the system needs to send
 *@return        void
 */
...
/**
 * broadcast_pth Broadcast packet receiving thread, processes information and returns it to the host computer
 * @return        NULL
 */
