...
/**
 * Workaround for missing preprocessor macros in some older WIN32 compiler versions.
 */
#if defined(_WIN32)
...
/**
 * C header interface declaration for link-file compatibility in mixed C/C++ programming.\n
 * Effective when using a C compiler; not effective when using a C++ compiler.
 */
...
/**
 * Check whether a handle is valid; used to evaluate the result of handle-creating interface functions.
 */
#define  AK_VALID_HANDLE(__h)         ((__h) > AK_INVAL_HANDLE)


/**
 * Compute a 32-bit composite code; generally used to generate hash values or magic numbers.
 */
#define   AK_MK4CC(__byte0, __byte1, __byte2, __byte3)  \
	((__byte0) << 24 | (__byte1)<<16 | (__byte2)<<8 | (__byte3))

/**
 * Compute a 16-bit composite code; generally used to generate hash values or magic numbers.
 */
