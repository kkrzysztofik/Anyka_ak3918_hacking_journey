...
int check_record_ext_name( char *file_name, int i_len )                         //check whether the recording file extension meets requirements
{
 ...
 */
static unsigned long search_record_file(struct search_file_param *search, search_record_callback callback)    //function for indexing all recording files globally
{
    ...
    */
    ...
    for( i = 0 ; i < i_num_scandir ; i ++ ) {
        ...
        switch( pp_dirent_list[ i ]->d_type ){
            case DT_DIR:                                                        //directories that do not match the naming rules
                ...
                //DEBUG_PRINT( COLOR_MODE_NORMAL , COLOR_BACK_BLACK , COLOR_FRONT_BLUE, "pp_dirent_list[ i ]->d_name= %s", pp_dirent_list[ i ]->d_name )
                break;
            case DT_REG:                                                        //files that do not match the naming rules
        ...
        */
        ...
        //DEBUG_PRINT( COLOR_MODE_NORMAL , COLOR_BACK_BLACK , COLOR_FRONT_BLUE, "file_path= '%s'", file_path )
        callback(file_path, cur_size, file_type);                               //callback to add to the list
        total_size += cur_size;
...
static inline int search_dir_record_file(const char *path, struct search_file_param *search, struct search_replay_param *replay, search_replay_callback callback)
{
    ...
    unsigned long cur_size = 0;
    int i_len_path ;                                                            //length of the current search directory

    i_len_path = snprintf( file_path , LEN_PATH_FILE , "%s" , path );           //get the string length of the current search directory
    while ((dir_ent = readdir(dirp)) != NULL) {
        if( check_record_ext_name( dir_ent->d_name, strlen( dir_ent->d_name ) ) != replay->type ) { //check whether the current filename has an avi or mp4 extension
            //DEBUG_HINT( COLOR_MODE_NORMAL , COLOR_BACK_BLACK , COLOR_FRONT_RED )
            continue;
        }

        memcpy( file_path + i_len_path , dir_ent->d_name , strlen( dir_ent->d_name ) + 1 );         //copy and concatenate the full path of the current file
...
static inline int search_dir_record_file(const char *path, struct search_file_param *search, struct search_replay_param *replay, search_replay_callback callback)
{
    ...
    unsigned long cur_size = 0;
    int i_len_path ;                                                            //length of the current search directory
    int i_num_scandir;                                                          //number of entries after scanning the directory
    int i;
    ...
    for( i = 0; i < i_num_scandir; i ++ ) {
        ...
        if( check_record_ext_name( pp_dirent_list[ i ]->d_name, strlen( pp_dirent_list[ i ]->d_name ) ) != replay->type ) { //check whether the current filename has an avi or mp4 extension
            FREE_POINT( pp_dirent_list[ i ] );
            continue;
        }
        memcpy( file_path + i_len_path , pp_dirent_list[ i ]->d_name , strlen( pp_dirent_list[ i ]->d_name ) + 1 );         //copy and concatenate the full path of the current file
        if ( get_record_file_size(file_path) <= 0) {                            //get file size
            FREE_POINT( pp_dirent_list[ i ] );
            continue;
        }
        calendar_time = record_fs_get_time_by_name(file_path);
        ...
        if(((replay->start_time <= calendar_time) && (replay->end_time > calendar_time) ) ||
             ...
             ((replay->start_time >= calendar_time) && (replay->end_time <= calendar_end_time))) {
             */
        //compare the two timestamps in the video file against the timestamps in the video request list to determine whether the time requirement is met
        if ( check_time_axis( replay->start_time , replay->end_time , calendar_time , calendar_end_time ) == AK_TRUE ) {
            ...
            DEBUG_PRINT( COLOR_MODE_NORMAL , COLOR_BACK_BLACK , COLOR_FRONT_GREEN,
                         ...
                         file_path , calendar_time , calendar_end_time , replay->start_time , replay->end_time )
                         */
            if (callback(file_path, cur_size, calendar_time, replay)) {         //callback function, add to the list
                ak_print_error_ex("add %s failed\n", file_path);
            }
            else {
                file_count ++;
                            ...
                            search_replay_callback callback)
{
    ...
    time_t start_time = replay->start_time - replay->start_time % ONE_HOUR_SECOND - ONE_DAY_SECOND;//expand start search range by 1 hour
    time_t end_time = replay->end_time - replay->end_time % ONE_HOUR_SECOND + ONE_HOUR_SECOND ;     //expand end search range by 1 hour
