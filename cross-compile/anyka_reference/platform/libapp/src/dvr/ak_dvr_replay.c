...
static int check_entry_time(struct dvr_replay_entry *entry)
{
    ...
    if ((record_start <= request_start) && (record_end > request_start)) {      //recording start time is earlier than the request time
        entry->play_start_pos = request_start - record_start;                   //set playback start offset based on request time and recording start time
...
replay_data_end:
    ...
    if ( record_replay.status != REPLAY_STATUS_STOP ) {                                   //if replay running status is not REPLAY_STATUS_STOP, execute the end-of-file callback
        //record_replay.send_data(record_replay.request.user_data, DEMUX_DATA_UNKNOW, NULL, 0);       //callback for end of file playback
    }
    return ret;
}
...
static int file_to_ret_list(struct list_head *entry_head,
                            struct list_head *ret_list)                         //convert dvr_file_entry to dvr_replay_entry
{
    struct dvr_file_entry *entry = NULL;
    struct dvr_file_entry *ptr = NULL;
    ...
    for( ;; ) {                                                                 //loop for sorted output
        i_unixts_mark = 0 ;
        p_entry_mark = NULL;

        list_for_each_entry_safe(entry, ptr, entry_head, list) {                //traverse the file list linked list
            //DEBUG_PRINT( COLOR_MODE_NORMAL , COLOR_BACK_BLACK , COLOR_FRONT_BLUE, "entry->path= '%s'\n", entry->path )
            if (entry->total_time <= 0) {
                list_del_init(&(entry->list));
                free_record_file_entry(entry);
            }
            if ( ( i_unixts_mark == 0 ) || ( entry->calendar_time < i_unixts_mark ) ) {     //find the file with the smallest timestamp
                i_unixts_mark = entry->calendar_time;
                p_entry_mark = entry;
            }
        }
        if( p_entry_mark == NULL ){                                             //no more elements in the linked list
            break;
        }
        ret_entry = get_entry_info( p_entry_mark );
        if ( ( ret_entry = get_entry_info( p_entry_mark ) ) != NULL ) {         //convert to dvr_replay_entry structure
            list_add_tail(&(ret_entry->list), ret_list);
            //DEBUG_PRINT( COLOR_MODE_NORMAL , COLOR_BACK_BLACK , COLOR_FRONT_BLUE, "ret_entry->path= '%s'\n", ret_entry->path )
            replay_count ++;
        }
        list_del_init( &( p_entry_mark->list ) );                               //remove current structure from linked list
        free_record_file_entry( p_entry_mark );                                 //release structure memory
    }
