...
#define RESERVED_PERCENT               (10)        //SD card reserved free space ratio
...
#define DVR_DAY                     35          //Parameters for DVR POLL INIT - extended day index length to account for record file list usage during list creation
#define DVR_HOUR                    24          //24 hours
#define DVR_MIN                     60          //60 minutes
#define DVR_INTERVAL                1           //recording duration
...
#ifdef USE_PTR_PATH                                                             //use dynamic allocation for recording file record node
/**
...
#else                                                                           //pre-allocated recording node approach
...
static struct dvr_file_entry* malloc_record_file_entry(int file_path_len)       //allocate recording file descriptor structure using static allocation
{
                            ...
                            struct list_head *dir_head)
{
    ...
    list_for_each_entry_reverse(ptr, dir_head, list){
        if (ptr) {
            if(dir_ent->calendar_time > ptr->calendar_time){
                ...
                list_add_tail(&(dir_ent->list), &(ptr->list));                  //add to tail
    ...
    ak_thread_mutex_lock(&(record_file.entry_mutex));
    //DEBUG_PRINT( COLOR_MODE_NORMAL , COLOR_BACK_BLACK , COLOR_FRONT_BLUE, "dir_ent->dir_path= '%s'\n", dir_ent->dir_path )
    list_add_tail(&(dir_ent->list), dir_head);                                  //add to tail
    ak_thread_mutex_unlock(&(record_file.entry_mutex));
 ...
 */
static void remove_earliest_file(struct record_file *file, unsigned long occupy_size)     //delete the earliest file
{
    ...
    ak_thread_mutex_lock(&(file->entry_mutex));
    for( ;; ) {
        list_for_each_entry( p_record_dir_loop, &(record_file.head[ file->param.type ] ), list ) { //traverse to find the directory with the smallest timestamp
            if ( ( i_min_day == 0 ) || ( i_min_day > p_record_dir_loop->calendar_time ) ) {
                ...
                */
                ...
                p_record_dir_remove = p_record_dir_loop;                        //save the found directory pointer
            }
        }

        if ( ( i_min_day != 0 ) && ( p_record_dir_remove != NULL ) ){
            list_for_each_entry( p_dvr_file_loop, &(p_record_dir_remove->file_head), list) {        //traverse into directory pointer to find the file with the smallest timestamp
                if ( ( i_min_file == 0 ) || ( i_min_file > p_dvr_file_loop->calendar_time ) ){      //found the file with the smallest timestamp
                    i_min_file = p_dvr_file_loop->calendar_time;                //mark the file with the smallest timestamp
                    p_dvr_file_remove = p_dvr_file_loop;                        //save file pointer
                }
            }
        }
        if ( ( p_record_dir_remove != NULL ) && ( p_dvr_file_remove != NULL ) ) {         //check whether directory and file were found
            /*
            DEBUG_PRINT( COLOR_MODE_NORMAL , COLOR_BACK_BLACK , COLOR_FRONT_BLUE ,
            ...
            */
            remove( p_dvr_file_remove->path ) ;                                 //delete the file
            ak_thread_mutex_lock( &( file->size_mutex ) ) ;
            file->used_size -= p_dvr_file_remove->size ;                        //decrease global file capacity
            file->free_size += p_dvr_file_remove->size ;                        //increase global free capacity
            ak_thread_mutex_unlock( &(file->size_mutex ) ) ;
            list_del_init( &( p_dvr_file_remove->list ) ) ;                     //remove file from list
            free_record_file_entry( p_dvr_file_remove ) ;                       //release pointer
            delete_empty_dir( p_record_dir_remove ) ;                           //check if directory is empty, delete it if so
            ...
            i_min_day = 0;
            i_min_file = 0;
            if (file->free_size > occupy_size) {                                //check whether space requirement is satisfied
...
static void get_sdcard_record(struct record_file *file)
{
        ...
        if (file->free_size < reserved_size) {
            ...
            if ((file->free_size + file->used_size) > reserved_size) {
                ...
                for(i=0; i<5; i++){
                    ...
                    if (file->free_size > reserved_size) {                          //after deletion, check again whether capacity is sufficient
                        break;
                    }
                }
                if(5 == i){
    ...
    if (size_enough){
        ...
        file->free_size -= reserved_size;                                       //free interval minus reserved space; free_size == 0 means disk is considered full
        ak_thread_mutex_unlock(&(file->size_mutex));
    } else {
...
static void stat_record_file(struct record_file *file)
{
    ...
    unsigned long occupy_size = (statbuf.st_blocks >> 1);                       //get current recording file size in KB: number of required sectors * 2 converts to KB
    ...
    if(statbuf.st_size < (100 * 1024)){
        ...
        if ( ( file->free_size == 0 ) &&
             ( file->used_size == 0 ) &&
             ( reserved_size == MIN_DISK_SIZE_FOR_RECORD ) ) {                  //in dvr_record_demo files can be saved; since free_size is not initialized there (value is 0), assign free_size here
        ...
        */
        if(occupy_size > file->free_size){                          //current recording file size after saving exceeds the SD card remaining capacity threshold
            if(file->param.cyc_flag){                               //cyclic recording mode is enabled according to the configuration file parameter
                for(i=0; i<5; i++){                                 //if deleting earlier recording files is still not enough, keep deleting; maximum 3 deletion cycles
                    remove_earliest_file(file, occupy_size);        //delete an earlier recording file
                    if(occupy_size < file->free_size){              //space is now available after deletion
                        break;
                    }
                }
                if(5 == i){
...
//after boot and SD card mount, check for video cache under /mnt/tmp that is less than 5 minutes; if found, generate it as AVI or MP4 and save to /mnt/CYC_DV
static void check_poweroff_file(void)
{
    int index = 0;
    static struct record_file record_file_buf = {0};                //record_file structure cache
    memcpy(&record_file_buf, &record_file, sizeof(record_file));    //save current record_file structure (data)
    ...
    if (!ak_check_file_exist(record_file.param.rec_info_file)) {
        ...
        if (record_file.name_tmp) {
            ...
            //the above operations can cause device replay exceptions; restore the original record_file structure (data) after the device successfully generates the video
...
/*
Used to check whether two time axes overlap.
*/
                                ...
                                struct list_head *entry_list)
{
    ...
    start_time = start_time - start_time % ONE_HOUR_SECOND - ONE_DAY_SECOND;//expand start search range by 24 hours
    end_time = end_time - end_time % ONE_HOUR_SECOND + ONE_HOUR_SECOND ;    //expand end search range by 1 hour

    list_for_each_entry( p_record_dir, &(record_file.head[ type ] ), list ) {   //traverse directories
        ...
        DEBUG_PRINT( COLOR_MODE_NORMAL , COLOR_BACK_BLACK , COLOR_FRONT_BLUE ,
                     ...
                     */
        if( check_time_axis( i_dir_start , i_dir_end , start_time, end_time ) == AK_TRUE ) {       //check directory: start time + 1 hour, end time - 1 hour
            /*
            DEBUG_PRINT( COLOR_MODE_NORMAL , COLOR_BACK_BLACK , COLOR_FRONT_GREEN ,
                         "start_time= %lu end_time= %lu p_record_dir->dir_path= '%s' p_record_dir->calendar_time= %lu last= %lu\n",
                         start_time, end_time , p_record_dir->dir_path , p_record_dir->calendar_time , p_record_dir->calendar_time + ONE_DAY_SECOND );
                         */
            list_for_each_entry(p_dvr_file, &(p_record_dir->file_head), list) {           //traverse files
                ...
                DEBUG_PRINT( COLOR_MODE_NORMAL , COLOR_BACK_BLACK , COLOR_FRONT_BLUE ,
                             ...
                             */
                if( check_time_axis( i_file_start , i_file_end , start_time , end_time ) == AK_TRUE ) {       //check file time range
                    /*
                    DEBUG_PRINT( COLOR_MODE_NORMAL , COLOR_BACK_BLACK , COLOR_FRONT_GREEN ,
                                 "p_dvr_file->path = '%s' i_file_start= %lu i_file_end= %lu start_time= %lu end_time= %lu\n",
                                 p_dvr_file->path, i_file_start, i_file_end, start_time, end_time);
...
#define SCAN_AUTO
...
#ifdef SCAN_AUTO
    if (DVR_FILE_STATUS_FETCH_OK == record_file.status) {                       //search in-memory index list
    ...
    else {                                                                      //search SD card files to get list
        set_record_sleep_ms( SEARCH_RECORD_MS_SLOW );                           //reduce the search sleep speed for building the recording list cache
        list_count = get_replay_from_dir(start_time, end_time, type, entry_list);
        set_record_sleep_ms( SEARCH_RECORD_MS_FAST );                           //restore the search sleep speed for building the recording list cache
...
int ak_record_exist( time_t start_time, time_t end_time, enum dvr_file_type type )
{
    ...
    while(start_time <= end_time) {
        ...
        while ( ( dir_ent = readdir( dirp ) ) != NULL) {
            ...
            //( check_record_file_name(dir_ent->d_name, record_file.search.prefix) != type) || //check file type
            if ( ( ak_is_regular_file(file_path) != AK_TRUE) || //check whether it is a regular file
                 ( check_record_ext_name(dir_ent->d_name, strlen( dir_ent->d_name ) ) != type) ||
                 (  get_record_file_size(file_path) <= 0 ) ) {                  //file length is less than or equal to 0
                continue;
            }
...
{
    /// Switched to a new handling approach.
}
