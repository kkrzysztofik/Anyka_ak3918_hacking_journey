...
struct tag_FolderInfo_chkdsk
{
    ...
    T_U32 size[2];
    T_U32 chkdsk_sum;
    T_U32 start_sector;  // first sector of fdt
...
#define FILE_FIND_DEEP_CNT       7    // Maximum number of search layers cannot exceed 6 = 7 - 1
#define FIND_FILENAME_PATHLEN    262  // maximum path length
...
struct tag_FileInfo_BeforePower
{
    ...
    T_U32 deep_cnt;          // current layer count
...
{
    T_PFILEINFO_BEFOREPOWER find_fileinfo; // records the first cluster number and fdt of each layer
 ...
 * RETURN:     return  pFilePosInfo  for success, otherwise return ak_null,   free pFilePosInfo  by File_GetfileclusterInfo_ToFree()
**************************************************************************/
// Get the logical addresses of files and folders in this interface
 ...
 * RETURN:     return (T_U32)file ,fasle return 0,
 Notes:
 This interface is for special purposes only, supports read-only and overlay modes.
 The file must exist; if not, an error occurs. File size cannot be 0.
 Supports writing only up to the original file size; exceeding it triggers an error.
 T_U32 FDTOffset should be the first position of fdt, obtainable via File_GetFileinfo_fastopen
 or File_get_findfile_info_WithID(T_U32 obj, T_PFILEINFO_BEFOREPOWER fileinfo).
 Cannot use File_FindInfo_WithID because it calls the old interface which records short filename fdt.
**************************************************************************/
