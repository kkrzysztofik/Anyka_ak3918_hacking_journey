...
{
    T_U32  driver;               //
    T_U32  first_parentID;       // root directory of the first layer
    T_U16 *fileName;             //
    ...
    T_U16 *pattern;              // Compare with Filter_Match, the buffer was provided by caller.
    T_U16 *fileter_pattern;      // Compare with Filter_Match to fileter, the buffer was provided by caller.
    T_U8   updatedir_flag;       // flag for searching up one layer
    T_U8   Dirorfolder_flag;     // flag for root directory or folder
    T_BOOL deep_flag;   // if forward search for the last one
    T_BOOL forward_find_flag;   // if forward search for the last one
    T_BOOL backward_find_flag;   // if backward search for the last one
