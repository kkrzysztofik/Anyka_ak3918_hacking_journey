...
#define CMD_MMC_FS_FIX      "mmc_test -F"                                                       //used to check and repair the TF card filesystem
#define LEN_CMD       100
#define LEN_RES       2048
 ...
 * notes:
 */
int ak_dvr_repair_ro_disk( )                                                    //re-repair the SD card
{
	char cmd[LEN_CMD] = {0};
	char *pc_dev = NULL ;
	char res[LEN_CMD];
	...
	if ( pc_dev != NULL ) {
		...
		sync();
		snprintf(cmd, LEN_CMD, "mount %s %s", pc_dev , PATH_MOUNT_POINT);       //re-mount the filesystem
		ak_cmd_exec(cmd, res, LEN_CMD);
		return AK_SUCCESS;
	}
	else {
