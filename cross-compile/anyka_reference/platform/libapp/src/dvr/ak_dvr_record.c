...
struct ak_dvr_record {
    ...
    unsigned long seq_num_end;      //frame sequence number at which the video ends
    time_t unixts_end;              //unix timestamp at which the video ends
    ...
    */
    ...
    if ( ret == AK_SUCCESS ) {
        ...
        if(ventry->stream.len >= IFRAME_MAX_BYTES) {
            ...
            if (RECORD_STATUS_SLEEP == record->status) {
            ...
            else if ( ( ret = add_video_mux_list(record, ventry) ) == AK_SUCCESS ) {       //check whether adding the video frame to the linked list succeeded
                if ( record->trigger_type == RECORD_TRIGGER_TYPE_PLAN ) {       //scheduled recording mode
                    /*
                    ...
                    */
                    if ( ( ventry->stream.frame_type == FRAME_TYPE_I ) &&
                         ( ventry->stream.ts >= record->end_time ) ) {          //check whether the current frame is an I-frame
                        /*
                        DEBUG_PRINT( COLOR_MODE_NORMAL , COLOR_BACK_BLACK , COLOR_FRONT_GREEN ,
                                     ...
                                     */
                        record->seq_num_end = ventry->stream.seq_no;        //record the sequence number of the ending video frame

                        gettimeofday( &timeval_now, NULL );                 //get unix timestamp
                        record->unixts_end = timeval_now.tv_sec;            //record the start timestamp (in seconds) for the next video
                ...
                /*
                else if ( record->trigger_type == RECORD_TRIGGER_TYPE_ALARM ) { //triggered recording mode
                }
                */
            }
        }
                ...
                const struct video_stream *stream)
{
    ...
    if(NULL != record->save_mux_handle){
        ...
        if (ret) {
            ...
            ak_seconds_to_date(record->unixts_end,&(record->date));             //create the time identifier for the next file
            record->seq_num_end = 0 ;                                           //reset seq_num_end and unixts_end
            record->unixts_end = 0;
...
static void sync_video_data(struct ak_dvr_record *record)
{
    ...
    /* video data stream */
    list_for_each_entry_safe(ventry, vptr, &(record->video.mux_head), list) {   //traverse the video frames in the current buffer via loop
		ull_ts_end = ventry->stream.ts;                                         //get the current frame
        if( c_get_iframe == AK_TRUE ) {                                         //I-frame already obtained; traverse to the tail of the list to get the last frame timestamp
            continue;
        }
        if(FRAME_TYPE_I == ventry->stream.frame_type){
    ...
    if( c_get_iframe == AK_TRUE ){                                              //adjust the video start time
    ...
    DEBUG_PRINT( COLOR_MODE_NORMAL , COLOR_BACK_BLACK , COLOR_FRONT_BLUE ,
                         ...
                         tm_res.tm_hour, tm_res.tm_min , tm_res.tm_sec )
                         */
    ak_seconds_to_date( timeval_now.tv_sec , &(record->date) );                 //set the video start time
}
