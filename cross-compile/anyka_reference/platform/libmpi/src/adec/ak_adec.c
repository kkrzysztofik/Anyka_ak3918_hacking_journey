...
struct audio_decode {
    ...
    unsigned char wait_flag;        ///< Flag indicating whether the bitstream is terminated via the wait mechanism; initialized to TRUE,
                                    ///< and changed when ak_stream_cancel_stream or ak_stream_wait_stream is called.
...
void* ak_adec_open(const struct audio_param *param)
{
	...
	decode->wait_flag = AK_TRUE; ///< Must be initialized to TRUE because the loop condition depends on this value.
...
int ak_adec_cancel_stream_no_wait (void *stream_handle)
{
	...
	/// The current exit mode does not wait.
	decode->wait_flag = AK_FALSE;

	/// After changing the flag, call the wait interface.
