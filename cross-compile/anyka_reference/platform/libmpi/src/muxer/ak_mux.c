...
struct mux_handle {
	...
	/// Flag indicating that a keyframe (IDR frame) has been received.
	int gotidr;
};
				...
				FILE *record_fp, FILE *index_fp, FILE *moov_fp, FILE *stbl_fp)
{
	...
	mux->moov_fp = moov_fp;
	mux->stbl_fp = stbl_fp;
	mux->gotidr = AK_FALSE; ///< Initialized to false: no keyframe received yet.
	...
	open_input.m_ulIndexMemSize = 2 * MUX_MAX_FPS * 	//calculated at max frame rate to avoid insufficient index memory if frame rate changes mid-recording
...
int ak_mux_add_video(void *mux_handle, const struct video_stream *video)
{
	...
	if (!mux->gotidr) {
		/// Waiting for the first keyframe.
		if (FRAME_TYPE_I == video->frame_type) {
			mux->gotidr = AK_TRUE;
		} else {
			/// Return an error and wait for the next keyframe.
