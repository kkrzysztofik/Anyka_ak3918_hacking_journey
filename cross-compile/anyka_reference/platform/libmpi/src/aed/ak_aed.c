...
static int aed_calc(void *pbuf, int len, int value)
{
    ...
    if (checkcnt < 2)  {
        /* Prevent instability at the start of recording; the first two frames are excluded from the DC offset calculation */
        checkcnt ++;
    } else if (checkcnt <= 20) {
    /* Due to hardware speed constraints, only frames 3 through 20 participate in the DC offset calculation */
        checkcnt ++;
        for (i = 0; i < len / 2; i++) {
            dcavr += pdata[i];
        }
