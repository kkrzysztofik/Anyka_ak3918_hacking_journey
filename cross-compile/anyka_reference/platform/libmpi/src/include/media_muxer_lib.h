...
typedef struct
{
    ...
    T_U32                   m_ulTimeScale;  //time scale, the reciprocal of this value is the unit of the incoming timestamp. e.g. m_ulTimeScale=1000 means the time unit is (1/1000)s, i.e. 1ms
    ...
    T_U16   m_nHeight;
    T_U16   m_nFPS;
    T_U16   m_nKeyframeInterval;            //keyframe interval; this parameter is no longer used in the muxer library
    T_U16   m_nAudioPacksPerChunk;          //for MP4 files, sets the number of audio frames per chunk; this determines the length of consecutively stored audio data

    //audio
    T_eMEDIALIB_AUDIO_CODE  m_eAudioType;
    T_U32   m_ulAudioBitrate;   //audio bitrate
    T_U32   m_ulSamplesPerPack; //number of sample points per audio packet
    T_U32   m_nSampleRate;      //sample rate (e.g. 8000)
    T_U16   m_nChannels;        //stereo (2) or mono (1)
    T_U16   m_wBitsPerSample;   //for ADPCM: bits per compressed sample (typically 4); otherwise fixed at 16 bits (16)
    //the following fields can be left unset for PCM and AMR
