...
{   
    ...
    T_U32                   m_ulAudioDecBufSize;    // Expected size of audio decoding buffer
    T_U32                   m_ulVideoDecBufSize;    // Expected size of video decoding buffer
    T_eMEDIALIB_ROTATE      m_Rotate;
    T_U8                    m_AudioAttribute;       //no use, only used in audio lib with dsp
...
typedef struct
{
    ...
    T_U16   m_nChannel;     // Stereo (2), Mono (1)
    T_U16   m_BitsPerSample; // 16-bit fixed (16)
    T_U32   m_nSampleRate;  // Sample rate (8000)
    T_U16   m_ulDuration;   // Duration of each audio packet
}T_AUDIO_RECORD_INFO;
...
{   
    T_eMEDIALIB_STATE       m_State;
    T_U32                   m_ulAudioEncBufSize;    // Expected size of audio encoding buffer
    T_U32                   m_ulVideoEncBufSize;    // Expected size of video encoding buffer
...
// Decryption callback function and parameter structure
