...
/**
 * Get the current motor position via ioctl.
 */
...
static int check_dg (struct ak_motor *motor, int degree)
{
    ...
    int cur_step = __get_motor_step (motor, set_step); ///< Get the current position.
    int step = set_step - cur_step;

    /// Clamp to maximum and minimum values.
    set_step = set_step >= 0 ? set_step : 0;
    set_step = set_step < motor->fulldst_step ? set_step : motor->fulldst_step - 1;

    /// Calculate step deviation.
...
int ak_drv_ptz_check_self(enum ptz_feedback_pin pin_type)
{
	...
	if(akmotor[0].fd > 0){
		...
		_calibrated = AK_TRUE; ///< Self-test complete flag.
		ret = AK_SUCCESS;
	}
