...
static void ai_join_path_suffix(char *dst, size_t dst_len, const char *base, const char *suffix)
{
	if (!dst || dst_len == 0) {
		return;
	}
	...
	if (base_len + suffix_len >= dst_len) {
		dst[0] = 0;
		return;
	}
...
static void ai_set_file_name(const char *origin_name)
{
	...
	strncpy(temp_path, origin_name, AI_PATH_LEN - 1);
	temp_path[AI_PATH_LEN - 1] = 0;
	...
	ai_join_path_suffix(
		ai_ipcsrv.file_info[AI_ORIGIN_LEVEL].save_pcm_path,
		sizeof(ai_ipcsrv.file_info[AI_ORIGIN_LEVEL].save_pcm_path),
		temp_path,
		"_origin.pcm");
	ai_join_path_suffix(
		ai_ipcsrv.file_info[AI_RESAMPLE_LEVEL].save_pcm_path,
		sizeof(ai_ipcsrv.file_info[AI_RESAMPLE_LEVEL].save_pcm_path),
		temp_path,
		"_resample.pcm");
	ai_join_path_suffix(
		ai_ipcsrv.file_info[AI_ASLC_LEVEL].save_pcm_path,
		sizeof(ai_ipcsrv.file_info[AI_ASLC_LEVEL].save_pcm_path),
		temp_path,
		"_aslc.pcm");
}
...
int cb_ai_set_status(char *status, unsigned int len, int sock)
{
	...
	if ((ipc.second > 0) && (strlen(ipc.file) > 0)) {
		...
		snprintf(res, RES_LEN, "start to save %d second pcm to file: %.900s\n",
				ipc_info->save_second,
