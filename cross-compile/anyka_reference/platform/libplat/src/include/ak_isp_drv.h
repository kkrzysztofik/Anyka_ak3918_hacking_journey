...
typedef struct ak_isp_blc {
    T_U16     black_level_enable;	// Enable flag
    T_U16     bl_r_a;				//[ 0,1023]
...
typedef struct ak_isp_blc_attr {
    T_U16       blc_mode;             // 0 linkage mode, 1 manual mode
    AK_ISP_BLC  m_blc;
...
typedef struct ak_isp_rgb_gamma_attr {
    ...
    T_U16   g_key[16];
    T_U16   b_key[16];
    T_U16   rgb_gamma_enable;  // If not enabled, it is a straight line
} AK_ISP_RGB_GAMMA_ATTR;

typedef struct ak_isp_y_gamma_attr {
	T_U16    ygamma[129];    //10bit
	T_U16    ygamma_key[16]; // Key points of the curve
	T_U16    ygamma_uv_adjust_enable;
	T_U16    ygamma_uv_adjust_level;
	T_U16    ygamma_cnoise_yth1;   // Ygamma chrominance noise suppression threshold
	T_U16    ygamma_cnoise_yth2;   // Ygamma chrominance noise suppression threshold
...
typedef struct ak_isp_s_nr1_attr {
    T_U16       nr1_mode;             // nr1 mode, automatic or linkage mode
    AK_ISP_NR1  manual_nr1;
    AK_ISP_NR1  linkage_nr1[9];       // Linkage parameters      
}AK_ISP_NR1_ATTR;
...
typedef struct ak_isp_nr2_attr {
    T_U16       nr2_mode;          // Manual or linkage mode
...
typedef struct ak_isp_gb_attr {
    T_U16      gb_mode;       // Mode selection, manual or linkage
    AK_ISP_GB  manual_gb;
...
typedef struct ak_isp_ccm_attr {
    T_U16       cc_mode;  		// Color correction matrix linkage or manual
    AK_ISP_CCM  manual_ccm; 
    AK_ISP_CCM  ccm[4]; 		// Four linkage matrices
}AK_ISP_CCM_ATTR;
...
typedef struct ak_isp_wdr {
	...
	T_U16	area_tb1[65];	  // Curve 10bit
	T_U16	area_tb2[65];	  // Curve 10bit
	T_U16	area_tb3[65];	  // Curve 10bit
	T_U16	area_tb4[65];	  // Curve 10bit
	T_U16	area_tb5[65];	  // Curve 10bit
	T_U16	area_tb6[65];	  // Curve 10bit
    ...
    T_U16   hdr_cnoise_suppress_yth1;   // Chrominance noise luminance threshold 1           
    T_U16   hdr_cnoise_suppress_yth2;   // Chrominance noise luminance threshold 2
    T_U16   hdr_cnoise_suppress_gain;   // Chrominance suppression
}AK_ISP_WDR;

typedef struct ak_isp_wdr_attr {
	T_U16 wdr_mode;              // Mode selection, manual or linkage
...
typedef struct  ak_isp_contrast_ATTR {
	T_U16	cc_mode;	// Mode selection, manual or linkage
...
typedef  struct ak_isp_fcs {
    ...
    T_U16  fcs_enable;   		// Enable flag
    T_U16  fcs_uv_nr_enable;  	// Enable flag
    T_U16  fcs_uv_nr_th;  		//[0, 1023]
}AK_ISP_FCS;

typedef  struct ak_isp_fcs_attr {
    T_U16       fcs_mode;   	// Mode selection, manual or linkage
...
typedef struct ak_isp_hue_attr {
	T_U16        hue_mode;		// linkage or manual
	AK_ISP_HUE   manual_hue; 
	AK_ISP_HUE   hue[4];		// Four linkage parameters
}AK_ISP_HUE_ATTR;
...
typedef struct ak_isp_effect_attr {
    ...
    T_S16  uv_a;    //[-256, 255]
    T_S16  uv_b;    //[-256, 255]
    T_U16  dark_margin_en;   // Dark margin enable
}AK_ISP_EFFECT_ATTR;

typedef struct ak_isp_ddpc { 
	T_U16 ddpc_enable;         // Dynamic DPC enable flag
	T_U16 ddpc_th;             // 10bit
	T_U16 white_dpc_enable;    // White point elimination enable flag
	T_U16 black_dpc_enable;    // Black point elimination enable flag
}AK_ISP_DDPC;
 
typedef struct ak_isp_ddpc_attr {
	T_U16 ddpc_mode;            // Mode selection, manual or linkage
	AK_ISP_DDPC manual_ddpc;
	AK_ISP_DDPC linkage_ddpc[9]; 
}AK_ISP_DDPC_ATTR;

typedef struct ak_isp_sdpc_attr { 
    T_U32   sdpc_enable; 		// Static DPC enable flag

    /* Static dead pixel coordinates, max 1024
    Data format {6h0 ,y_position[9:0],5'h0,x_position[10:0]} */
...
typedef struct ak_isp_af_stat_info {
   T_U32  af_statics[5];            // Statistical results  
}AK_ISP_AF_STAT_INFO;
...
typedef  struct ak_isp_awb_attr {   
    ...
    // Parameters that need to be set for AWB software part
    T_U16   auto_wb_step;                 // White balance step calculation
    T_U16   total_cnt_thresh;            // Pixel count threshold
    T_U16   colortemp_stable_cnt_thresh; // Stable frame count, environmental color temp is considered changed if this count is reached
...
typedef  struct ak_isp_awb_stat_info {
    ...
    T_U32   total_B[10];
    T_U32   total_cnt[10];
    // White balance gain values calculated by auto white balance algorithm
    T_U16   r_gain;
    T_U16   g_gain;
    T_U16   b_gain;
    T_S16   r_offset;
    T_S16   g_offset;
    T_S16   b_offset;
    T_U16   current_colortemp_index;     // Environmental color temperature marker
    T_U16   colortemp_stable_cnt[10];         // Stable frame count for each color temp
...
typedef  struct ak_isp_ae_run_info {
    T_U8   current_calc_avg_lumi;         		// Current calculated average luminance
    T_U8   current_calc_avg_compensation_lumi; 	// Luminance after exposure compensation

    T_U8   current_darked_flag;                 // Day/Night marker
    ...
    T_U32  current_d_gain_step;             	// Current digital gain step
    T_U32  current_isp_d_gain_step;        		// Current ISP digital gain step
    T_U32  current_exp_time_step;          		// Current exposure time step
}AK_ISP_AE_RUN_INFO;
    
typedef struct ak_isp_aec_algo {
    T_U32  aec_status;       		// Auto exposure control status, stable or unstable  
    T_U32  aec_locked;      		// Auto exposure locked or not
    
    T_U8  exp_time_need_updata;   	// Needs update, reduces I2C read/write         
    T_U8  a_gain_need_updata;    	// Needs update
    T_U8  d_gain_need_updata;       // Needs update
...
typedef struct ak_isp_mask_color_attr {
    T_U8  color_type;   // 0: Specified mask color; 1:
    T_U8  mk_alpha;
    T_U8  y_mk_color;
    T_U8  u_mk_color;
    T_U8  v_mk_color;
