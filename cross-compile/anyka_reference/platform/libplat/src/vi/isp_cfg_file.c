	   	...
	   	unsigned int sensor_id, unsigned char style_id, unsigned int *isp_offset)
{
	...
	do {
		...
		// Accept both version 3 and 4 configuration files for backward compatibility
		if (headinfo.main_version != ISP_VERSION_MAIN && headinfo.main_version != 3) {
			ak_print_error_ex(ISP_CFG "config file: %s has unsupported version, main_version:%d (expected 3 or 4)\n",
				path,headinfo.main_version);
			goto check_data_end;
		}

		// Log warning if using older version
		if (headinfo.main_version < ISP_VERSION_MAIN) {
			ak_print_warning_ex(ISP_CFG "config file: %s is using older version %d (current: %d)\n",
				path, headinfo.main_version, ISP_VERSION_MAIN);
		}
		...
		if (isp_module_check_cfg(buf + offset, &isp_size, headinfo.main_version) < 0) {
