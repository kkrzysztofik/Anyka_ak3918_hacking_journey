...
static int reload_config_awb_step(void)
{
	...
	awb.auto_wb_step = auto_awb_step;
	auto_awb_step = 0; // set to 0 to indicate reload finished
	return AK_ISP_set_awb_attr(&awb);
}
 ...
 * notes:
 */
int isp_module_check_cfg(char *cfgbuf, unsigned int *size, unsigned int version)
{
	unsigned short i = 0;
	...
	for (i = ISP_BB; i <= ISP_HUE; i++) {
		if (offset + ISP_MODULE_ID_SIZE + ISP_MODULE_LEN_SIZE > *size) {
			ak_print_error_ex("size err: offset %u exceeds size %u\n", offset, *size);
			return -1;
		}
		...
		if (moduleid != i) {
			ak_print_error_ex("data err! Expected module %d, got %d\n", i, moduleid);
			return -1;
		}

		/* For v3 files, be more flexible with module lengths since structures may differ */
		if (version == 3) {
			/* For v3, just verify length is reasonable (> 0 and not too large) */
			if (length == 0 || length > ISP_MODULE_MAX_SIZE) {
				ak_print_error_ex("v3 module %d: suspicious length %u\n", i, length);
				return -1;
			}
			offset += length;
			total += length;
		} else {
			/* For v4 and later, use strict structure size validation */
			if (length != Isp_Struct_len[i]) {
				ak_print_error_ex("v4 module %d: expected length %u, got %u\n", i, Isp_Struct_len[i], length);
				return -1;
			}
	...
	/* sensor id check */
	if (offset + ISP_MODULE_ID_SIZE > *size) {
		ak_print_error_ex("size err: cannot read sensor id, offset %u exceeds size %u\n", offset, *size);
		return -1;
	}
	...
	if (offset + ISP_MODULE_LEN_SIZE > *size) {
		ak_print_error_ex("size err: cannot read sensor length, offset %u exceeds size %u\n", offset, *size);
		return -1;
	}
...
/**
 * brief: convert v3 ISP module data to v4 format
 * @module_id[IN]: ISP module identifier
 * @v3_buf[IN]: source buffer with v3 module data (including header)
 * @v3_size[IN]: size of v3 module data
 * @v4_buf[OUT]: destination buffer for v4 module data
 * @v4_size[IN]: expected v4 module size from Isp_Struct_len[]
 * return: 0 success
 * notes: v4 structs extend v3 by appending fields at the end.
 *         New v4 fields are zero-filled (disabled/default).
 */
static int isp_convert_v3_to_v4(enum isp_module_id module_id,
	const char *v3_buf, unsigned int v3_size,
	char *v4_buf, unsigned int v4_size)
{
	/* Zero-fill v4 buffer so new fields get safe defaults */
	memset(v4_buf, 0, v4_size);

	/* Copy header (param_id + length) */
	memcpy(v4_buf, v3_buf, ISP_MODULE_HEAD_SIZE);

	/* Update length field to v4 size */
	unsigned short v4_len = (unsigned short)v4_size;
	memcpy(v4_buf + ISP_MODULE_ID_SIZE, &v4_len, ISP_MODULE_LEN_SIZE);

	/* Copy payload - min of v3 and v4 payload sizes */
	unsigned int v3_payload = v3_size - ISP_MODULE_HEAD_SIZE;
	unsigned int v4_payload = v4_size - ISP_MODULE_HEAD_SIZE;
	unsigned int copy_size = (v3_payload < v4_payload) ? v3_payload : v4_payload;

	memcpy(v4_buf + ISP_MODULE_HEAD_SIZE, v3_buf + ISP_MODULE_HEAD_SIZE, copy_size);
...
int isp_module_init(char *cfgbuf, unsigned int size)
{
	...
	/* check config - auto-detect v3 vs v4 format */
	unsigned int detected_version = 4;
	int ret = isp_module_check_cfg(cfgbuf, &len, 4);
	if (ret < 0) {
		/* v4 validation failed, try v3 format */
		ak_print_normal_ex("v4 validation failed, trying v3 format\n");
		len = size;  /* reset length */
		ret = isp_module_check_cfg(cfgbuf, &len, 3);
		if (ret < 0) {
			ak_print_error_ex("both v3 and v4 validation failed\n");
			return ret;
		}
		detected_version = 3;
		ak_print_notice_ex("successfully loaded config as v3 format\n");
	} else {
		detected_version = 4;
	}

	/* set attr - read actual module lengths from file for v3/v4 compatibility */
	for (i = ISP_BB; i <= ISP_HUE; i++) {
		unsigned short actual_module_id = 0;
		unsigned short actual_module_len = 0;

		/* Read module ID and length from file */
		memcpy(&actual_module_id, pbuf+offset, ISP_MODULE_ID_SIZE);
		memcpy(&actual_module_len, pbuf+offset + ISP_MODULE_ID_SIZE, ISP_MODULE_LEN_SIZE);

		/* Validate module ID matches expected module */
		if (actual_module_id != i) {
			ak_print_error_ex("Module ID mismatch at offset %u: expected %u, got %u\n", offset, i, actual_module_id);
			return -1;
		}
		...
		if (ISP_SHARP == i){
			...
			*/
			if (actual_module_len >= ISP_MODULE_HEAD_SIZE + sizeof(AK_ISP_SHARP_ATTR)) {
        ...
        if (ISP_WDR == i) {
			...
			*/
			if (actual_module_len >= ISP_MODULE_HEAD_SIZE + sizeof(AK_ISP_WDR_ATTR)) {
		...
		if (detected_version == 3 && actual_module_len != Isp_Struct_len[i]) {
			/* v3 module needs conversion to v4 size */
			char v4_buf[ISP_MODULE_MAX_SIZE];
			unsigned int v4_size = Isp_Struct_len[i];

			ak_print_notice_ex("converting module %d from v3 (%u bytes) to v4 (%u bytes)\n",
				i, actual_module_len, v4_size);

			isp_convert_v3_to_v4(i, pbuf+offset, actual_module_len, v4_buf, v4_size);
			isp_set_attr(i, v4_buf, v4_size);
		} else {
			isp_set_attr(i, pbuf+offset, actual_module_len);
		}
		offset += actual_module_len;
	}
...
void *isp_3D_NR_create(int width, int height)
{
	...
	/* Need another buffer for 3D noise reduction */
...
int isp_set_attr(enum isp_module_id module_id, char *buf, unsigned int size)
{
	...
	if ((module_id <= ISP_HUE) && (size != Isp_Struct_len[module_id])) {
		ak_print_error_ex("size err: module_id=%d, expected=%u, got=%u\n",
			module_id, Isp_Struct_len[module_id], size);
		return -1;
	}
...
int isp_switch_mode(char *cfgbuf, unsigned int size)
{
	...
	for (i = ISP_BB; i <= ISP_HUE; i++) {
		...
		/* Read module ID and length from file for v3/v4 compatibility */
		memcpy(&actual_module_id, cfgbuf+offset, ISP_MODULE_ID_SIZE);
		memcpy(&actual_module_len, cfgbuf+offset + ISP_MODULE_ID_SIZE, ISP_MODULE_LEN_SIZE);
		...
		*/
		if (ISP_MISC != i) {
			isp_set_attr(i, cfgbuf+offset, actual_module_len);
		}
		offset += actual_module_len;
	}
