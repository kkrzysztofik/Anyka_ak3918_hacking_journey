...
#define VI_FRAME_BACKPRESSURE_LOG_INTERVAL (50)
...
static int vi_set_capture_on(void *handle, enum isp_vi_status status)
{
	...
	if (pdev->capture_on) {
		ak_print_notice_ex(PLAT_VI "capture_on already enabled, skip\n");
		return 0;
	}
	 ...
	 * Some firmware/lib combos internally require allocation sized as if
	 * sub/ch3 paths are at least VGA even when sub runtime resolution is
	 * configured lower (e.g. 640x360). Under-allocation can stall the VI/VENC
	 * pipeline and trap ak_vi_get_frame in NO_DATA/backpressure.
	 */
	int alloc_sub_w = pdev->chn_sub->max_width;
	int alloc_sub_h = pdev->chn_sub->max_height;
	if (alloc_sub_w < CHIP_MAIN_CHN_MIN_WIDTH) {
		alloc_sub_w = CHIP_MAIN_CHN_MIN_WIDTH;
	}
	if (alloc_sub_h < CHIP_MAIN_CHN_MIN_HEIGHT) {
		alloc_sub_h = CHIP_MAIN_CHN_MIN_HEIGHT;
	}
	int alloc_sub_size = CH_SUB_SIZE(alloc_sub_w, alloc_sub_h);
	/*
	 * Keep ch3 allocation enabled and match compat sub allocation.
	 * This matches working vendor footprints observed in libre_anyka_app logs.
	 */
	int ch3_size = CH_CH3_SIZE(alloc_sub_w, alloc_sub_h);

	int yuv_total_len = main_size + alloc_sub_size + ch3_size
		+ MOVE_CNT_INFO_SZ + BUFFER_RESERVE_BYTES;

	ak_print_notice_ex(PLAT_VI
		"capture_on request: status=%d, main_max=%dx%d, sub_max=%dx%d, main_size=%d, sub_size=%d, alloc_sub=%dx%d(%d), ch3_size=%d, total=%d\n",
		status,
		pdev->chn_main->max_width, pdev->chn_main->max_height,
		pdev->chn_sub->max_width, pdev->chn_sub->max_height,
		main_size, sub_size, alloc_sub_w, alloc_sub_h, alloc_sub_size, ch3_size, yuv_total_len);
	...
	if (!ret) {
		pdev->capture_on = 1;
		ak_print_notice_ex(PLAT_VI "capture_on success: status=%d, total=%d\n",
			status, yuv_total_len);
	} else {
		ak_print_error_ex(PLAT_VI
			"capture_on failed: status=%d, ret=%d, total=%d, main_max=%dx%d, sub_max=%dx%d\n",
			status, ret, yuv_total_len,
			pdev->chn_main->max_width, pdev->chn_main->max_height,
			pdev->chn_sub->max_width, pdev->chn_sub->max_height);
	}
...
int ak_vi_get_frame(void *handle, struct video_input_frame *frame)
{
	...
	if ((!list_empty(&(pdev->frame_list))) &&
		...
		static unsigned int backpressure_cnt = 0;
		++backpressure_cnt;
		set_error_no(ERROR_TYPE_NO_DATA);
		if ((backpressure_cnt <= 3)
				|| ((backpressure_cnt % VI_FRAME_BACKPRESSURE_LOG_INTERVAL) == 0)) {
			int queued = 0;
			struct ak_frame *iter = NULL;
			list_for_each_entry(iter, &(pdev->frame_list), list) {
				++queued;
			}
			ak_print_warning_ex(PLAT_VI
				"frame queue backpressure: frame_count=%d, queued=%d, user_ref=%d, user_map=0x%x, limit=%d\n",
				pdev->frame_count, queued, pdev->user_ref, pdev->user_map,
				USER_SPACE_MAX_FRAMES);
		}
		goto get_frame_end;
	}
...
int ak_vi_release_frame(void *handle, struct video_input_frame *frame)
{
	...
	if (found) {
		...
		set_error_no(ERROR_TYPE_INVALID_ARG);
		static unsigned int release_miss_cnt = 0;
		++release_miss_cnt;
		if ((release_miss_cnt <= 3)
				|| ((release_miss_cnt % VI_FRAME_BACKPRESSURE_LOG_INTERVAL) == 0)) {
			ak_print_warning_ex(PLAT_VI
				"release frame miss: ts=%llu, user_flg=%d, frame_count=%d\n",
				frame->vi_frame[VIDEO_CHN_MAIN].ts, user->user_flg, pdev->frame_count);
		}
	}
