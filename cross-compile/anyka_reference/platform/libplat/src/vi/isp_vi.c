...
#define BUFF_NUM 				4
#define DEFAULT_DROP_FRAME_NUM 	4
...
#define ISP_DQBUF_LOG_INTERVAL	(50)
#define ISP_SELECT_LOG_INTERVAL	(20)
...
static unsigned int select_timeout_cnt = 0;
static unsigned int select_error_cnt = 0;
static unsigned int dqbuf_eagain_cnt = 0;
static unsigned int dqbuf_eio_cnt = 0;
static unsigned int dqbuf_other_cnt = 0;
static unsigned int dqbuf_userptr_miss_cnt = 0;
...
static int read_frame(int fd)
{
	...
	if (-1 == xioctl(fd, VIDIOC_DQBUF, &v4l2_buf)) {
		...
		case EAGAIN:
			++dqbuf_eagain_cnt;
			if ((dqbuf_eagain_cnt <= 3)
					|| ((dqbuf_eagain_cnt % ISP_DQBUF_LOG_INTERVAL) == 0)) {
				ak_print_warning_ex(
					"VIDIOC_DQBUF EAGAIN: count=%u, fd=%d, queued_frames=%u\n",
					dqbuf_eagain_cnt, fd, (unsigned int)BUFF_NUM);
			}
			return -1;
		...
		default:
			++dqbuf_other_cnt;
			ak_print_error_ex(
				"VIDIOC_DQBUF failed: errno=%d(%s), eagain=%u, eio=%u, other=%u\n",
				errno, strerror(errno), dqbuf_eagain_cnt, dqbuf_eio_cnt, dqbuf_other_cnt);
			return -1;
		}
	}
	last_read_errno = 0;
	...
	++dqbuf_userptr_miss_cnt;
	if ((dqbuf_userptr_miss_cnt <= 3)
			|| ((dqbuf_userptr_miss_cnt % ISP_DQBUF_LOG_INTERVAL) == 0)) {
		ak_print_warning_ex(
			"DQBUF userptr not in known buffers: count=%u, userptr=%p, index=%d, length=%u\n",
			dqbuf_userptr_miss_cnt, (void *)v4l2_buf.m.userptr, v4l2_buf.index, v4l2_buf.length);
	}
...
static struct v4l2_buffer* get_v4l2_ptr(void)
{
	...
	static unsigned int read_fail_streak = 0;
	...
	case -1:
		++select_error_cnt;
		if ((select_error_cnt <= 3)
				|| ((select_error_cnt % ISP_SELECT_LOG_INTERVAL) == 0)) {
			ak_print_error_ex("select error: errno=%d(%s), count=%u\n",
				errno, strerror(errno), select_error_cnt);
		}
		goto get_v4l2_end;
	case 0:
		/* when timeout,return failed to user */
		++select_timeout_cnt;
		if ((select_timeout_cnt <= 3)
				|| ((select_timeout_cnt % ISP_SELECT_LOG_INTERVAL) == 0)) {
			ak_print_warning_ex("select timeout: count=%u, fd=%d\n",
				select_timeout_cnt, video0_fd);
		}
	#if CALC_VI_CAP_TIME
	...
	default:
		if (read_frame(video0_fd)) {	//we should almost run to here
			...
			if ((read_fail_streak <= 3)
					|| ((read_fail_streak % ISP_DQBUF_LOG_INTERVAL) == 0)) {
				ak_print_warning_ex(
					"read frame failed: streak=%u, last_errno=%d(%s), select_timeout=%u, dqbuf_eagain=%u, dqbuf_eio=%u, dqbuf_other=%u\n",
					read_fail_streak,
					last_read_errno,
		...
		if (read_fail_streak > 0) {
			ak_print_notice_ex("read frame recovered after %u failures\n", read_fail_streak);
			read_fail_streak = 0;
		}
