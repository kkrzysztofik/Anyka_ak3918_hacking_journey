...
char* ak_seconds_to_string(time_t secs)
{
	...
	localtime_r(&secs, &value);
	{
		...
		int min = value.tm_min;
		int sec = value.tm_sec;
...
long ak_date_to_seconds(const struct ak_date *date)
{
    ...
    /// Since daylight saving time is ignored here, there may be an offset,
    /// so local time must be used for calibration again.
    localtime_r (&utc, &tmp);
    if (!tmp.tm_isdst) {
    	/// Local time does not use daylight saving time at this point, so it can be returned directly.
    	return utc;
    }
	...
	tmp.tm_min = date->minute;
	tmp.tm_sec = date->second;
	//tmp.tm_isdst = 1; ///< Keep the tm_isdst parameter unchanged, and generate the UTC time again.
