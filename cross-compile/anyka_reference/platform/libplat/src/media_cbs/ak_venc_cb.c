...
static unsigned int reg_access_err_cnt = 0;
...
void venc_cb_init_reg(unsigned long regstart, unsigned long size)
{
	...
	if (!reg_rw_ctrl.va_start) {
		ak_print_error_ex("venc_cb_init_reg map failed: regstart=0x%08x size=0x%08x errno=%d\n",
			(unsigned int)regstart, (unsigned int)size, errno);
		return;
	}

	/* store vaddr/paddr start address */
	reg_rw_ctrl.pa_start = regstart;
	reg_rw_ctrl.len = akuio_get_mapped_regs_size((void *)reg_rw_ctrl.va_start);
	if (0 == reg_rw_ctrl.len) {
		reg_rw_ctrl.len = size;
	}
	if (reg_rw_ctrl.len < size) {
		ak_print_warning_ex(
			"venc_cb_init_reg mapped window smaller than requested: "
			"requested=0x%08x mapped=0x%08x start=0x%08x\n",
			(unsigned int)size,
...
int venc_cb_reg_ready(void)
{
	return (reg_rw_ctrl.va_start != 0) && (reg_rw_ctrl.len != 0);
}

unsigned long venc_cb_reg_len(void)
{
	return reg_rw_ctrl.len;
}

/* read register */
unsigned long venc_cb_read_reg(unsigned long reg)
{
	if (!reg_rw_ctrl.va_start || !reg_rw_ctrl.len) {
		++reg_access_err_cnt;
		if ((reg_access_err_cnt <= 3) || ((reg_access_err_cnt % 100) == 0)) {
			ak_print_error_ex("venc_cb_read_reg skip: map not ready, reg=0x%08x va=0x%08x len=0x%08x\n",
				(unsigned int)reg, (unsigned int)reg_rw_ctrl.va_start, (unsigned int)reg_rw_ctrl.len);
		}
		return 0;
	}
	if ((reg < reg_rw_ctrl.pa_start) || (reg >= (reg_rw_ctrl.pa_start + reg_rw_ctrl.len))) {
		++reg_access_err_cnt;
		if ((reg_access_err_cnt <= 3) || ((reg_access_err_cnt % 100) == 0)) {
			ak_print_error_ex("venc_cb_read_reg skip: reg out of range, reg=0x%08x pa_start=0x%08x len=0x%08x\n",
				(unsigned int)reg, (unsigned int)reg_rw_ctrl.pa_start, (unsigned int)reg_rw_ctrl.len);
		}
		return 0;
	}
...
void venc_cb_write_reg(unsigned long reg, unsigned long val)
{
	if (!reg_rw_ctrl.va_start || !reg_rw_ctrl.len) {
		++reg_access_err_cnt;
		if ((reg_access_err_cnt <= 3) || ((reg_access_err_cnt % 100) == 0)) {
			ak_print_error_ex("venc_cb_write_reg skip: map not ready, reg=0x%08x val=0x%08x va=0x%08x len=0x%08x\n",
				(unsigned int)reg, (unsigned int)val,
				(unsigned int)reg_rw_ctrl.va_start, (unsigned int)reg_rw_ctrl.len);
		}
		return;
	}
	if ((reg < reg_rw_ctrl.pa_start) || (reg >= (reg_rw_ctrl.pa_start + reg_rw_ctrl.len))) {
		++reg_access_err_cnt;
		if ((reg_access_err_cnt <= 3) || ((reg_access_err_cnt % 100) == 0)) {
			ak_print_error_ex("venc_cb_write_reg skip: reg out of range, reg=0x%08x val=0x%08x pa_start=0x%08x len=0x%08x\n",
				(unsigned int)reg, (unsigned int)val,
				(unsigned int)reg_rw_ctrl.pa_start, (unsigned int)reg_rw_ctrl.len);
		}
		return;
	}
