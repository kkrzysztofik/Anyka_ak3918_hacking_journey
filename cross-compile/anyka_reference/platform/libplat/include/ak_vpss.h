...
struct vpss_af_attr {
	...
	unsigned short   af_th;       // [0, 128], af_th is the threshold, values > threshold are taken and summed for high-frequency quantization in sharp module
...
struct vpss_isp_ae_attr {
	unsigned long exp_time_max;			// Maximum exposure time
    unsigned long exp_time_min;			// Minimum exposure time
    unsigned long d_gain_max;      		// Maximum digital gain
    unsigned long d_gain_min;     		// Minimum digital gain
    unsigned long isp_d_gain_min;  		// Minimum ISP digital gain
    unsigned long isp_d_gain_max;  		// Maximum ISP digital gain
    unsigned long a_gain_max;     		// Maximum analog gain
    unsigned long a_gain_min;      		// Minimum analog gain
    unsigned long exp_step;            	// User exposure adjustment step
    unsigned long exp_stable_range;     // Stable range
    unsigned long target_lumiance;     	// Target luminance
...
struct vpss_isp_3d_nr {
	...
	unsigned short updata_ref_y;		// Update Y
	unsigned short updata_ref_uv;		// Update UV
	unsigned short tnr_refFrame_format;	// Reference frame format
	...
	unsigned long md_th;				// [0, 65535] Motion detection threshold [0-127]
};
...
struct vpss_isp_awb_attr {
    ...
    unsigned short gr_low[10];			// White balance step range
    unsigned short gb_low[10]; 			// Maximum total count threshold
    unsigned short gr_high[10];
    unsigned short gb_high[10];
    unsigned short rb_low[10];			//rb_low[i]<=rb_high[i]
    unsigned short rb_high[10];

    // AWB initialization parameters
    unsigned short auto_wb_step;                // Auto white balance step
    unsigned short total_cnt_thresh;            // Total count threshold
    unsigned short colortemp_stable_cnt_thresh; // Stable frame count threshold
...
struct vpss_isp_ae_run_info {
    unsigned char current_calc_avg_lumi;         		// Current calculated average luminance
    unsigned char current_calc_avg_compensation_lumi;	// Current calculated average compensation luminance

    unsigned char current_darked_flag;                 	// Day/night marker

    long  current_a_gain;				// Current analog gain
    long  current_d_gain;				// Current digital gain
    long  current_isp_d_gain;			// Current ISP digital gain
    long  current_exp_time;				// Current exposure time

    unsigned long  current_a_gain_step;	// Current analog gain step
    unsigned long  current_d_gain_step; // Current digital gain step
    unsigned long  current_isp_d_gain_step;	// Current ISP digital gain step
    unsigned long  current_exp_time_step;	// Current exposure time step
};
...
struct vpss_isp_wdr {
    unsigned short   hdr_uv_adjust_level;        //uv adjustment intensity, [0,31]
    unsigned short   hdr_cnoise_suppress_slop;   //suppression slope
    ...
    unsigned short   hdr_uv_adjust_enable;       //uv adjustment enable
    unsigned short   hdr_cnoise_suppress_yth1;   // color noise luminance threshold 1
    unsigned short   hdr_cnoise_suppress_yth2;   // color noise luminance threshold 2
    unsigned short   hdr_cnoise_suppress_gain;   //color noise suppression
};

struct vpss_wdr_attr {
	unsigned short      wdr_mode;              // mode selection, manual or linkage
