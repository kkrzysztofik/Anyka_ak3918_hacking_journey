...
Comprehensive ONVIF implementation (Device, Media, PTZ, Imaging, RTSP integration, WS-Discovery) for the Anyka AK3918 platform. Provides live RTSP streaming, PTZ control, imaging (day/night + IR LED), and basic SOAP/HTTP handling.
...
## 3. Configuration (`/etc/jffs2/ankya_cfg.ini`)
...
## 5. RTSP Streaming Integration

RTSP server starts when video input + encoding init succeed.
...
- RTSP integration for main stream
- WS-Discovery responses
- Simple config system (INI)
