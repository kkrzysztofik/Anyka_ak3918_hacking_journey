 ...
 * 
 * This module consolidates all service request handling functionality to eliminate
 * duplication and provide a single, consistent API for service operations.
 */
...
int onvif_service_handler_init(onvif_service_handler_instance_t *handler,
  ...
  memset(handler, 0, sizeof(onvif_service_handler_t));
  ...
  if (onvif_xml_builder_init(&handler->xml_builder, handler->xml_buffer, sizeof(handler->xml_buffer), NULL) != ONVIF_SUCCESS) {
...
int onvif_service_handler_generate_success(onvif_service_handler_instance_t *handler,
  ...
  if (handler->config.enable_logging) {
    onvif_service_handler_log(handler, action_name, "Request processed successfully", 0);
  }
...
int onvif_service_handler_generate_error(onvif_service_handler_instance_t *handler,
  ...
  int result = onvif_generate_fault_response(response, SOAP_FAULT_RECEIVER, error_message);
  if (result != ONVIF_SUCCESS) {
    return result;
  }
...
onvif_xml_builder_t *onvif_service_handler_get_xml_builder(onvif_service_handler_instance_t *handler) {
...
int onvif_service_handler_reset_xml_builder(onvif_service_handler_instance_t *handler) {
  ...
  // Reset XML builder
  onvif_xml_builder_cleanup(&handler->xml_builder);
  return onvif_xml_builder_init(&handler->xml_builder, handler->xml_buffer, sizeof(handler->xml_buffer), NULL);
}
...
int onvif_service_handler_register_action(onvif_service_handler_instance_t *handler,
  ...
  service_action_def_t *new_actions = ONVIF_REALLOC(handler->actions, 
                                                   new_count * sizeof(service_action_def_t));
  if (!new_actions) {
    return ONVIF_ERROR;
  }
...
int onvif_handle_unsupported_action(onvif_response_t *response) {
  ...
  return onvif_generate_fault_response(response, SOAP_FAULT_SENDER, "Unsupported action");
}
...
int onvif_handle_missing_parameter(onvif_response_t *response, const char *param_name) {
  ...
  return onvif_generate_fault_response(response, SOAP_FAULT_SENDER, error_message);
}
...
int onvif_handle_service_error(onvif_response_t *response, const char *error_message) {
  ...
  return onvif_generate_fault_response(response, SOAP_FAULT_RECEIVER, error_message);
}
