 ...
 * @date 2025
 *
 * This module consolidates all XML functionality to eliminate duplication and
 * provide a single, consistent API for XML operations across all ONVIF
 * services.
 */
...
int onvif_xml_builder_start_document(onvif_xml_builder_t *builder,
  ...
  int result =
      memory_safe_snprintf(builder->buffer + builder->current_pos,
                           builder->buffer_size - builder->current_pos,
                           "<?xml version=\"1.0\" encoding=\"%s\"?>\n", enc);

  if (result < 0 ||
      (size_t)result >= (builder->buffer_size - builder->current_pos)) {
    builder->error = 1;
    return ONVIF_ERROR;
  }
...
int onvif_xml_parser_extract_int(onvif_xml_parser_t *parser,
  ...
  char buffer[32];
  int result = onvif_xml_parser_extract_value(parser, start_tag, end_tag,
...
int onvif_xml_validate(const char *xml, size_t length,
  ...
  const onvif_xml_validation_config_t *cfg =
      config ? config : &default_validation_config;
...
int onvif_xml_validate_basic(const char *xml, size_t length) {
  ...
  if (length < 5 ||
      (strncmp(xml, "<?xml", 5) != 0 && strncmp(xml, "<", 1) != 0)) {
    return ONVIF_ERROR_INVALID;
  }
...
int onvif_xml_parse_ptz_position(onvif_xml_parser_t *parser,
  ...
  // Parse X coordinate
  if (onvif_xml_parser_extract_float(parser, "<tt:PanTilt><tt:x>",
                                     "</tt:x></tt:PanTilt>",
                                     &position->pan_tilt.x) != ONVIF_SUCCESS) {
    return ONVIF_ERROR;
  }

  // Parse Y coordinate
  if (onvif_xml_parser_extract_float(parser, "<tt:PanTilt><tt:y>",
                                     "</tt:y></tt:PanTilt>",
                                     &position->pan_tilt.y) != ONVIF_SUCCESS) {
    return ONVIF_ERROR;
  }

  // Parse Z coordinate
  if (onvif_xml_parser_extract_float(parser, "<tt:Zoom><tt:x>",
                                     "</tt:x></tt:Zoom>",
                                     &position->zoom) != ONVIF_SUCCESS) {
    return ONVIF_ERROR;
  }
...
int onvif_xml_parse_ptz_speed(onvif_xml_parser_t *parser,
  ...
  if (onvif_xml_parser_extract_float(parser, "<tt:PanTilt><tt:x>",
                                     "</tt:x></tt:PanTilt>",
                                     &speed->pan_tilt.x) != ONVIF_SUCCESS) {
    return ONVIF_ERROR;
  }

  if (onvif_xml_parser_extract_float(parser, "<tt:PanTilt><tt:y>",
                                     "</tt:y></tt:PanTilt>",
                                     &speed->pan_tilt.y) != ONVIF_SUCCESS) {
    return ONVIF_ERROR;
  }
...
int onvif_xml_parse_imaging_settings(onvif_xml_parser_t *parser,
  ...
  // Parse brightness
  if (onvif_xml_parser_extract_int(parser, "<tt:Brightness>",
                                   "</tt:Brightness>",
                                   &settings->brightness) != ONVIF_SUCCESS) {
    return ONVIF_ERROR;
  }

  // Parse contrast
  if (onvif_xml_parser_extract_int(parser, "<tt:Contrast>", "</tt:Contrast>",
                                   &settings->contrast) != ONVIF_SUCCESS) {
    return ONVIF_ERROR;
  }

  // Parse saturation
  if (onvif_xml_parser_extract_int(parser, "<tt:ColorSaturation>",
                                   "</tt:ColorSaturation>",
                                   &settings->saturation) != ONVIF_SUCCESS) {
    return ONVIF_ERROR;
  }
...
int onvif_xml_parse_video_source_configuration(
  ...
  // Parse token
  char token[64];
  if (onvif_xml_parser_extract_value(parser, "<tt:ConfigurationToken>",
                                     "</tt:ConfigurationToken>", token,
                                     sizeof(token)) != ONVIF_SUCCESS) {
    return ONVIF_ERROR;
  }
  ...
  // Parse name
  char name[64];
  if (onvif_xml_parser_extract_value(parser, "<tt:Name>", "</tt:Name>", name,
                                     sizeof(name)) != ONVIF_SUCCESS) {
    return ONVIF_ERROR;
  }
...
int onvif_xml_parse_profile_token(const char *xml, char *token,
                                  size_t token_size) {
  return onvif_xml_extract_string_value(
      xml, "<tt:ProfileToken>", "</tt:ProfileToken>", token, token_size);
}

int onvif_xml_parse_protocol(const char *xml, char *protocol,
                             size_t protocol_size) {
  return onvif_xml_extract_string_value(xml, "<tt:Protocol>", "</tt:Protocol>",
                                        protocol, protocol_size);
}

int onvif_xml_parse_configuration_token(const char *xml, char *token,
                                        size_t token_size) {
  return onvif_xml_extract_string_value(xml, "<tt:ConfigurationToken>",
                                        "</tt:ConfigurationToken>", token,
                                        token_size);
}
