 ...
 * for the RTSP server, including RTP packet creation, transmission, and encoder setup.
 */
...
int rtsp_setup_encoder(rtsp_server_t *server) {
    ...
    venc_config.bitrate = server->config.video_config.bitrate * 1000; // Convert kbps to bps
    ...
    platform_log_notice("Video encoder created: %dx%d@%dfps, %dkbps\n", 
                        venc_config.width, venc_config.height, venc_config.fps, venc_config.bitrate/1000);
    return 0;
}
...
int rtsp_setup_audio_encoder(rtsp_server_t *server) {
    if (!server || !server->config.audio_enabled) return 0;
    
    // Create audio input using platform abstraction
    if (platform_ai_open(&server->ai_handle) != PLATFORM_SUCCESS) {
        platform_log_error("Failed to create audio input\n");
        return -1;
    }
    
    // Create audio encoder using platform abstraction
    platform_audio_config_t aenc_config;
    aenc_config.sample_rate = 8000;
    aenc_config.channels = 1;
    aenc_config.bits_per_sample = 16;
    aenc_config.codec = PLATFORM_AUDIO_CODEC_G711A;
    
    if (platform_aenc_init(&server->aenc_handle, &aenc_config) != PLATFORM_SUCCESS) {
        platform_log_error("Failed to create audio encoder\n");
    ...
    platform_log_notice("Audio encoder created: %dHz, %d channels, %d bits\n", 
...
int rtsp_init_rtp_session(rtsp_session_t *session) {
    ...
    platform_log_notice("RTP session initialized: RTP port %d, RTCP port %d, SSRC %u\n", 
                        session->rtp_session.rtp_port, session->rtp_session.rtcp_port, session->rtp_session.ssrc);
    return 0;
}
...
int rtsp_init_audio_rtp_session(rtsp_session_t *session) {
    ...
    platform_log_notice("Audio RTP session initialized: RTP port %d, RTCP port %d, SSRC %u\n", 
                        session->audio_rtp_session.rtp_port, session->audio_rtp_session.rtcp_port, session->audio_rtp_session.ssrc);
    return 0;
}
...
int rtsp_send_audio_rtp_packet(rtsp_session_t *session, const uint8_t *data, size_t len, uint32_t timestamp) {
    ...
    // RTP header
    rtp_packet[pos++] = 0x80; // Version (2), Padding (0), Extension (0), CC (0)
    rtp_packet[pos++] = RTP_PT_PCMA; // Payload type
