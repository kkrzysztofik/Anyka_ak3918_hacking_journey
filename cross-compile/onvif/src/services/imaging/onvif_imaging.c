...
static int handle_imaging_validation_error(const error_context_t *context, const error_result_t *result, onvif_response_t *response);
static int handle_imaging_vpss_error(const error_context_t *context, const error_result_t *result, onvif_response_t *response);
static int handle_imaging_config_error(const error_context_t *context, const error_result_t *result, onvif_response_t *response);
...
int onvif_imaging_init(void *vi_handle) {
    ...
    // Note: Imaging settings are now managed through the centralized config system
    // The centralized config will be passed to the service initialization
    ...
    init_default_auto_config(&g_auto_config);
    
    // Load auto configuration
    // Note: Auto configuration is now managed through the centralized config system
...
static onvif_service_handler_instance_t g_imaging_handler;
...
static int handle_get_imaging_settings(const service_handler_config_t *config,
  ...
  if (onvif_xml_extract_string_value(request->body, IMAGING_XML_VIDEO_TOKEN_TAG, "</VideoSourceToken>", 
  ...
  onvif_xml_builder_element_with_formatted_text(xml_builder, "tt:Brightness", "%d", settings.brightness);
  onvif_xml_builder_element_with_formatted_text(xml_builder, "tt:ColorSaturation", "%d", settings.saturation);
  onvif_xml_builder_element_with_formatted_text(xml_builder, "tt:Contrast", "%d", settings.contrast);
  onvif_xml_builder_element_with_formatted_text(xml_builder, "tt:Sharpness", "%d", settings.sharpness);
  onvif_xml_builder_element_with_formatted_text(xml_builder, "tt:Hue", "%d", settings.hue);
  ...
  return onvif_generate_complete_response(response, ONVIF_SERVICE_IMAGING, "GetImagingSettings", xml_content);
}
...
static int handle_set_imaging_settings(const service_handler_config_t *config,
  ...
  if (onvif_xml_parse_imaging_settings(&parser, &settings) != 0) {
    ...
    return error_handle_parameter(&error_ctx, "imaging_settings", "invalid", response);
  }
  ...
  if (result == ONVIF_SUCCESS) {
    // Generate success response using SOAP response utility
    return onvif_generate_complete_response(response, ONVIF_SERVICE_IMAGING, "SetImagingSettings", "");
  } else {
    return onvif_generate_fault_response(response, SOAP_FAULT_RECEIVER, "Internal server error");
  }
}
...
int onvif_imaging_service_init(config_manager_t *config) {
  ...
  int result = onvif_service_handler_init(&g_imaging_handler, &handler_config,
...
int onvif_imaging_handle_request(onvif_action_type_t action, const onvif_request_t *request, onvif_response_t *response) {
    ...
    return onvif_service_handler_handle_request(&g_imaging_handler, action, request, response);
}