...
static int parse_profile_token(const char *request, char *token,
                               size_t token_size);
static int parse_protocol(const char *request, char *protocol,
                          size_t protocol_size);
static int build_profile_xml(const struct media_profile *profile, char *xml,
                             size_t xml_size);
static int build_video_source_xml(const struct video_source *source, char *xml,
                                  size_t xml_size);
static int build_stream_uri_xml(const struct stream_uri *uri, char *xml,
                                size_t xml_size);
static int validate_profile_token(const char *token);
static int validate_protocol(const char *protocol);
static int find_profile_by_token(const char *token,
                                 struct media_profile **profile);
static void init_default_profiles(void);
static int handle_media_validation_error(const error_context_t *context,
                                         const error_result_t *result,
                                         onvif_response_t *response);
static int handle_media_stream_error(const error_context_t *context,
                                     const error_result_t *result,
                                     onvif_response_t *response);
...
static struct media_profile g_media_profiles[] = {  // NOLINT
    {.token = MEDIA_MAIN_PROFILE_TOKEN,
     ...
     .video_source = {.source_token = "VideoSource0",
                      .bounds = {MEDIA_MAIN_RESOLUTION_WIDTH,
                                 MEDIA_MAIN_RESOLUTION_HEIGHT, 0, 0}},
     .video_encoder = {.token = "VideoEncoder0",
                       .encoding = "H264",
                       .resolution = {MEDIA_MAIN_RESOLUTION_WIDTH,
                                      MEDIA_MAIN_RESOLUTION_HEIGHT},
    ...
    {.token = MEDIA_SUB_PROFILE_TOKEN,
     ...
     .fixed = 1,
     .video_source = {.source_token = "VideoSource0",
                      .bounds = {MEDIA_SUB_RESOLUTION_WIDTH,
                                 MEDIA_SUB_RESOLUTION_HEIGHT, 0, 0}},
     .video_encoder = {.token = "VideoEncoder1",
                       .encoding = "H264",
                       .resolution = {MEDIA_SUB_RESOLUTION_WIDTH,
                                      MEDIA_SUB_RESOLUTION_HEIGHT},
...
int onvif_media_get_video_sources(struct video_source **sources, int *count) {
  static struct video_source video_sources[] = {
      {.token = "VideoSource0",
       ...
       .resolution = {MEDIA_MAIN_RESOLUTION_WIDTH,
                      MEDIA_MAIN_RESOLUTION_HEIGHT},
       .imaging = {.brightness = 50.0f,
                   .color_saturation = 50.0f,
                   .contrast = 50.0f,
                   .sharpness = 50.0f}}};
...
int onvif_media_get_video_source_configurations(
    struct video_source_configuration **configs, int *count) {
  static struct video_source_configuration video_configs[] = {
      {.token = "VideoSourceConfig0",
       ...
       .bounds = {MEDIA_MAIN_RESOLUTION_WIDTH, MEDIA_MAIN_RESOLUTION_HEIGHT, 0,
                  0}}};
...
int onvif_media_get_video_encoder_configurations(
  ...
  static struct video_encoder_configuration video_enc_configs[] = {
      {.token = "VideoEncoder0",
       ...
       .multicast =
           {.address = "0.0.0.0", .port = 0, .ttl = 5, .auto_start = 0}},
      {.token = "VideoEncoder1",
...
int onvif_media_get_audio_source_configurations(
    struct audio_source_configuration **configs, int *count) {
...
int onvif_media_get_audio_encoder_configurations(
...
int onvif_media_get_metadata_configurations(
    struct metadata_configuration **configs, int *count) {
  ...
  if (strcmp(config->encoding, "AAC") == 0) {
    ...
    if (config->bitrate < MEDIA_AUDIO_BITRATE_AAC_MIN ||
      ...
      platform_log_error("Invalid AAC bitrate: %d kbps (must be %d-%d)\n",
                         config->bitrate, MEDIA_AUDIO_BITRATE_AAC_MIN,
                         MEDIA_AUDIO_BITRATE_AAC_MAX);
    ...
    if (config->sample_rate != 8000 && config->sample_rate != 16000 &&
      ...
      platform_log_error(
          "Unsupported AAC sample rate: %d Hz (supported: 8000, 16000, 22050, "
          "44100, 48000)\n",
          config->sample_rate);
      return ONVIF_ERROR_NOT_SUPPORTED;
    }
...
static int parse_profile_token(const char *request, char *token,
  ...
  return onvif_xml_extract_string_value(request, MEDIA_XML_PROFILE_TOKEN_TAG,
...
static int parse_protocol(const char *request, char *protocol,
  ...
  return onvif_xml_extract_string_value(request, MEDIA_XML_PROTOCOL_TAG,
...
static int validate_profile_token(const char *token) {
  ...
  if (strcmp(token, MEDIA_MAIN_PROFILE_TOKEN) == 0 ||
      strcmp(token, MEDIA_SUB_PROFILE_TOKEN) == 0) {
    return ONVIF_SUCCESS;
  }
...
static int build_profile_xml(const struct media_profile *profile, char *xml,
  ...
  int result = snprintf(
      ...
      "      "
      "<tt:DefaultAbsolutePantTiltPositionSpace>%s</"
      "tt:DefaultAbsolutePantTiltPositionSpace>\n"
      "      "
      "<tt:DefaultAbsoluteZoomPositionSpace>%s</"
      "tt:DefaultAbsoluteZoomPositionSpace>\n"
      "      "
      "<tt:DefaultRelativePantTiltTranslationSpace>%s</"
      "tt:DefaultRelativePantTiltTranslationSpace>\n"
      "      "
      "<tt:DefaultRelativeZoomTranslationSpace>%s</"
      "tt:DefaultRelativeZoomTranslationSpace>\n"
      "      "
      "<tt:DefaultContinuousPantTiltVelocitySpace>%s</"
      "tt:DefaultContinuousPantTiltVelocitySpace>\n"
      "      "
      "<tt:DefaultContinuousZoomVelocitySpace>%s</"
      "tt:DefaultContinuousZoomVelocitySpace>\n"
      "    </tt:PTZConfiguration>\n"
      "  </trt:Profile>\n",
      profile->token, profile->fixed ? "true" : "false", profile->name,
      profile->video_source.source_token, profile->video_source.bounds.x,
      profile->video_source.bounds.y, profile->video_source.bounds.width,
      profile->video_source.bounds.height, profile->video_encoder.token,
      profile->video_encoder.encoding, profile->video_encoder.resolution.width,
      profile->video_encoder.resolution.height, profile->video_encoder.quality,
      profile->video_encoder.framerate_limit,
      profile->video_encoder.encoding_interval,
      profile->video_encoder.bitrate_limit, profile->video_encoder.gov_length,
      profile->audio_source.source_token, profile->audio_encoder.token,
      profile->audio_encoder.encoding, profile->audio_encoder.bitrate,
      profile->audio_encoder.sample_rate, profile->ptz.node_token,
...
static int build_video_source_xml(const struct video_source *source, char *xml,
  ...
  int result = snprintf(xml, xml_size,
                        ...
                        "    </tt:Imaging>\n"
                        "  </tt:VideoSource>\n",
                        source->token, source->resolution.width,
                        source->resolution.height, source->imaging.brightness,
                        source->imaging.color_saturation,
...
static int handle_get_profiles(const service_handler_config_t *config,
  ...
  for (int i = 0; i < MEDIA_PROFILE_COUNT_DEFAULT; i++) {
    onvif_xml_builder_start_element(xml_builder, "tt:Profile", "token",
                                    g_media_profiles[i].token, NULL);

    onvif_xml_builder_element_with_text(xml_builder, "tt:Name",
                                        g_media_profiles[i].name, NULL);
    onvif_xml_builder_element_with_formatted_text(
        xml_builder, "tt:Fixed", "%s",
        g_media_profiles[i].fixed ? "true" : "false");

    // Video source
    onvif_xml_builder_start_element(
        xml_builder, "tt:VideoSource", "token",
        g_media_profiles[i].video_source.source_token, NULL);
    onvif_xml_builder_start_element(xml_builder, "tt:Bounds", NULL);
    onvif_xml_builder_element_with_formatted_text(
        xml_builder, "tt:width", "%d",
        g_media_profiles[i].video_source.bounds.width);
    onvif_xml_builder_element_with_formatted_text(
        xml_builder, "tt:height", "%d",
        g_media_profiles[i].video_source.bounds.height);
    onvif_xml_builder_element_with_formatted_text(
        xml_builder, "tt:x", "%d", g_media_profiles[i].video_source.bounds.x);
    onvif_xml_builder_element_with_formatted_text(
        xml_builder, "tt:y", "%d", g_media_profiles[i].video_source.bounds.y);
    onvif_xml_builder_end_element(xml_builder, "tt:Bounds");
    onvif_xml_builder_end_element(xml_builder, "tt:VideoSource");

    // Video encoder
    onvif_xml_builder_start_element(xml_builder, "tt:VideoEncoder", "token",
                                    g_media_profiles[i].video_encoder.token,
                                    NULL);
    onvif_xml_builder_element_with_text(
        xml_builder, "tt:Name", g_media_profiles[i].video_encoder.encoding,
        NULL);
    onvif_xml_builder_element_with_text(
        xml_builder, "tt:Encoding", g_media_profiles[i].video_encoder.encoding,
        NULL);

    onvif_xml_builder_start_element(xml_builder, "tt:Resolution", NULL);
    onvif_xml_builder_element_with_formatted_text(
        xml_builder, "tt:Width", "%d",
        g_media_profiles[i].video_encoder.resolution.width);
    onvif_xml_builder_element_with_formatted_text(
        xml_builder, "tt:Height", "%d",
        g_media_profiles[i].video_encoder.resolution.height);
    onvif_xml_builder_end_element(xml_builder, "tt:Resolution");

    onvif_xml_builder_element_with_formatted_text(
        xml_builder, "tt:Quality", "%.1f",
        g_media_profiles[i].video_encoder.quality);
    onvif_xml_builder_element_with_formatted_text(
        xml_builder, "tt:FrameRateLimit", "%d",
        g_media_profiles[i].video_encoder.framerate_limit);
    onvif_xml_builder_element_with_formatted_text(
        xml_builder, "tt:EncodingInterval", "%d",
        g_media_profiles[i].video_encoder.encoding_interval);
    onvif_xml_builder_element_with_formatted_text(
        xml_builder, "tt:BitrateLimit", "%d",
        g_media_profiles[i].video_encoder.bitrate_limit);
    onvif_xml_builder_element_with_formatted_text(
        xml_builder, "tt:GovLength", "%d",
        g_media_profiles[i].video_encoder.gov_length);
  ...
  return onvif_generate_complete_response(response, ONVIF_SERVICE_MEDIA,
                                          "GetProfiles", xml_content);
}
...
static int handle_get_stream_uri(const service_handler_config_t *config,
  ...
  // Initialize error context
  error_context_t error_ctx;
  error_context_init(&error_ctx, "Media", "GetStreamUri",
                     "stream_uri_retrieval");
  ...
  if (onvif_xml_parse_protocol(request->body, protocol, sizeof(protocol)) !=
      0) {
...
static int handle_create_profile(const service_handler_config_t *config,
  ...
  if (!config || !response || !xml_builder) {
    return error_handle_parameter(&error_ctx, "parameters", "missing",
                                  response);
  }
  ...
  struct media_profile profile;
  int result =
      onvif_media_create_profile(profile_name, profile_token, &profile);
  ...
  // Build response XML
  onvif_xml_builder_start_element(xml_builder, "trt:Profile", "token",
                                  profile.token, NULL);
  onvif_xml_builder_element_with_text(xml_builder, "tt:Name", profile.name,
                                      NULL);
  onvif_xml_builder_element_with_formatted_text(
      xml_builder, "tt:Fixed", "%s", profile.fixed ? "true" : "false");
  onvif_xml_builder_end_element(xml_builder, "trt:Profile");

  const char *xml_content = onvif_xml_builder_get_string(xml_builder);
  return onvif_generate_complete_response(response, ONVIF_SERVICE_MEDIA,
                                          "CreateProfile", xml_content);
}
...
static int handle_delete_profile(const service_handler_config_t *config,
  ...
  if (strstr(request->body, "<trt:ProfileToken>") != NULL) {
    onvif_xml_extract_string_value(request->body, "<trt:ProfileToken>",
                                   "</trt:ProfileToken>", profile_token,
                                   sizeof(profile_token));
  }

  if (strlen(profile_token) == 0) {
    return error_handle_parameter(&error_ctx, "profile_token", "missing",
                                  response);
  }
...
static int handle_set_video_source_configuration(
    const service_handler_config_t *config, const onvif_request_t *request,
    onvif_response_t *response, onvif_xml_builder_t *xml_builder) {
  error_context_t error_ctx;
  error_context_init(&error_ctx, "Media", "SetVideoSourceConfiguration",
                     "config_update");
  ...
  // Parse configuration token and parameters from request using common XML
  // parser
  char config_token[64];
  if (onvif_xml_parse_configuration_token(request->body, config_token,
                                          sizeof(config_token)) != 0) {
    return error_handle_parameter(&error_ctx, "configuration_token", "missing",
                                  response);
  }
  ...
  if (onvif_xml_parse_video_source_configuration(&parser, &video_config) != 0) {
    ...
    return error_handle_parameter(&error_ctx, "video_source_configuration",
                                  "invalid", response);
  }
  ...
  // Set configuration
  int result =
      onvif_media_set_video_source_configuration(config_token, &video_config);
...
static int handle_set_video_encoder_configuration(
    const service_handler_config_t *config, const onvif_request_t *request,
    onvif_response_t *response, onvif_xml_builder_t *xml_builder) {
  error_context_t error_ctx;
  error_context_init(&error_ctx, "Media", "SetVideoEncoderConfiguration",
                     "config_update");
  ...
  if (onvif_xml_parse_video_encoder_configuration(&parser, &encoder_config) !=
      0) {
    onvif_xml_parser_cleanup(&parser);
    return error_handle_parameter(&error_ctx, "video_encoder_configuration",
                                  "invalid", response);
  }
  ...
  // Set configuration
  int result = onvif_media_set_video_encoder_configuration(config_token,
                                                           &encoder_config);
...
static int handle_start_multicast_streaming(
    const service_handler_config_t *config, const onvif_request_t *request,
    onvif_response_t *response, onvif_xml_builder_t *xml_builder) {
  error_context_t error_ctx;
  error_context_init(&error_ctx, "Media", "StartMulticastStreaming",
                     "multicast_start");
...
static int handle_stop_multicast_streaming(
    const service_handler_config_t *config, const onvif_request_t *request,
    onvif_response_t *response, onvif_xml_builder_t *xml_builder) {
  error_context_t error_ctx;
  error_context_init(&error_ctx, "Media", "StopMulticastStreaming",
                     "multicast_stop");
...
static int handle_get_metadata_configurations(
    const service_handler_config_t *config, const onvif_request_t *request,
    onvif_response_t *response, onvif_xml_builder_t *xml_builder) {
  error_context_t error_ctx;
  error_context_init(&error_ctx, "Media", "GetMetadataConfigurations",
                     "metadata_config_retrieval");
  ...
  if (result != ONVIF_SUCCESS) {
    return error_handle_system(&error_ctx, result, "metadata_config_retrieval",
                               response);
  }
  ...
  for (int i = 0; i < count; i++) {
    onvif_xml_builder_start_element(xml_builder, "tt:Configuration", "token",
                                    configs[i].token, NULL);
    onvif_xml_builder_element_with_text(xml_builder, "tt:Name", configs[i].name,
                                        NULL);
    onvif_xml_builder_element_with_formatted_text(xml_builder, "tt:UseCount",
                                                  "%d", configs[i].use_count);
    onvif_xml_builder_element_with_formatted_text(
        xml_builder, "tt:SessionTimeout", "PT%dS", configs[i].session_timeout);
    onvif_xml_builder_element_with_formatted_text(
        xml_builder, "tt:Analytics", "%s",
        configs[i].analytics ? "true" : "false");
  ...
  const char *xml_content = onvif_xml_builder_get_string(xml_builder);
  return onvif_generate_complete_response(
      response, ONVIF_SERVICE_MEDIA, "GetMetadataConfigurations", xml_content);
}

static int handle_set_metadata_configuration(
    const service_handler_config_t *config, const onvif_request_t *request,
    onvif_response_t *response, onvif_xml_builder_t *xml_builder) {
  error_context_t error_ctx;
  error_context_init(&error_ctx, "Media", "SetMetadataConfiguration",
                     "metadata_config_update");
  ...
  if (strstr(request->body, "<trt:ConfigurationToken>") != NULL) {
    onvif_xml_extract_string_value(request->body, "<trt:ConfigurationToken>",
                                   "</trt:ConfigurationToken>", config_token,
                                   sizeof(config_token));
  }
  ...
  strncpy(metadata_config.token, config_token,
          sizeof(metadata_config.token) - 1);
  ...
  if (strstr(request->body, "<tt:Analytics>") != NULL) {
    ...
    if (onvif_xml_extract_string_value(request->body, "<tt:Analytics>",
                                       "</tt:Analytics>", analytics_str,
  ...
  if (strstr(request->body, "<tt:SessionTimeout>") != NULL) {
    char timeout_str[16];
    if (onvif_xml_extract_string_value(request->body, "<tt:SessionTimeout>",
                                       "</tt:SessionTimeout>", timeout_str,
                                       sizeof(timeout_str)) == 0) {
      // Parse PT60S format
      if (strstr(timeout_str, "PT") != NULL &&
          strstr(timeout_str, "S") != NULL) {
        char *endptr = NULL;
  ...
  // Set configuration
  int result =
      onvif_media_set_metadata_configuration(config_token, &metadata_config);

  if (result != ONVIF_SUCCESS) {
    return error_handle_system(&error_ctx, result, "metadata_config_update",
                               response);
  }
...
static const service_action_def_t media_actions[] = {
    ...
    {ONVIF_ACTION_SET_VIDEO_SOURCE_CONFIGURATION, "SetVideoSourceConfiguration",
     handle_set_video_source_configuration, 1},
    {ONVIF_ACTION_SET_VIDEO_ENCODER_CONFIGURATION,
     "SetVideoEncoderConfiguration", handle_set_video_encoder_configuration, 1},
    {ONVIF_ACTION_START_MULTICAST_STREAMING, "StartMulticastStreaming",
     handle_start_multicast_streaming, 1},
    {ONVIF_ACTION_STOP_MULTICAST_STREAMING, "StopMulticastStreaming",
     handle_stop_multicast_streaming, 1},
    {ONVIF_ACTION_GET_METADATA_CONFIGURATIONS, "GetMetadataConfigurations",
     handle_get_metadata_configurations, 0},
    {ONVIF_ACTION_SET_METADATA_CONFIGURATION, "SetMetadataConfiguration",
     handle_set_metadata_configuration, 1}};
...
int onvif_media_init(config_manager_t *config) {
  ...
  int result = onvif_service_handler_init(
      &g_media_handler, &handler_config, media_actions,
...
int onvif_media_handle_request(onvif_action_type_t action,
                               const onvif_request_t *request,
                               onvif_response_t *response) {
  if (!g_handler_initialized) {
    return ONVIF_ERROR;
  }
  return onvif_service_handler_handle_request(&g_media_handler, action, request,
                                              response);
}
