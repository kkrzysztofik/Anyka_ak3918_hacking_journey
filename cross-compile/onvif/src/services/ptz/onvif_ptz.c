...
static const struct ptz_node g_ptz_node = {
    ...
    .supported_ptz_spaces =
        {.absolute_pan_tilt_position_space =
             {.uri = "http://www.onvif.org/ver10/tptz/PanTiltSpaces/"
...
static int handle_ptz_validation_error(const error_context_t *context,
                                       const error_result_t *result,
                                       onvif_response_t *response);
static int handle_ptz_system_error(const error_context_t *context,
                                   const error_result_t *result,
                                   onvif_response_t *response);
...
int onvif_ptz_get_configuration(const char *config_token,
  ...
  config->pan_tilt_limits.range
      .uri[sizeof(config->pan_tilt_limits.range.uri) - 1] = '\0';
...
int onvif_ptz_get_status(const char *profile_token, struct ptz_status *status) {
  ...
  status->position.pan_tilt.x =
      degrees_to_normalize_pan(adapter_status.h_pos_deg);
  status->position.pan_tilt.y =
      degrees_to_normalize_tilt(adapter_status.v_pos_deg);
  ...
  status->move_status.pan_tilt =
      (adapter_status.h_speed > 0 || adapter_status.v_speed > 0)
          ? PTZ_MOVE_MOVING
          : PTZ_MOVE_IDLE;
  ...
  (void)strftime(status->utc_time, sizeof(status->utc_time),
                 "%Y-%m-%dT%H:%M:%S.000Z", utc_tm);

  return ONVIF_SUCCESS;
}

int onvif_ptz_absolute_move(const char *profile_token,
                            const struct ptz_vector *position,
...
int onvif_ptz_relative_move(const char *profile_token,
                            const struct ptz_vector *translation,
                            const struct ptz_speed *speed) {
  ONVIF_CHECK_NULL(profile_token);
...
int onvif_ptz_continuous_move(const char *profile_token,
                              const struct ptz_speed *velocity, int timeout) {
  ONVIF_CHECK_NULL(profile_token);
...
int onvif_ptz_get_presets(const char *profile_token,
                          struct ptz_preset **preset_list, int *count) {
  ONVIF_CHECK_NULL(profile_token);
...
int onvif_ptz_set_preset(const char *profile_token,  // NOLINT
  ...
  if (snprintf(preset->token, sizeof(preset->token), "Preset%d",
               g_ptz_preset_count + 1) >= sizeof(preset->token)) {
    return ONVIF_ERROR;
  }
...
int onvif_ptz_goto_preset(const char *profile_token,  // NOLINT
                          const char *preset_token_to_goto,
                          const struct ptz_speed *speed) {
  ONVIF_CHECK_NULL(profile_token);
  ...
  return onvif_ptz_absolute_move(profile_token,
                                 &g_ptz_presets[index].ptz_position, speed);
}
...
static int handle_get_nodes(const service_handler_config_t *config,
  ...
  onvif_xml_builder_element_with_text(xml_builder, "tt:Name", g_ptz_node.name,
                                      NULL);
  onvif_xml_builder_element_with_text(xml_builder, "tt:SupportedPTZSpaces",
                                      "true", NULL);
  ...
  onvif_xml_builder_start_element(xml_builder,
                                  "tt:AbsolutePanTiltPositionSpace", NULL);
  ...
  onvif_xml_builder_element_with_formatted_text(
      xml_builder, "tt:Min", "%.1f",
      g_ptz_node.supported_ptz_spaces.absolute_pan_tilt_position_space.x_range
          .min);
  onvif_xml_builder_element_with_formatted_text(
      xml_builder, "tt:Max", "%.1f",
      g_ptz_node.supported_ptz_spaces.absolute_pan_tilt_position_space.x_range
          .max);
  ...
  onvif_xml_builder_element_with_formatted_text(
      xml_builder, "tt:Min", "%.1f",
      g_ptz_node.supported_ptz_spaces.absolute_pan_tilt_position_space.y_range
          .min);
  onvif_xml_builder_element_with_formatted_text(
      xml_builder, "tt:Max", "%.1f",
      g_ptz_node.supported_ptz_spaces.absolute_pan_tilt_position_space.y_range
  ...
  onvif_xml_builder_start_element(xml_builder, "tt:AbsoluteZoomPositionSpace",
                                  NULL);
  onvif_xml_builder_element_with_text(
      xml_builder, "tt:URI",
      g_ptz_node.supported_ptz_spaces.absolute_zoom_position_space.uri, NULL);
  onvif_xml_builder_start_element(xml_builder, "tt:XRange", NULL);
  onvif_xml_builder_element_with_formatted_text(
      xml_builder, "tt:Min", "%.1f",
      g_ptz_node.supported_ptz_spaces.absolute_zoom_position_space.x_range.min);
  onvif_xml_builder_element_with_formatted_text(
      xml_builder, "tt:Max", "%.1f",
      g_ptz_node.supported_ptz_spaces.absolute_zoom_position_space.x_range.max);
  onvif_xml_builder_end_element(xml_builder, "tt:XRange");
  ...
  onvif_xml_builder_element_with_formatted_text(
      xml_builder, "tt:MaximumNumberOfPresets", "%d",
      g_ptz_node.maximum_number_of_presets);

  // Home supported
  onvif_xml_builder_element_with_formatted_text(
      xml_builder, "tt:HomeSupported", "%s",
      g_ptz_node.home_supported ? "true" : "false");
  ...
  return onvif_generate_complete_response(response, ONVIF_SERVICE_PTZ,
                                          "GetNodes", xml_content);
}
...
static int handle_absolute_move(const service_handler_config_t *config,
  ...
  if (onvif_xml_parse_ptz_position(&parser, &position) != 0) {
    ...
    return error_handle_parameter(&error_ctx, "ptz_position", "invalid",
                                  response);
  }
...
static int handle_get_presets(const service_handler_config_t *config,
  ...
  for (int i = 0; i < count; i++) {
    onvif_xml_builder_start_element(xml_builder, "tt:Preset", "token",
                                    preset_list[i].token, NULL);
    onvif_xml_builder_element_with_text(xml_builder, "tt:Name",
                                        preset_list[i].name, NULL);
...
static int handle_set_preset(const service_handler_config_t *config,
  ...
  onvif_xml_extract_string_value(request->body, "<trt:Name>", "</trt:Name>",
                                 preset_name, sizeof(preset_name));

  // Set preset using existing function
  char preset_token[64];
  int result = onvif_ptz_set_preset(profile_token, preset_name, preset_token,
                                    sizeof(preset_token));
  ...
  // Build response XML
  onvif_xml_builder_element_with_text(xml_builder, "tptz:PresetToken",
                                      preset_token, NULL);

  // Generate success response using consistent SOAP response utility
  const char *xml_content = onvif_xml_builder_get_string(xml_builder);
  return onvif_generate_complete_response(response, ONVIF_SERVICE_PTZ,
                                          "SetPreset", xml_content);
}
...
static int handle_goto_preset(const service_handler_config_t *config,
  ...
  char preset_token[64];
  if (onvif_xml_extract_string_value(request->body, "<trt:PresetToken>",
                                     "</trt:PresetToken>", preset_token,
                                     sizeof(preset_token)) != 0) {
    return error_handle_parameter(&error_ctx, "preset_token", "missing",
                                  response);
  }
...
int onvif_ptz_init(config_manager_t *config) {
  ...
  int result =
      onvif_service_handler_init(&g_ptz_handler, &handler_config, ptz_actions,
 ...
 */
int onvif_ptz_handle_request(onvif_action_type_t action,
                             const onvif_request_t *request,
                             onvif_response_t *response) {
  if (!g_handler_initialized) {
    return ONVIF_ERROR;
  ...
  return onvif_service_handler_handle_request(&g_ptz_handler, action, request,
                                              response);
}
...
static void *continuous_move_timeout_thread(void *unused_arg) {
  ...
  if (g_ptz_continuous_move_active) {
    platform_log_info(
        "PTZ continuous move timeout after %ds, stopping movement\n",
...
int ptz_adapter_init(void) {
  ...
  if (!g_ptz_initialized) {
    ...
    if (ret == PLATFORM_SUCCESS) {
      ...
      platform_ptz_move_to_position(g_ptz_current_pan_pos,
                                    g_ptz_current_tilt_pos);
...
int ptz_adapter_relative_move(int pan_delta_degrees,   // NOLINT
  ...
  if (pan_delta_degrees != 0) {
    platform_ptz_direction_t dir = (pan_delta_degrees > 0)
    ...
    if (ret == PLATFORM_SUCCESS) {
      g_ptz_current_pan_pos +=
          (dir == PLATFORM_PTZ_DIRECTION_LEFT) ? steps : -steps;
    }
  }

  /* Vertical movement - based on akipc implementation with step size 8 */
  if (tilt_delta_degrees != 0) {
    platform_ptz_direction_t dir = (tilt_delta_degrees > 0)
                                       ? PLATFORM_PTZ_DIRECTION_DOWN
                                       : PLATFORM_PTZ_DIRECTION_UP;
    ...
    if (ret == PLATFORM_SUCCESS) {
      g_ptz_current_tilt_pos +=
          (dir == PLATFORM_PTZ_DIRECTION_DOWN) ? steps : -steps;
    }
  }
...
int ptz_adapter_continuous_move(int pan_velocity, int tilt_velocity,  // NOLINT
  ...
  if (pan_velocity != 0) {
    platform_ptz_direction_t dir = (pan_velocity > 0)
                                       ? PLATFORM_PTZ_DIRECTION_RIGHT
                                       : PLATFORM_PTZ_DIRECTION_LEFT;
    platform_ptz_turn(
        dir, PTZ_MAX_PAN_DEGREES); /* Large number for continuous movement */
  }

  if (tilt_velocity != 0) {
    platform_ptz_direction_t dir = (tilt_velocity > 0)
                                       ? PLATFORM_PTZ_DIRECTION_DOWN
                                       : PLATFORM_PTZ_DIRECTION_UP;
    platform_ptz_turn(
        dir, PTZ_MAX_TILT_DEGREES); /* Large number for continuous movement */
  }
  ...
  if (timeout_seconds > 0) {
    ...
    if (pthread_create(&g_ptz_continuous_move_timer_thread, NULL,
...
int ptz_adapter_set_preset(const char *name, int preset_id) {
  ...
  platform_log_info("PTZ set preset %s (id=%d) at pan=%d, tilt=%d\n",
                    name ? name : "unnamed", preset_id, g_ptz_current_pan_pos,
                    g_ptz_current_tilt_pos);
