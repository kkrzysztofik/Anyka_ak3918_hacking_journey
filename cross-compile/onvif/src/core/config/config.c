...
static const config_parameter_t auto_daynight_parameters_init[] = {
    {"auto_day_night_enable", CONFIG_TYPE_BOOL, NULL, sizeof(int), 0, 1, "1",
     0},
    {"day_night_mode", CONFIG_TYPE_INT, NULL, sizeof(int), 0, 2, "2", 0},
    {"day_to_night_lum", CONFIG_TYPE_INT, NULL, sizeof(int), 0, 10000, "6400",
     0},
    {"night_to_day_lum", CONFIG_TYPE_INT, NULL, sizeof(int), 0, 10000, "2048",
     0},
...
static const config_parameter_t network_parameters_init[] = {
    ...
    {"ws_discovery_port", CONFIG_TYPE_INT, NULL, sizeof(int), 1, 65535, "3702",
     0}};
...
static void initialize_parameter_arrays(void) {
  ...
  memcpy(imaging_parameters, imaging_parameters_init,
         sizeof(imaging_parameters));

  // Copy auto day/night parameters
  memcpy(auto_daynight_parameters, auto_daynight_parameters_init,
         sizeof(auto_daynight_parameters));

  // Copy network parameters
  memcpy(network_parameters, network_parameters_init,
  ...
  memcpy(main_stream_parameters, main_stream_parameters_init,
         sizeof(main_stream_parameters));

  // Copy sub stream parameters
  memcpy(sub_stream_parameters, sub_stream_parameters_init,
         sizeof(sub_stream_parameters));
}
...
static int key_equals(const char *first_string, const char *second_string) {
  if (!first_string || !second_string) {
    platform_log_error(
        "key_equals() called with NULL parameter (first_string=%p, "
        "second_string=%p)",
...
static int is_known_section(const char *section_name) {
  ...
  for (size_t i = 0; i < sizeof(default_sections) / sizeof(default_sections[0]);
       i++) {
...
static config_section_t parse_section_name(const char *section_name) {
  if (!section_name) {
    platform_log_error(
        "parse_section_name() called with NULL section_name parameter");
 ...
 */
static config_parameter_t *find_parameter(config_manager_t *config,
                                          config_section_t section,
                                          const char *key) {
  if (!config || !key) {
    platform_log_error(
        "find_parameter() called with NULL parameter (config=%p, key=%p)",
        config, key);
    return NULL;
  }
 ...
 */
static long parse_integer_value(const char *value,
                                const config_parameter_t *param) {
  char *end_ptr = NULL;
  long int_val = strtol(value, &end_ptr, 10);

  // Check for conversion errors
  if (end_ptr == value || *end_ptr != '\0' || int_val < INT_MIN ||
      int_val > INT_MAX) {
    platform_log_warning(
        "invalid integer value '%s' for parameter '%s', using default", value,
 ...
 */
static long validate_port_parameter(long int_val,
                                    const config_parameter_t *param) {
  ...
  if (!validation_is_valid(&validation)) {
    platform_log_warning("port validation failed for %s: %s, using default",
 ...
 * @return Validated integer value
 */
static long validate_range_parameter(long int_val,
                                     const config_parameter_t *param) {
  validation_result_t validation = validate_int(
      param->key, (int)int_val, param->min_value, param->max_value);
  if (!validation_is_valid(&validation)) {
    platform_log_warning(
        "integer validation failed for %s: %s, clamping to valid range",
        param->key, validation_get_error_message(&validation));
...
static int is_port_parameter(const config_parameter_t *param) {
  return (key_equals(param->key, "http_port") ||
          key_equals(param->key, "rtsp_port") ||
          key_equals(param->key, "snapshot_port") ||
          key_equals(param->key, "ws_discovery_port"));
}
 ...
 */
static int set_int_parameter_value(config_parameter_t *param,
                                   const char *value) {
 ...
 * @return 1 on success, 0 on failure
 */
static int set_bool_parameter_value(config_parameter_t *param,
                                    const char *value) {
  int bool_val = (key_equals(value, "true") || key_equals(value, "1") ||
 ...
 * @return 1 on success, 0 on failure
 */
static int set_string_parameter_value(config_parameter_t *param,
                                      const char *value) {
  // Validate string length based on parameter type
  validation_result_t validation;
  if (key_equals(param->key, "username")) {
    validation = validate_string("username", value, MIN_USERNAME_LENGTH,
                                 MAX_USERNAME_LENGTH, 0);
  } else if (key_equals(param->key, "password")) {
    validation = validate_string("password", value, MIN_PASSWORD_LENGTH,
                                 MAX_PASSWORD_LENGTH, 0);
  } else {
    validation =
        validate_string(param->key, value, 0, param->value_size - 1, 1);
  }

  if (!validation_is_valid(&validation)) {
    platform_log_warning(
        "string validation failed for %s: %s, using truncated value",
        param->key, validation_get_error_message(&validation));
  ...
  size_t copy_len =
      (value_len < param->value_size - 1) ? value_len : param->value_size - 1;
 ...
 * @return 1 on success, 0 on failure
 */
static int set_float_parameter_value(config_parameter_t *param,
                                     const char *value) {
  char *end_ptr = NULL;
  ...
  if (end_ptr == value || *end_ptr != '\0') {
    platform_log_warning(
        "invalid float value '%s' for parameter '%s', using default", value,
        param->key ? param->key : "unknown");
    if (param->default_value) {
...
static void set_parameter_value(config_parameter_t *param, const char *value) {
  if (!param || !value) {
    platform_log_warning(
        "set_parameter_value() called with NULL parameter (param=%p, value=%p)",
        param, value);
    return;
  }
...
static void set_default_values(config_manager_t *config) {
  if (!config) {
    platform_log_error(
        "set_default_values() called with NULL config parameter");
    return;
  }
...
static void link_parameters_to_config(config_manager_t *config) {
  if (!config || !config->app_config) {
    platform_log_error(
        "link_parameters_to_config() called with NULL parameter (config=%p, "
        "app_config=%p)",
  ...
  if (config->app_config->auto_daynight) {
    ...
    auto_daynight_parameters[1].value_ptr =
        &config->app_config->auto_daynight->mode;
    ...
    auto_daynight_parameters[4].value_ptr =
        &config->app_config->auto_daynight->lock_time_seconds;
  }

  // Link network parameters
  if (config->app_config->network) {
    network_parameters[0].value_ptr = &config->app_config->network->rtsp_port;
    network_parameters[1].value_ptr =
        &config->app_config->network->snapshot_port;
    network_parameters[2].value_ptr =
        &config->app_config->network->ws_discovery_port;
  }
  ...
  if (config->app_config->device) {
    ...
    device_parameters[2].value_ptr =
        config->app_config->device->firmware_version;
  ...
  if (config->app_config->main_stream) {
    ...
    main_stream_parameters[1].value_ptr =
        &config->app_config->main_stream->bitrate;
  }
  ...
  if (config->app_config->sub_stream) {
    ...
    sub_stream_parameters[1].value_ptr =
        &config->app_config->sub_stream->bitrate;
  }
}
 ...
 */
static int validate_config_line(const char *line, int *has_valid_section,
                                int line_number, int *has_valid_key_value) {
...
static int validate_config_file_format(const char *config_file) {
  if (!config_file) {
    platform_log_error(
        "validate_config_file_format() called with NULL config_file parameter");
    return ONVIF_VALIDATION_FAILED;
  }

  FILE *file_ptr = fopen(config_file, "r");
  if (!file_ptr) {
    platform_log_error("failed to open config file '%s' for validation: %s",
  ...
  while (fgets(line, sizeof(line), file_ptr)) {
    ...
    if (!validate_config_line(line, &has_valid_section, line_number,
                              &has_valid_key_value)) {
...
static void convert_device_values_to_onvif(config_manager_t *config) {
  if (!config || !config->app_config) {
    platform_log_warning(
        "convert_device_values_to_onvif() called with NULL parameter "
  ...
  if (!config->app_config->auto_daynight) {
    platform_log_info(
        "auto_daynight config not available, skipping conversion");
    return;
  }
  ...
  if (auto_config->day_to_night_threshold > 0) {
    ...
    auto_config->day_to_night_threshold =
        (auto_config->day_to_night_threshold * 100) / 10000;
  ...
  if (auto_config->night_to_day_threshold > 0) {
    // Convert from device range (0-10000) to ONVIF range (0-100)
    auto_config->night_to_day_threshold =
        (auto_config->night_to_day_threshold * 100) / 10000;
...
int config_init(config_manager_t *config,
  ...
  if (!config || !app_config) {
    platform_log_error(
        "config_init() called with NULL parameter (config=%p, app_config=%p)",
        config, app_config);
  ...
  config->section_count =
      sizeof(default_sections) / sizeof(default_sections[0]);
 ...
 */
static int parse_section_header(const char *line, int line_number,
  ...
  if (end) {
    ...
    if (len >= MAX_SECTION_NAME_LENGTH) {
      platform_log_warning("section name too long at line %d, truncating",
                           line_number);
    ...
    *current_section_known = is_known_section(current_section);
    if (*current_section_known) {
      *current_section_type = parse_section_name(current_section);
      ...
      platform_log_info("processing unknown section '[%s]' at line %d",
                        current_section, line_number);
    }
  } else {
    platform_log_warning("malformed section header at line %d, skipping",
                         line_number);
 ...
 */
static int parse_key_value_pair(const char *line, int line_number, char *key,
                                char *value) {
  char *equals_sign = strchr(line, '=');
  if (!equals_sign) {
    platform_log_warning("malformed key=value pair at line %d, skipping",
                         line_number);
    return 0;
  }
...
static int process_config_line(const char *line, int line_number,
                               config_manager_t *config, char *current_section,
                               config_section_t *current_section_type,
                               int *current_section_known) {
  ...
  if (trimmed_line[0] == '[') {
    return parse_section_header(trimmed_line, line_number, current_section,
                                current_section_type, current_section_known);
  }
  ...
  if (*current_section_known) {
    config_parameter_t *param =
        find_parameter(config, *current_section_type, key);
    if (param) {
      set_parameter_value(param, value);
    } else {
      platform_log_info(
          "unknown parameter '%s' in section '[%s]' at line %d, skipping", key,
          current_section, line_number);
    }
  }
...
static FILE *open_config_file(const char *config_file) {
  ...
  if (!file_ptr) {
    ...
    if (!file_ptr) {
      platform_log_error(
          "could not open config file '%s': %s (tried alternate: "
...
int config_load(config_manager_t *config, const char *config_file) {
  if (!config || !config_file) {
    platform_log_error(
        "config_load() called with NULL parameter (config=%p, config_file=%p)",
        config, config_file);
    return ONVIF_ERROR_INVALID;
  }
  ...
  while (fgets(line, sizeof(line), file_ptr)) {
    ...
    process_config_line(line, line_number, config, current_section,
                        &current_section_type, &current_section_known);
  }
  ...
  for (size_t i = 0; i < config->section_count; i++) {
    for (size_t j = 0; j < config->sections[i].parameter_count; j++) {
      ...
      if (param->required && !param->value_ptr) {
        platform_log_error(
            "required parameter '%s' in section %d has no value pointer",
      ...
      if (param->type == CONFIG_TYPE_INT && param->value_ptr) {
        ...
        if (param->min_value != param->max_value &&
          ...
          platform_log_error(
              "parameter '%s' value %d is out of range [%d, %d] in section %d",
...
int config_get_value(config_manager_t *config, config_section_t section,
  ...
  if (!config || !key || !value_ptr) {
    platform_log_error(
        "config_get_value() called with NULL parameter (config=%p, key=%p, "
        "value_ptr=%p)",
  ...
  if (!param || param->type != value_type) {
    platform_log_error(
        "parameter '%s' not found or type mismatch (expected %d, got %d) in "
        "section %d",
  ...
  platform_log_error("parameter '%s' has no value pointer in section %d", key,
                     section);
  return ONVIF_ERROR_NOT_FOUND;
}
...
int config_set_value(config_manager_t *config, config_section_t section,
  ...
  if (!config || !key || !value_ptr) {
    platform_log_error(
        "config_set_value() called with NULL parameter (config=%p, key=%p, "
        "value_ptr=%p)",
        config, key, value_ptr);
    return ONVIF_ERROR_INVALID;
  }
...
int config_reset_to_defaults(config_manager_t *config) {
  if (!config) {
    platform_log_error(
        "config_reset_to_defaults() called with NULL config parameter");
    return ONVIF_ERROR_INVALID;
  }
...
const config_parameter_t *config_get_parameter(config_manager_t *config,
  ...
  if (!config || !key) {
    platform_log_error(
        "config_get_parameter() called with NULL parameter (config=%p, key=%p)",
        config, key);
    return NULL;
  }
...
int config_get_summary(config_manager_t *config, char *summary,
  ...
  if (!config || !summary) {
    platform_log_error(
        "config_get_summary() called with NULL parameter (config=%p, "
        "summary=%p)",
        config, summary);
    return ONVIF_ERROR_INVALID;
  }
  ...
  int result = snprintf(
      ...
      config->app_config->auto_daynight
          ? config->app_config->auto_daynight->enable_auto_switching
          : 0,
      config->app_config->auto_daynight
          ? config->app_config->auto_daynight->mode
  ...
  if ((size_t)result >= summary_size) {
    platform_log_error("summary buffer too small (required=%d, available=%zu)",
                       result, summary_size);
    return ONVIF_ERROR;
  }
