...
int config_lifecycle_load_configuration(struct application_config *cfg) {
  ...
  if (config_load(&g_config_global_config_manager, ONVIF_CONFIG_FILE) !=
      ONVIF_SUCCESS) {
  ...
  config_get_value(&g_config_global_config_manager, CONFIG_SECTION_MAIN_STREAM,
                   "main_fps", &config_main_fps, CONFIG_TYPE_INT);
  config_get_value(&g_config_global_config_manager, CONFIG_SECTION_MAIN_STREAM,
                   "main_kbps", &main_kbps, CONFIG_TYPE_INT);
  config_get_value(&g_config_global_config_manager, CONFIG_SECTION_SUB_STREAM,
                   "sub_fps", &config_sub_fps, CONFIG_TYPE_INT);
  config_get_value(&g_config_global_config_manager, CONFIG_SECTION_SUB_STREAM,
                   "sub_kbps", &sub_kbps, CONFIG_TYPE_INT);
  ...
  if (stream_config_init_from_anyka(cfg->main_stream, true,
                                    (unsigned int)main_kbps,
                                    main_fps) != ONVIF_SUCCESS) {
    platform_log_warning(
        "warning: failed to initialize main stream config, using defaults\n");
    stream_config_init_defaults(cfg->main_stream, true);
  }

  // Initialize sub stream configuration
  if (stream_config_init_from_anyka(cfg->sub_stream, false,
                                    (unsigned int)sub_kbps,
                                    sub_fps) != ONVIF_SUCCESS) {
    platform_log_warning(
        "warning: failed to initialize sub stream config, using defaults\n");
  ...
  if (config_get_summary(&g_config_global_config_manager, config_summary,
...
bool config_lifecycle_loaded(void) { return g_config_loaded; }
