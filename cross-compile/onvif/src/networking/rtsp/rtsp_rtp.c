...
int rtsp_setup_encoder(rtsp_server_t *server) {
  ...
  venc_config.bitrate =
      server->config.video_config.bitrate;        // bitrate is already in kbps
  venc_config.codec = PLATFORM_VIDEO_CODEC_H264;  // Default to H264

  if (platform_venc_init(&server->venc_handle, &venc_config) !=
      PLATFORM_SUCCESS) {
  ...
  platform_log_notice(
      "Video encoder created and stream started: %dx%d@%dfps, %dkbps\n",
...
int rtsp_setup_audio_encoder(rtsp_server_t *server) {
  ...
  if (ai_result != PLATFORM_SUCCESS) {
    platform_log_warning(
        "Failed to create audio input (error: %d) - continuing without audio\n",
        ai_result);
  ...
  platform_result_t aenc_result =
      platform_aenc_init(&server->aenc_handle, &aenc_config);
  if (aenc_result != PLATFORM_SUCCESS) {
    platform_log_warning(
        "Failed to create audio encoder (error: %d) - cleaning up audio "
        "input\n",
        aenc_result);
  ...
  platform_log_info(
      "Audio encoder created successfully: %dHz, %d channels, %d bits\n",
...
int rtsp_init_rtp_session(rtsp_session_t *session) {
  ...
  if (bind(session->rtp_session.rtp_sockfd, (struct sockaddr *)&rtp_addr,
           sizeof(rtp_addr)) < 0) {
  ...
  getsockname(session->rtp_session.rtp_sockfd, (struct sockaddr *)&rtp_addr,
              &addr_len);
  ...
  if (bind(session->rtp_session.rtcp_sockfd, (struct sockaddr *)&rtp_addr,
           sizeof(rtp_addr)) < 0) {
  ...
  memcpy(&session->rtp_session.client_addr, &session->addr,
         sizeof(session->addr));
  session->rtp_session.client_addr.sin_port =
      htons(session->rtp_session.rtp_port);
  ...
  platform_log_notice(
      "RTP session initialized: RTP port %d, RTCP port %d, SSRC %u\n",
...
int rtsp_init_audio_rtp_session(rtsp_session_t *session) {
  if (!session || !session->audio_enabled) return 0;
  ...
  if (bind(session->audio_rtp_session.rtp_sockfd, (struct sockaddr *)&rtp_addr,
           sizeof(rtp_addr)) < 0) {
  ...
  // Get assigned port
  socklen_t addr_len = sizeof(rtp_addr);
  getsockname(session->audio_rtp_session.rtp_sockfd,
              (struct sockaddr *)&rtp_addr, &addr_len);
  session->audio_rtp_session.rtp_port = ntohs(rtp_addr.sin_port);
  session->audio_rtp_session.rtcp_port =
      session->audio_rtp_session.rtp_port + 1;

  // Bind audio RTCP socket
  rtp_addr.sin_port = htons(session->audio_rtp_session.rtcp_port);
  if (bind(session->audio_rtp_session.rtcp_sockfd, (struct sockaddr *)&rtp_addr,
           sizeof(rtp_addr)) < 0) {
  ...
  // Set client address for UDP transmission
  memcpy(&session->audio_rtp_session.client_addr, &session->addr,
         sizeof(session->addr));
  session->audio_rtp_session.client_addr.sin_port =
      htons(session->audio_rtp_session.rtp_port);

  // Initialize audio RTCP
  if (rtcp_init_session((struct rtp_session *)&session->audio_rtp_session) <
      0) {
  ...
  platform_log_notice(
      "Audio RTP session initialized: RTP port %d, RTCP port %d, SSRC %u\n",
 ...
 * Send RTP packet
 */
int rtsp_send_rtp_packet(rtsp_session_t *session, const uint8_t *data,
...
int rtsp_send_rtp_packet_udp(rtsp_session_t *session, const uint8_t *data,
  ...
  rtp_packet[pos++] =
      (session->rtp_session.seq_num >> 8) & 0xFF;  // Sequence number
...
int rtsp_send_audio_rtp_packet(rtsp_session_t *session, const uint8_t *data,
                               size_t len, uint32_t timestamp) {
  if (!session || !data || len == 0 || !session->audio_enabled) return -1;
  ...
  rtp_packet[pos++] =
      (session->audio_rtp_session.sequence >> 8) & 0xFF;  // Sequence number
  ...
  // Send packet
  ssize_t sent =
      sendto(session->audio_rtp_session.rtp_sockfd, rtp_packet, pos, 0,
