...
int sdp_generate(struct sdp_session *sdp, char *buffer, size_t buffer_size) {
    ...
    while (media) {
        ...
        switch (media->type) {
            case SDP_MEDIA_VIDEO: media_type_str = "video"; break;
            case SDP_MEDIA_AUDIO: media_type_str = "audio"; break;
            case SDP_MEDIA_APPLICATION: media_type_str = "application"; break;
            default: media_type_str = "video"; break;
        }
        
        len += snprintf(buffer + len, buffer_size - len, "m=%s %d %s %d\r\n", 
                       media_type_str, media->port, media->protocol, media->payload_type);
        ...
        switch (media->direction) {
            case SDP_DIR_SENDRECV: direction_str = "sendrecv"; break;
            case SDP_DIR_SENDONLY: direction_str = "sendonly"; break;
            case SDP_DIR_RECVONLY: direction_str = "recvonly"; break;
            case SDP_DIR_INACTIVE: direction_str = "inactive"; break;
            default: direction_str = "sendrecv"; break;
        }
        len += snprintf(buffer + len, buffer_size - len, "a=%s\r\n", direction_str);
    ...
    while (*line_start) {
        ...
        if (line[0] == 'v' && line[1] == '=') {
            ...
            if (sscanf(line + 2, "%15s %d %15s %d", media_type, &port, protocol, &payload_type) == 4) {
                ...
                if (strcmp(media_type, "audio") == 0) type = SDP_MEDIA_AUDIO;
                else if (strcmp(media_type, "application") == 0) type = SDP_MEDIA_APPLICATION;
