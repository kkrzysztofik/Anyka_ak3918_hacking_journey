 ...
 * 
 * This file contains all session management related functions for the RTSP server,
 * including session creation, cleanup, timeout handling, and statistics.
 */
...
enum rtsp_session_state rtsp_session_get_state(rtsp_session_t *session) {
    if (!session) return RTSP_STATE_INVALID;
...
int rtsp_session_get_info(rtsp_session_t *session, char *buffer, size_t buffer_size) {
    if (!session || !buffer || buffer_size < 64) return -1;
    ...
    switch (session->state) {
        case RTSP_STATE_INIT: state_str = "INIT"; break;
        case RTSP_STATE_READY: state_str = "READY"; break;
        case RTSP_STATE_PLAYING: state_str = "PLAYING"; break;
        case RTSP_STATE_RECORDING: state_str = "RECORDING"; break;
        default: state_str = "UNKNOWN"; break;
    }
    
    return snprintf(buffer, buffer_size, 
                   "Session %s: %s:%d, State: %s, Age: %lds, Idle: %lds, Packets: %u, Bytes: %lu",
                   session->session_id, client_ip, ntohs(session->addr.sin_port),
                   state_str, rtsp_session_get_age(session), rtsp_session_get_idle_time(session),
