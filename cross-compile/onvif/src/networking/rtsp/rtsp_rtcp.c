...
int rtcp_send_sr(struct rtp_session *rtp_session) {
  if (!rtp_session || !rtp_session->rtcp_enabled) return -1;
  ...
  if (rtp_session->rtcp_sockfd >= 0) {
    sendto(rtp_session->rtcp_sockfd, packet, pos, 0,
           (struct sockaddr *)&rtp_session->client_rtcp_addr,
           sizeof(rtp_session->client_rtcp_addr));
  }
...
int rtcp_handle_packet(struct rtp_session *rtp_session, const uint8_t *data,
                       size_t len) {
  if (!rtp_session || !data || len < 4) return -1;

  // Parse RTCP header
  uint8_t version = (data[0] >> 6) & 0x03;
  uint8_t pt = data[1];

  if (version != 2) return -1;  // Invalid version
...
void *rtcp_thread(void *arg) {
  ...
  while (rtp_session->rtcp_enabled) {
    ...
    if (rtp_session->rtcp_sockfd >= 0) {
      ...
      int result =
          select(rtp_session->rtcp_sockfd + 1, &readfds, NULL, NULL, &timeout);
      if (result > 0 && FD_ISSET(rtp_session->rtcp_sockfd, &readfds)) {
        ssize_t bytes =
            recvfrom(rtp_session->rtcp_sockfd, buffer, sizeof(buffer), 0,
