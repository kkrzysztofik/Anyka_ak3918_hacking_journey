...
static int copy_binary_data(const char *src, size_t src_len, char **dest, size_t *dest_len) {
  if (!src || src_len == 0) {
    *dest = NULL;
    *dest_len = 0;
    return 0;
...
int http_to_onvif_request(const http_request_t *http_req, onvif_request_t *onvif_req) {
  ...
  // Copy body data
  if (copy_binary_data(http_req->body, http_req->body_length, 
                       &onvif_req->body, &onvif_req->body_length) != 0) {
    return -1;
  }
  
  // Copy headers data
  if (copy_string_data(http_req->headers, &onvif_req->headers, &onvif_req->headers_length) != 0) {
...
int onvif_to_http_response(const onvif_response_t *onvif_resp, http_response_t *http_resp) {
  ...
  // Copy content type
  if (copy_string_data(onvif_resp->content_type, &http_resp->content_type, NULL) != 0) {
