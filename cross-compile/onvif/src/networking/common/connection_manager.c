...
uint64_t get_time_ms(void) {
  ...
  return (uint64_t)time_value.tv_sec * 1000 +
         (uint64_t)time_value.tv_usec / 1000;
}
 ...
 */
connection_t *connection_get_list_head(void) { return g_connections; }

/**
 * @brief Lock the connections mutex
 */
void connection_lock_mutex(void) { pthread_mutex_lock(&g_connections_mutex); }
