...
int security_add_security_headers(http_response_t *response,
                                  security_context_t *context);
...
#define SECURITY_VALIDATE_XML(xml, len) \
  security_validate_xml_structure((xml), (len), NULL)

#define SECURITY_LOG_ATTACK(type, ip, details) \
  security_log_attack((type), (ip), (details))
