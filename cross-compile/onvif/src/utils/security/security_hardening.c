...
#include "platform/platform.h" // NOLINT
...
int security_init(security_level_t level) {
  ...
  platform_log_info("Security system initialized with level %d\n", level);
...
void security_cleanup(void) {
  ...
  platform_log_info("Security system cleaned up\n");
}
...
int security_validate_http_headers(const http_request_t* request, security_context_t* context) {
  ...
  if (request->headers && request->header_count > 0) {
    for (size_t i = 0; i < request->header_count; i++) {
      ...
      if (strstr(header_value, "<script") != NULL || strstr(header_value, "javascript:") != NULL || strstr(header_value, "vbscript:") != NULL) {
        platform_log_error("XSS attempt detected in header '%s': %s\n", request->headers[i].name ? request->headers[i].name : "unknown",
                           header_value);
        return ONVIF_ERROR;
      }

      // Check for SQL injection attempts in header values
      if (strstr(header_value, "'; DROP") != NULL || strstr(header_value, "UNION SELECT") != NULL || strstr(header_value, "OR 1=1") != NULL) {
        platform_log_error("SQL injection attempt detected in header '%s': %s\n", request->headers[i].name ? request->headers[i].name : "unknown",
                           header_value);
        return ONVIF_ERROR;
      }
    }
...
int security_validate_xml_structure(const char* xml, size_t length, security_context_t* context) {
  (void)context; // Reserved for future security context tracking
  ONVIF_VALIDATE_NULL(xml, "xml");

  if (length == 0 || length > MAX_INPUT_LENGTH) {
    platform_log_error("Invalid XML length: %zu\n", length);
    return ONVIF_ERROR;
  }

  // Log XML content for debugging when validation fails
  platform_log_debug("Validating XML structure (length=%zu): %.*s\n", length, (int)length, xml);

  // Check for XML bomb attacks
  if (security_detect_xml_bomb(xml, length) != ONVIF_SUCCESS) {
    platform_log_error("XML bomb attack detected\n");
    platform_log_debug("XML bomb content: %.*s\n", (int)length, xml);
    return ONVIF_ERROR;
  }

  // Check for XXE attacks
  if (security_detect_xxe_attack(xml, length) != ONVIF_SUCCESS) {
    platform_log_error("XXE attack detected\n");
    platform_log_debug("XXE attack content: %.*s\n", (int)length, xml);
    return ONVIF_ERROR;
  }
  ...
  for (size_t i = 0; i < length; i++) {
    if (xml[i] == '<') {
      ...
      if (depth > MAX_XML_DEPTH) {
        platform_log_error("XML depth too deep: %d\n", depth);
        platform_log_debug("XML depth error content: %.*s\n", (int)length, xml);
        return ONVIF_ERROR;
      }
    } else if (xml[i] == '>') {
      depth--;
      ...
      if (attribute_count > MAX_XML_ATTRIBUTES) {
        platform_log_error("Too many XML attributes: %d\n", attribute_count);
        platform_log_debug("XML attributes error content: %.*s\n", (int)length, xml);
        return ONVIF_ERROR;
      }
    }
  }
...
int security_sanitize_input(const char* input, char* output, size_t output_size, security_context_t* context) {
  ...
  if (input_len >= output_size) {
    platform_log_error("Input too long for sanitization: %zu >= %zu\n", input_len, output_size);
    return ONVIF_ERROR;
  }
  ...
  for (int i = 0; i < g_rate_limit_count; i++) {
    if (strcmp(g_rate_limits[i].client_ip, client_ip) == 0) {
      ...
      if (g_rate_limits[i].request_count >= g_max_requests_per_minute) {
        ...
        platform_log_error("Rate limit exceeded for client: %s\n", client_ip);
        return ONVIF_ERROR;
      }
...
int security_detect_sql_injection(const char* input) {
  ...
  const char* patterns[] = {"'; DROP", "UNION SELECT", "OR 1=1", "AND 1=1", "EXEC(", "EXECUTE(", "sp_", "xp_", NULL};

  for (int i = 0; patterns[i] != NULL; i++) {
    if (strstr(input, patterns[i]) != NULL) {
      platform_log_error("SQL injection pattern detected: %s\n", patterns[i]);
      return ONVIF_ERROR;
    }
  }
...
int security_detect_xss_attack(const char* input) {
  ...
  const char* patterns[] = {"<script", "javascript:", "vbscript:", "onload=", "onerror=", "onclick=", "eval(", "document.cookie", NULL};

  for (int i = 0; patterns[i] != NULL; i++) {
    if (strstr(input, patterns[i]) != NULL) {
      platform_log_error("XSS pattern detected: %s\n", patterns[i]);
      return ONVIF_ERROR;
    }
  }
...
int security_detect_path_traversal(const char* input) {
  ...
  const char* patterns[] = {"../", "..\\", "/etc/passwd", "/etc/shadow", "C:\\", "..%2f", "..%5c", NULL};

  for (int i = 0; patterns[i] != NULL; i++) {
    if (strstr(input, patterns[i]) != NULL) {
      platform_log_error("Path traversal pattern detected: %s\n", patterns[i]);
      return ONVIF_ERROR;
    }
  }
...
int security_detect_xml_bomb(const char* xml, size_t length) {
  (void)length; // Not used - pattern-based detection
  ONVIF_VALIDATE_NULL(xml, "xml");

  // Check for exponential entity expansion
  const char* bomb_patterns[] = {"&lol9;", "&lol8;", "&lol7;", "&lol6;", "&lol5;", "&lol4;", "&lol3;", "&lol2;", "&lol1;", "&lol0;", NULL};

  for (int i = 0; bomb_patterns[i] != NULL; i++) {
    if (strstr(xml, bomb_patterns[i]) != NULL) {
      platform_log_error("XML bomb pattern detected: %s\n", bomb_patterns[i]);
      return ONVIF_ERROR;
    }
  }
...
int security_detect_xxe_attack(const char* xml, size_t length) {
  ...
  const char* xxe_patterns[] = {"DOCTYPE", "SYSTEM", "PUBLIC", "file://", "ftp://", "gopher://", NULL};

  for (int i = 0; xxe_patterns[i] != NULL; i++) {
    if (strstr(xml, xxe_patterns[i]) != NULL) {
      platform_log_error("XXE pattern detected: %s\n", xxe_patterns[i]);
      return ONVIF_ERROR;
    }
  }
...
void security_log_attack(const char* attack_type, const char* client_ip, const char* details) {
  platform_log_error("SECURITY ALERT: %s attack from %s - %s\n", attack_type ? attack_type : "Unknown", client_ip ? client_ip : "Unknown",
...
void security_log_security_event(const char* event_type, const char* client_ip, int severity) {
  ...
  platform_log_error("SECURITY EVENT [%s]: %s from %s\n", severity_str, event_type ? event_type : "Unknown", client_ip ? client_ip : "Unknown");
}
...
int security_is_private_ip(const char* ip_address) {
  ...
  if (strncmp(ip_address, "172.", 4) == 0) {
    ...
    if (end_ptr != ip_address + 4 && second_octet >= IP_RFC1918_172_MIN && second_octet <= IP_RFC1918_172_MAX) {
      return 1;
    }
  }
...
int security_add_security_headers(http_response_t* response, security_context_t* context) {
  ...
  if (http_response_add_header(response, "X-Content-Type-Options", "nosniff") != 0) {
    platform_log_error("Failed to add X-Content-Type-Options header\n");
    return ONVIF_ERROR;
  }

  // X-Frame-Options: DENY
  if (http_response_add_header(response, "X-Frame-Options", "DENY") != 0) {
    platform_log_error("Failed to add X-Frame-Options header\n");
    return ONVIF_ERROR;
  }

  // X-XSS-Protection: 1; mode=block
  if (http_response_add_header(response, "X-XSS-Protection", "1; mode=block") != 0) {
    platform_log_error("Failed to add X-XSS-Protection header\n");
    return ONVIF_ERROR;
  }

  // Content-Security-Policy: default-src 'none'
  if (http_response_add_header(response, "Content-Security-Policy", "default-src 'none'") != 0) {
    platform_log_error("Failed to add Content-Security-Policy header\n");
    return ONVIF_ERROR;
  }

  // Referrer-Policy: no-referrer
  if (http_response_add_header(response, "Referrer-Policy", "no-referrer") != 0) {
    platform_log_error("Failed to add Referrer-Policy header\n");
    return ONVIF_ERROR;
  }

  platform_log_debug("Security headers added successfully\n");
  return ONVIF_SUCCESS;
}
...
int security_validate_request(const http_request_t* request, security_context_t* context) {
  ...
  if (security_is_client_blocked(context->client_ip, context)) {
    platform_log_error("Blocked client %s attempted request\n", context->client_ip);
  ...
  if (security_validate_http_headers(request, context) != ONVIF_SUCCESS) {
    platform_log_error("HTTP headers validation failed for client %s\n", context->client_ip);
...
int security_validate_request_body(const http_request_t* request, security_context_t* context) {
  ...
  if (security_validate_xml_structure(request->body, request->body_length, context) != ONVIF_SUCCESS) {
    platform_log_error("XML security validation failed for client %s\n", context->client_ip);
  ...
  if (security_detect_sql_injection(request->body) != ONVIF_SUCCESS) {
    platform_log_error("SQL injection detected in XML body from client %s\n", context->client_ip);
  ...
  if (security_detect_xss_attack(request->body) != ONVIF_SUCCESS) {
    platform_log_error("XSS attack detected in XML body from client %s\n", context->client_ip);
