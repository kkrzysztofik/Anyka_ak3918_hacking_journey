...
int common_service_init_handler(common_service_context_t *context,
  ...
  int result =
      onvif_service_handler_init(handler, &context->handler_config,
  ...
  if (result == ONVIF_SUCCESS) {
    ...
    service_log_init_context(&log_ctx, context->service_name, "init",
                             SERVICE_LOG_INFO);
    ...
    service_log_init_context(&log_ctx, context->service_name, "init",
                             SERVICE_LOG_ERROR);
    service_log_operation_failure(&log_ctx, "Service initialization", result,
                                  "Handler init failed");
  }
...
int common_service_register_error_handlers(       // NOLINT
    common_service_context_t *context,            // NOLINT
    error_handler_callback_t validation_handler,  // NOLINT
    error_handler_callback_t system_handler,      // NOLINT
    error_handler_callback_t config_handler) {    // NOLINT
  if (!context) {
    return ONVIF_ERROR_INVALID;
  }
  ...
  // Note: This is a simplified implementation since error_register_handler
  // doesn't exist In a full implementation, these would be stored and used when
  // errors occur
  ...
  if (result == ONVIF_SUCCESS) {
    service_log_context_t log_ctx;
    service_log_init_context(&log_ctx, context->service_name, "error_handlers",
                             SERVICE_LOG_INFO);
...
int common_service_handle_request(common_service_context_t *context,
  ...
  if (!context->initialized) {
    service_log_context_t log_ctx;
    service_log_init_context(&log_ctx, context->service_name, "handle_request",
                             SERVICE_LOG_ERROR);
    service_log_operation_failure(&log_ctx, "Request handling", ONVIF_ERROR,
                                  "Service not initialized");
    return ONVIF_ERROR;
  }

  return onvif_service_handler_handle_request(handler, action, request,
                                              response);
}
...
void common_service_cleanup(common_service_context_t *context,
  ...
  if (handler && context->initialized) {
    ...
    service_log_context_t log_ctx;
    service_log_init_context(&log_ctx, context->service_name, "cleanup",
                             SERVICE_LOG_INFO);
