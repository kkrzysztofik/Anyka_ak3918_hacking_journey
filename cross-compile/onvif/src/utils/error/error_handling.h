...
#define ONVIF_VALIDATE_STRING(str, field_name, min_len, max_len) \
    do { \
        ...
        if (len < (min_len) || len > (max_len)) { \
            platform_log_error("Validation failed: %s length %zu not in range [%zu, %zu] at %s:%d\n", \
...
/* Logging macros */
#define ONVIF_LOG_ERROR(fmt, ...) \
    platform_log_error("[ERROR] " fmt, ##__VA_ARGS__)

#define ONVIF_LOG_WARNING(fmt, ...) \
    platform_log_warning("[WARNING] " fmt, ##__VA_ARGS__)

#define ONVIF_LOG_INFO(fmt, ...) \
    platform_log_info("[INFO] " fmt, ##__VA_ARGS__)

#define ONVIF_LOG_DEBUG(fmt, ...) \
    platform_log_debug("[DEBUG] " fmt, ##__VA_ARGS__)
