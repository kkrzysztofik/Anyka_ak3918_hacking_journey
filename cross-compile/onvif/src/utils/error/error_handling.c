...
static const error_pattern_def_t error_patterns[] = {
    {ERROR_PATTERN_VALIDATION_FAILED, "Validation failed", SOAP_FAULT_SENDER,
     "Validation failed"},
    {ERROR_PATTERN_NOT_FOUND, "Resource not found", SOAP_FAULT_SENDER,
     "Resource not found"},
    ...
    {ERROR_PATTERN_INVALID_PARAMETER, "Invalid parameter", SOAP_FAULT_SENDER,
     "Invalid parameter"},
    {ERROR_PATTERN_MISSING_PARAMETER, "Missing required parameter",
     SOAP_FAULT_SENDER, "Missing required parameter"},
    {ERROR_PATTERN_AUTHENTICATION_FAILED, "Authentication failed",
     SOAP_FAULT_SENDER, "Authentication failed"},
    {ERROR_PATTERN_AUTHORIZATION_FAILED, "Authorization failed",
     SOAP_FAULT_SENDER, "Authorization failed"}};

static const error_pattern_def_t *find_error_pattern(error_pattern_t pattern) {
  for (size_t i = 0; i < sizeof(error_patterns) / sizeof(error_patterns[0]);
       i++) {
...
int error_create_result_from_pattern(error_pattern_t pattern,
...
int error_handle_pattern(const error_context_t *context,
                         error_pattern_t pattern, const char *custom_message,
  ...
  // Create error result
  error_result_t result;
  if (error_create_result_from_pattern(pattern, custom_message, &result) !=
      ONVIF_SUCCESS) {
    return ONVIF_ERROR;
  }
...
int error_handle_validation(const error_context_t *context,
                            int validation_result, const char *field_name,
                            onvif_response_t *response) {
  if (!context || !response) {
    return ONVIF_ERROR_INVALID;
  }
  ...
  (void)snprintf(custom_message, sizeof(custom_message),
                 "Validation failed for field '%s' (code: %d)",
                 field_name ? field_name : "unknown", validation_result);

  return error_handle_pattern(context, ERROR_PATTERN_VALIDATION_FAILED,
                              custom_message, response);
}

int error_handle_parameter(const error_context_t *context,
                           const char *parameter_name, const char *error_type,
                           onvif_response_t *response) {
  if (!context || !response) {
    return ONVIF_ERROR_INVALID;
  }
  ...
  char custom_message[256];
  (void)snprintf(custom_message, sizeof(custom_message),
                 "Parameter error: %s for parameter '%s'",
...
int error_handle_service(const error_context_t *context, int error_code,
  ...
  char custom_message[256];
  (void)snprintf(custom_message, sizeof(custom_message), "Service error %d: %s",
                 error_code,
                 error_message ? error_message : "Unknown service error");

  return error_handle_pattern(context, ERROR_PATTERN_INTERNAL_ERROR,
                              custom_message, response);
}

int error_handle_system(const error_context_t *context, int error_code,
  ...
  char custom_message[256];
  (void)snprintf(custom_message, sizeof(custom_message),
                 "System error %d during %s", error_code,
...
void onvif_log_error_with_context(int error_code,
                                  ...
                                  const char *file, int line,  // NOLINT
...
int onvif_get_error_context_string(const error_context_t *ctx, char *buffer,
                                   size_t buffer_size) {
  if (!ctx || !buffer || buffer_size == 0) {
    return -1;
  }

  int len = snprintf(buffer, buffer_size, "ERROR [%d] in %s() at %s:%d",
  ...
  if (ctx->message[0] != '\0') {
    int msg_len =
        snprintf(buffer + len, buffer_size - len, " - %s", ctx->message);
  ...
  if (ctx->context[0] != '\0') {
    int ctx_len =
        snprintf(buffer + len, buffer_size - len, " [%s]", ctx->context);
...
int error_should_log(const error_context_t *context,
...
int error_create_summary(const error_context_t *context,
                         const error_result_t *result, char *summary,
                         size_t summary_size) {
  ...
  int ret = snprintf(summary, summary_size, "[%s::%s] %s (Code: %d, SOAP: %s)",
                     ...
                     context->action_name ? context->action_name : "Unknown",
                     result->error_message, result->error_code,
                     result->soap_fault_code);

  if (context->error_context) {
    (void)snprintf(summary + strlen(summary), summary_size - strlen(summary),
...
void error_log_with_context(const error_context_t *context,
                            const error_result_t *result,
  ...
  char summary[512];
  if (error_create_summary(context, result, summary, sizeof(summary)) ==
      ONVIF_SUCCESS) {
...
int onvif_standardized_operation(const char *operation_name,
...
void onvif_standardized_log_error(const char *function, const char *file,
