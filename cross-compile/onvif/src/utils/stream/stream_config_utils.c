...
int stream_config_init_defaults(video_config_t *stream_config,
  ...
  if (!stream_config) {
    platform_log_error(
        "error: stream_config_init_defaults() called with NULL "
        "stream_config\n");
...
int stream_config_init_from_anyka(video_config_t *stream_config,
  ...
  if (!stream_config) {
    platform_log_error(
        "error: stream_config_init_from_anyka() called with NULL "
        "stream_config\n");
    return ONVIF_ERROR_INVALID;
  }
...
int stream_config_validate(const video_config_t *stream_config,
  ...
  if (!stream_config) {
    platform_log_error(
        "error: stream_config_validate() called with NULL stream_config\n");
    return ONVIF_ERROR_INVALID;
  }
  ...
  if (stream_config->fps < MIN_FPS || stream_config->fps > MAX_FPS) {
    platform_log_error("error: invalid FPS %d, must be between %d and %d\n",
                       stream_config->fps, MIN_FPS, MAX_FPS);
    return ONVIF_ERROR_INVALID;
  }

  // Validate bitrate based on stream type
  int max_bitrate = is_main_stream ? MAX_MAIN_BITRATE : MAX_SUB_BITRATE;
  if (stream_config->bitrate < MIN_BITRATE ||
      stream_config->bitrate > max_bitrate) {
    platform_log_error(
        "error: invalid bitrate %d, must be between %d and %d for %s stream\n",
  ...
  if (stream_config->width < MIN_WIDTH || stream_config->width > MAX_WIDTH) {
    platform_log_error("error: invalid width %d, must be between %d and %d\n",
                       stream_config->width, MIN_WIDTH, MAX_WIDTH);
    return ONVIF_ERROR_INVALID;
  }

  if (stream_config->height < MIN_HEIGHT ||
      stream_config->height > MAX_HEIGHT) {
  ...
  // Validate GOP size
  if (stream_config->gop_size < MIN_GOP_SIZE ||
      stream_config->gop_size > MAX_GOP_SIZE) {
    platform_log_error(
        "error: invalid GOP size %d, must be between %d and %d\n",
...
int stream_config_to_platform(const video_config_t *stream_config,
  ...
  if (!stream_config || !platform_config) {
    platform_log_error(
        "error: stream_config_to_platform() called with NULL parameter\n");
    return ONVIF_ERROR_INVALID;
  }
...
int stream_config_get_summary(const video_config_t *stream_config,
  ...
  if (!stream_config || !summary || summary_size == 0) {
    platform_log_error(
        "error: stream_config_get_summary() called with NULL parameter\n");
    return ONVIF_ERROR_INVALID;
  }
  ...
  int written = snprintf(
      summary, summary_size, "%s stream (%s): %dx%d@%dfps, %dkbps, GOP=%d",
  ...
  if (written < 0 || (size_t)written >= summary_size) {
    platform_log_error(
        "error: insufficient buffer size for stream config summary\n");
    return ONVIF_ERROR_INVALID;
  }
...
int stream_config_apply_to_rtsp(const video_config_t *stream_config,
  ...
  if (!stream_config || !rtsp_config) {
    platform_log_error(
        "error: stream_config_apply_to_rtsp() called with NULL parameter\n");
    return ONVIF_ERROR_INVALID;
  }
...
int stream_config_copy(video_config_t *dest, const video_config_t *src) {
  if (!dest || !src) {
    platform_log_error(
        "error: stream_config_copy() called with NULL parameter\n");
    return ONVIF_ERROR_INVALID;
  }
...
bool stream_config_equals(const video_config_t *config1,
  ...
  return (
      config1->fps == config2->fps && config1->bitrate == config2->bitrate &&
      config1->width == config2->width && config1->height == config2->height &&
      config1->gop_size == config2->gop_size &&
      config1->profile == config2->profile &&
      config1->codec_type == config2->codec_type &&
      config1->br_mode == config2->br_mode);
}
