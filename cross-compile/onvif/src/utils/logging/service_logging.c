...
void service_log_operation_success(const service_log_context_t* context, const char* operation) {
  ...
  if (context->action_name) {
    (void)snprintf(message, sizeof(message), "[%s:%s] %s completed successfully",
...
void service_log_operation_failure(const service_log_context_t* context, const char* operation,
  ...
  if (context->action_name) {
    (void)snprintf(message, sizeof(message), "[%s:%s] %s failed (code: %d): %s",
                   context->service_name, context->action_name, operation, error_code,
                   error_message ? error_message : "Unknown error");
  } else {
    (void)snprintf(message, sizeof(message), "[%s] %s failed (code: %d): %s", context->service_name,
                   operation, error_code, error_message ? error_message : "Unknown error");
  }
...
void service_log_validation_error(const service_log_context_t* context, const char* field_name,
  ...
  char message[512];
  if (context->action_name) {
    (void)snprintf(message, sizeof(message), "[%s:%s] Validation failed for field '%s' (value: %s)",
                   context->service_name, context->action_name, field_name, value ? value : "NULL");
  } else {
    (void)snprintf(message, sizeof(message), "[%s] Validation failed for field '%s' (value: %s)",
...
void service_log_config_error(const service_log_context_t* context, const char* config_key,
  ...
  char message[512];
  if (context->action_name) {
    (void)snprintf(message, sizeof(message), "[%s:%s] Configuration error for key '%s': %s",
                   context->service_name, context->action_name, config_key,
                   error_message ? error_message : "Unknown error");
  } else {
    (void)snprintf(message, sizeof(message), "[%s] Configuration error for key '%s': %s",
...
void service_log_platform_error(const service_log_context_t* context,
  ...
  char message[512];
  if (context->action_name) {
    (void)snprintf(message, sizeof(message), "[%s:%s] Platform operation '%s' failed (code: %d)",
                   context->service_name, context->action_name, platform_operation, error_code);
  } else {
    (void)snprintf(message, sizeof(message), "[%s] Platform operation '%s' failed (code: %d)",
...
void service_log_not_implemented(const service_log_context_t* context, const char* feature) {
  ...
  char message[512];
  if (context->action_name) {
    (void)snprintf(message, sizeof(message), "[%s:%s] Feature '%s' not implemented",
                   context->service_name, context->action_name, feature);
  } else {
    (void)snprintf(message, sizeof(message), "[%s] Feature '%s' not implemented",
                   context->service_name, feature);
  }
...
void service_log_timeout(const service_log_context_t* context, const char* operation,
  ...
  char message[512];
  if (context->action_name) {
    memory_safe_snprintf(message, sizeof(message), "[%s:%s] %s timed out after %dms",
                         context->service_name, context->action_name, operation, timeout_ms);
  } else {
    memory_safe_snprintf(message, sizeof(message), "[%s] %s timed out after %dms",
                         context->service_name, operation, timeout_ms);
  }
...
void service_log_warning(const service_log_context_t* context, const char* format, ...) {
  ...
  if (context->action_name) {
    memory_safe_snprintf(message, sizeof(message), "[%s:%s] %s", context->service_name,
                         context->action_name, formatted);
  } else {
