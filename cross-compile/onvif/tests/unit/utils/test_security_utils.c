...
static void test_xml_sanitization(void** state) {
    ...
    char xml2[] = "<?xml version=\"1.0\"?><!DOCTYPE foo [<!ENTITY xxe SYSTEM \"file:///etc/passwd\">]><root>&xxe;</root>";
...
static void test_base64_encode(void** state) {
    ...
    int result = onvif_base64_encode((const unsigned char*)input1, strlen(input1),
    ...
    // Test empty input
    result = onvif_base64_encode((const unsigned char*)"", 0,
                               output, sizeof(output), &output_len);
    assert_int_equal(result, ONVIF_SUCCESS);
    ...
    result = onvif_base64_encode((const unsigned char*)input1, strlen(input1),
    ...
    char small_output[4];
    result = onvif_base64_encode((const unsigned char*)input1, strlen(input1),
                               small_output, sizeof(small_output), &output_len);
...
static void test_base64_decode(void** state) {
    ...
    int result = onvif_base64_decode(input1, strlen(input1),
                                   output, sizeof(output), &output_len);
    assert_int_equal(result, ONVIF_SUCCESS);
    ...
    result = onvif_base64_decode(invalid_input, strlen(invalid_input),
                               output, sizeof(output), &output_len);
    assert_int_equal(result, ONVIF_ERROR_INVALID);
    ...
    // Test NULL output buffer
    result = onvif_base64_decode(input1, strlen(input1),
                               NULL, sizeof(output), &output_len);
    assert_int_equal(result, ONVIF_ERROR_INVALID);
}
...
static void test_base64_roundtrip(void** state) {
    ...
    // Encode
    int result = onvif_base64_encode((const unsigned char*)original, strlen(original),
                                   encoded, sizeof(encoded), &encoded_len);
    assert_int_equal(result, ONVIF_SUCCESS);

    // Decode
    result = onvif_base64_decode(encoded, encoded_len,
