...
static void test_unit_hash_sha256_empty(void** state) {
   ...
   */
  const uint8_t expected[ONVIF_SHA256_DIGEST_SIZE] = {
...
static void test_unit_hash_sha256_to_hex(void** state) {
  ...
  const uint8_t digest[ONVIF_SHA256_DIGEST_SIZE] = {
    0xba, 0x78, 0x16, 0xbf, 0x8f, 0x01, 0xcf, 0xea, 0x41, 0x41, 0x40, 0xde, 0x5d, 0xae, 0x22, 0x23,
    0xb0, 0x03, 0x61, 0xa3, 0x96, 0x17, 0x7a, 0x9c, 0xb4, 0x10, 0xff, 0x61, 0xf2, 0x00, 0x15, 0xad};
...
static void test_unit_hash_sha256_compute_hex(void** state) {
  ...
  char hex_output[ONVIF_SHA256_HEX_SIZE];
  int result =
    onvif_sha256_compute_hex((const uint8_t*)input, strlen(input), hex_output, sizeof(hex_output));
  assert_int_equal(result, ONVIF_SUCCESS);
...
const struct CMUnitTest* get_hash_utils_unit_tests(size_t* count) {
  static const struct CMUnitTest tests[] = {
    ...
    cmocka_unit_test(test_unit_hash_password_hashing),
    cmocka_unit_test(test_unit_hash_password_verification),
