...
static void test_integration_config_lifecycle_full(void** state)
{
    ...
    /* Step 2: Bootstrap runtime manager */
    ...
    result = config_runtime_get_string(CONFIG_SECTION_NETWORK, "ip_address",
    ...
    result = config_runtime_get_string(CONFIG_SECTION_DEVICE, "name",
                                      value_string, sizeof(value_string));
    if (result == ONVIF_SUCCESS) {
    ...
    /* Step 9: Verify generation counter incremented */
