...
pub(crate) fn ptz_turn_internal(
    ...
    // SAFETY: `PtzDirection::to_direction_code()` returns values 0–3, which are exactly
    // the discriminants of the `ptz_turn_direction` C enum generated by bindgen.
    // The enum is `#[repr(u32)]`, and the Rust enum is exhaustive, so every
    // `direction` value maps to a valid `ptz_turn_direction` discriminant.
...
pub(crate) fn ptz_get_step_pos_internal(
    ...
    // SAFETY: `PtzMotor::to_device_id()` returns 0 or 1, which are exactly the
    // discriminants of the `ptz_device` C enum generated by bindgen.
...
pub(crate) fn ptz_stop_internal(
    ...
    // SAFETY: `PtzDirection::to_direction_code()` returns values 0–3, matching
    // the `ptz_turn_direction` C enum discriminants (see ptz_turn_internal comment).
    let sdk_direction: ptz_turn_direction =
        unsafe { std::mem::transmute(direction.to_direction_code()) };
