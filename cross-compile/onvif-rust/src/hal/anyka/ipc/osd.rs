...
impl AnykaIpc {
    ...
    pub async fn osd_init(&self, vi_handle: u64) -> PlatformResult<[ChannelDims; 2]> {
        let req_data = vi_handle.to_le_bytes().to_vec();
        let (status, resp) = self
            .request_async(CMD_OSD_INIT, &req_data)
            .await
            .map_err(|e| {
        ...
        // Length checked above; index reads are infallible for off in {0,4,8,12}.
        let read_i32 = |off: usize| -> i32 {
            i32::from_le_bytes([resp[off], resp[off + 1], resp[off + 2], resp[off + 3]])
        };
        Ok([
            ChannelDims {
                width: read_i32(0),
    ...
    pub async fn osd_set_rect(
        ...
        let (x, y, w, h) = geom;
        let mut req_data = vi_handle.to_le_bytes().to_vec();
        for v in [channel, rect, x, y, w, h] {
            req_data.extend_from_slice(&v.to_le_bytes());
        }
        let (status, _) = self
            .request_async(CMD_OSD_SET_RECT, &req_data)
            .await
            .map_err(|e| {
    ...
    pub async fn osd_draw_str(
        ...
        let (status, _) = self
            .request_async(CMD_OSD_DRAW_STR, &req_data)
            .await
            .map_err(|e| {
    ...
    pub async fn osd_set_enable(
        ...
        let (status, _) = self
            .request_async(CMD_OSD_SET_ENABLE, &req_data)
            .await
            .map_err(|e| {
    ...
    pub async fn osd_set_style(
        ...
        let (status, _) = self
            .request_async(CMD_OSD_SET_STYLE, &req_data)
            .await
            .map_err(|e| {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_osd_init_returns_channel_dims_from_daemon() {
        ...
        let dims = ipc.osd_init(0x1234).await.unwrap();
        assert_eq!(dims[0].width, 1280);
        assert_eq!(dims[1].height, 360);
    }

    #[tokio::test]
    async fn test_osd_draw_str_encodes_glyphs_little_endian() {
        ...
        ipc.osd_draw_str(0, 1, 16, 0, &[0x41, 0x42]).await.unwrap();
    ...
    #[tokio::test]
    async fn test_osd_set_style_rejects_out_of_range_alpha_before_ipc() {
        let daemon = FakeDaemon::start(|_cmd, _req| (AK_SUCCESS_I32, vec![]));
        let ipc = AnykaIpc::new_with_path(&daemon.socket_path).unwrap();
        ipc.set_epochs_for_test(1, 1);

        assert!(ipc.osd_set_style(1, 0, 0, 0).await.is_err());
        assert!(ipc.osd_set_style(1, 0, 0, 101).await.is_err());
    }
}
