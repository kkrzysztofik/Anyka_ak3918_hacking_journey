...
use crate::platform::VideoControl;
use std::ffi::c_void;
...
#[test]
fn test_apply_flip_mirror_stores_flag_when_closed() {
    // VI never opened — apply_flip_mirror must not touch the FFI at all,
    // and must still succeed (construction-time seed scenario).
    let mock = MockVideoHalTrait::new();
    let vi = AnykaVideoInput::with_ffi(Arc::new(mock), None);
...
#[tokio::test]
async fn test_apply_flip_mirror_calls_ffi_when_open() {
    let mut mock = mock_ffi_with_successful_open();
...
#[tokio::test]
async fn test_apply_flip_mirror_passes_false_through_when_open() {
    // Reapply path (e.g. re-attach with a stored rotated=false) must send
    // matching flip=mirror=false, not some stale true left over from a
    // previous call.
    let mut mock = mock_ffi_with_successful_open();
    mock.expect_vi_set_flip_mirror()
        .withf(|_, flip, mirror| !*flip && !*mirror)
    ...
    let result = vi.apply_flip_mirror(false);
    assert!(result.is_ok());
    assert!(!vi.rotated());
}
...
#[tokio::test]
async fn test_video_control_set_flip_mirror_calls_ffi_when_open() {
    let mut mock = mock_ffi_with_successful_open();
    mock.expect_vi_set_flip_mirror()
        .withf(|_, flip, mirror| *flip && *mirror)
        .times(1)
...
#[tokio::test]
async fn test_video_control_set_flip_mirror_propagates_ffi_error() {
    let mut mock = mock_ffi_with_successful_open();
    mock.expect_vi_set_flip_mirror()
        .times(1)
