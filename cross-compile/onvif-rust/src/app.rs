...
pub(crate) fn apply_configured_timezone(cfg: &AppConfig) {
    // Tests run app boots in parallel; serialize with the other tz-cell
    // writers. Boot is single-threaded in production, so no lock there.
    #[cfg(test)]
...
impl Application {
    ...
    async fn start_internal(
        ...
        let diagnostics = Arc::new(
            crate::diagnostics::state::DiagnosticsState::with_update_root(
                started_at,
