#!/bin/bash
# Verify Rust Binary for ARMv5TEJ
...
# Go up 1 level: scripts -> rust-hello
PROJECT_DIR="$(cd "${SCRIPT_DIR}/.." && pwd)"
BUILD_DIR="${PROJECT_DIR}/build"
BINARY_NAME="rust-hello"
BINARY_PATH="${BUILD_DIR}/bin/${BINARY_NAME}"

# Try alternative paths
if [ ! -f "${BINARY_PATH}" ]; then
  # Try target directory (current target: armv5te-unknown-linux-gnueabi)
  TARGET_BINARY="${PROJECT_DIR}/target/armv5te-unknown-linux-gnueabi/release/${BINARY_NAME}"
  if [ -f "${TARGET_BINARY}" ]; then
    BINARY_PATH="${TARGET_BINARY}"
  else
    TARGET_BINARY="${PROJECT_DIR}/target/armv5te-unknown-linux-gnueabi/debug/${BINARY_NAME}"
    if [ -f "${TARGET_BINARY}" ]; then
      BINARY_PATH="${TARGET_BINARY}"
    else
      # Try old target name (for backwards compatibility)
      TARGET_BINARY="${PROJECT_DIR}/target/arm-unknown-linux-uclibcgnueabi/release/${BINARY_NAME}"
      if [ -f "${TARGET_BINARY}" ]; then
        BINARY_PATH="${TARGET_BINARY}"
      else
        TARGET_BINARY="${PROJECT_DIR}/target/arm-unknown-linux-uclibcgnueabi/debug/${BINARY_NAME}"
        if [ -f "${TARGET_BINARY}" ]; then
          BINARY_PATH="${TARGET_BINARY}"
        fi
      fi
...
test_binary_exists() {
  ...
  if [ -f "${BINARY_PATH}" ]; then
    ...
    log_info "  - ${BUILD_DIR}/bin/${BINARY_NAME}"
    log_info "  - ${PROJECT_DIR}/target/armv5te-unknown-linux-gnueabi/release/${BINARY_NAME}"
    log_info "  - ${PROJECT_DIR}/target/armv5te-unknown-linux-gnueabi/debug/${BINARY_NAME}"
    log_info "  - ${PROJECT_DIR}/target/arm-unknown-linux-uclibcgnueabi/release/${BINARY_NAME}"
    log_info "  - ${PROJECT_DIR}/target/arm-unknown-linux-uclibcgnueabi/debug/${BINARY_NAME}"
    return 1
  fi
}
...
test_elf_flags() {
  ...
  # Note: This is a heuristic - the exact flag value may vary
...
test_no_vfp_instructions() {
  ...
  # Find cross-objdump
  OBJDUMP=""
  if command -v arm-anykav200-linux-uclibcgnueabi-objdump &> /dev/null; then
...
# Test 7: Check ABI attributes (if readelf supports it)
test_abi_attributes() {
  log_info "Test 7: Checking ABI attributes..."

  # Find cross-readelf
  READELF=""
  if command -v arm-anykav200-linux-uclibcgnueabi-readelf &> /dev/null; then
...
# Test 8: Check dynamic linking
test_dynamic_linking() {
  log_info "Test 8: Checking dynamic linking..."

  # Find cross-ldd or readelf
  if command -v arm-anykav200-linux-uclibcgnueabi-readelf &> /dev/null; then
    READELF="arm-anykav200-linux-uclibcgnueabi-readelf"
  elif command -v readelf &> /dev/null; then
    READELF="readelf"
...
# Main execution
main() {
  echo "=== Verifying Rust Binary for ARMv5TEJ ==="
