...
- [ ] Create/switch to `chore/toolchain-bump` from current tip (`chore/deps-bump` or equivalent).
- [ ] In `scripts/common.sh` set:
  - `LLVM_VERSION="22.1.8"`
  - `RUST_VERSION="1.97.1"`
  - `GDB_VERSION="17.2"`
  - Leave `CROSSTOOL_NG_VERSION="1.28.0"` and `UCLIBC_NG_VERSION="1.0.57"`.
- [ ] Re-run `check_upstream_versions` and confirm expected “held / up to date / update applied” story.
...
- [ ] Ensure host deps via builder’s dependency check (`build.sh` upfront check).
- [ ] Ensure host `~/.cargo/bin/rustc` and `cargo` exist for Rust bootstrap.
- [ ] From `toolchain/toolchain-builder`:

```bash
./build.sh --clean 2>&1 | tee /tmp/toolchain-build.log
```

- [ ] If a stage fails, fix the specific stage script (no blind version rollback of uClibc), then `./build.sh --resume`.
- [ ] On success, confirm install trees under `../arm-anykav200-crosstool-ng/`.
...
- [ ] Regenerate cargo config if project scripts require it (`cross-compile/onvif-rust/scripts/setup-cargo-config.sh` or documented equivalent).
- [ ] Confirm `rustc --print target-list | rg armv5te-unknown-linux-uclibceabi`.
...
- [ ] Fix any rustc-1.97 breakages in app/tests (no shims).
...
- [ ] Fix link/target issues if any (sysroot, clang wrapper, openssl-src patch).
...
- [ ] If 1.97 satisfies prior pins (e.g. `constant_time_eq` 0.5), bump those crates and re-run host tests.
- [ ] If a crate still needs newer rustc, leave pinned and note in commit message.
...
- [ ] Commit tracked changes (pins, stage fixes, specs/plans). **Do not** try to commit the binary install tree.
